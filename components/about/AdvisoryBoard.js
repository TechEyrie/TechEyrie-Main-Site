...
const AdvisoryBoard = ({ theme = "light" }) => {
  ...
  return (
    <section className={`overflow-hidden border-b transition-colors duration-500 ${isDark ? 'bg-[#162d24] text-[#f3f3f3] border-[#e0d1b6]/12' : 'bg-white text-black border-gray-100'}`}>
          ...
          <div className="relative z-10 max-w-6xl mx-auto">
            ...
            <div className="mb-8">
               <MaskedReveal>
                 ...
                 Elevating operations, Redefining brands
                 </span>
               </MaskedReveal>
            </div>
      ...
      {/* ADVISORS GRID — static luxury gradient (no GridAnimation here) */}
      <div className="relative overflow-hidden px-6 md:px-12 pb-24 md:pb-32 mt-16 pt-8 md:pt-12">
        ...
        <div className={`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 border-t border-l ${isDark ? 'border-[#e0d1b6]/12' : 'border-gray-100'}`}>
          {ADVISORS.map((advisor, index) => (
             <div 
               ...
               className={`relative group border-r border-b ${isDark ? 'border-[#e0d1b6]/12' : 'border-gray-100'} p-8 md:p-12 flex flex-col items-center backdrop-blur-sm ${isDark ? 'bg-white/[0.04]' : 'bg-white/50'}`}
             >
                {/* Spotlight Glow */}
                <div 
