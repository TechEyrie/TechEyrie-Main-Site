...
const Hero = ({ theme = "light" }) => {
  ...
  return (
    <section className={`relative pt-48 pb-32 md:pt-[30vh] md:pb-40 px-6 md:px-12 border-b ${isDark ? 'border-[#e0d1b6]/12' : 'border-gray-200'} overflow-hidden transition-colors duration-500 ${isDark ? 'bg-[#162d24]' : ''}`}>
      <GridAnimation theme={theme} />

      <div className="relative z-10 max-w-[90%] mx-auto text-center">
        <div className="mb-12">
          <SectionLabel text="About Tech Eyrie" className={`font-merriweather about-lux-label text-[13px] md:text-[15px] font-semibold tracking-[0.16em] ${isDark ? "" : "text-gray-300"}`} />
        </div>
        
        <div className="flex flex-col mb-8 md:mb-12">
          <MaskedReveal className="h-auto pb-1 md:pb-4">
            <h1 className={`font-italiana font-light text-[32px] sm:text-[42px] md:text-[58px] lg:text-[65px] xl:text-[75px] 2xl:text-[85px] leading-[1.05] tracking-[-0.03em] ${isDark ? 'text-[#f3f3f3]' : 'text-gray-900'}`}>
            Redefining Industries, Not

            </h1>
          </MaskedReveal>
          <MaskedReveal delay={0.15} className="h-auto pb-1 md:pb-4">
            <h1 className={`font-italiana font-light text-[32px] sm:text-[42px] md:text-[58px] lg:text-[72px] xl:text-[88px] 2xl:text-[104px] -mt-[0.2rem] sm:-mt-[0.3rem] md:-mt-[0.4rem] leading-[1.05] tracking-[-0.03em] ${isDark ? 'text-[#f3f3f3]' : 'text-gray-900'}`}>
