...
const Investors = ({ theme = "light" }) => {
  ...
  return (
    <section className={`overflow-hidden border-b transition-colors duration-500 ${isDark ? 'bg-[#162d24] text-[#f3f3f3] border-[#e0d1b6]/12' : 'bg-white text-black border-gray-100'}`}>
      ...
      {/* INVESTORS GRID — static luxury gradient (no GridAnimation here) */}
      <div className="relative overflow-hidden px-6 md:px-12 pb-24 md:pb-32 pt-8 md:pt-12">
        {isDark && <AboutLuxuryStaticBg />}
        <div className="relative z-[1] max-w-[1800px] mx-auto text-center">
        <div className={`grid grid-cols-1 md:grid-cols-2 max-w-5xl mx-auto border-t border-l ${isDark ? 'border-[#e0d1b6]/12' : 'border-gray-100'}`}>
          {INVESTORS.map((investor, index) => (
             <div 
               ...
               className={`relative group border-r border-b ${isDark ? 'border-[#e0d1b6]/12' : 'border-gray-100'} p-8 md:p-12 lg:p-16 flex flex-col items-center overflow-hidden backdrop-blur-sm ${isDark ? 'bg-white/[0.04]' : 'bg-white/50'}`}
             >
                {/* Spotlight Glow */}
                <div 
                ...
                {/* Image */}
                <div className={`relative w-full aspect-square max-w-[400px] mb-8 overflow-hidden z-10 transition-transform duration-500 will-change-transform group-hover:scale-105 ${isDark ? 'bg-[#101e27]' : 'bg-gray-50'}`}>
                   <img 
                     src={investor.image} 
                     alt={investor.name} 
