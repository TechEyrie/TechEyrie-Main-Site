 ...
 *
 * Copy of the original HeroSection for the dark page.
 * - First (hero) section is unchanged.
 * - Second (portfolio) section now has an explicit media slot inside each card.
 *   The hero cards land inside this slot, which:
 *     - Is 100% of the card width.
 *     - Uses a fixed aspect ratio for predictable layout.
 *     - Scales responsively together with the card.
 */
export default function HeroSectionMediaSlot({ theme = "light" }) {
  ...
  // --- GSAP Scroll Animation: hero content + fly hero cards into portfolio slots ---
  useLayoutEffect(() => {
    if (
      !isDesktop ||
    ...
    const ctx = gsap.context(() => {
      ...
      // Fly hero cards from hero section into portfolio slot placeholders
      setTimeout(() => {
        mediaAssets.forEach((_, index) => {
          ...
          ScrollTrigger.create({
            ...
            onUpdate: (self) => {
              ...
              // Start position: card's top-left in viewport (container + stack offset)
              ...
              // Scale so card width matches slot width exactly
              const targetScale = targetWidth / heroWidth;
              const startScale = 1 - index * 0.05;
              const currentScale =
                startScale + (targetScale - startScale) * progress;

              // End position: slot's top-left in viewport (card will sit exactly in slot)
  ...
  return (
    <div ref={containerRef} className="w-full">
      {/* --- FIRST SECTION: identical hero section --- */}
      <section
        ref={heroSectionRef}
      ...
      {/* --- SECOND SECTION: portfolio with explicit 100%-width media slot inside each card --- */}
      <section
        ref={portfolioSectionRef}
        className="w-full min-h-screen py-0 relative -mt-[20vh] overflow-visible"
      ...
      >
        ...
        <div className="w-full max-w-[1800px] mx-auto px-4 sm:px-6 lg:px-8 pt-[calc(20vh+2rem)] sm:pt-[calc(20vh+3rem)] md:pt-[calc(20vh+4rem)]">
          {/* Portfolio section headings — visible above the grid */}
          <header className="text-center mb-12 sm:mb-16">
            <p
              className={`font-merriweather text-sm sm:text-base md:text-lg mb-4 sm:mb-6 ${theme === "dark" ? "text-white/70" : "text-[#111111]/70"}`}
            >
              Lorem Ipsum
            </p>
            <h2
              className={`font-italiana text-3xl sm:text-4xl md:text-5xl lg:text-6xl xl:text-7xl leading-tight ${theme === "dark" ? "text-white" : "text-[#111111]"}`}
            >
          ...
          {/* Portfolio grid: each card has one reference slot (100% width); media lives inside it */}
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {mediaAssets.map((item, index) => (
              <div key={index} className="flex flex-col items-center">
                ...
                >
                  {/* Card background (covers full card: slot + text); shows on hover */}
                  <div
                  ...
                  {/* Reference location: hero cards fly here on scroll; 100% card width */}
                  <div
                    ref={(el) => {
                    ...
                    style={{
                      aspectRatio: "3 / 4",
                      zIndex: 10,
                      pointerEvents: isDesktop && !isInHeroSection ? "none" : "auto",
                    }}
                    onMouseEnter={() => !isDesktop && setHoveredCard(index)}
                    onMouseLeave={() => !isDesktop && setHoveredCard(null)}
                  >
                    {/* On desktop the flying hero card provides the media; on mobile render it here */}
                    {!isDesktop && (
                      <>
                        {item.type === "image" ? (
                          <Image
                    ...
                    {/* Triangle overlay on hover (mobile) */}
                    {!isDesktop && (
                      <div
                  ...
                  {/* Text block below the slot — fixed height so every card is same length */}
                  <div
