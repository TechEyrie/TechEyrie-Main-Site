...
import { neat2GradientConfig } from "./neatGradientConfig";
...
export default function HeroSectionMediaSlot({ theme = "light", sharedBackground = false }) {
  ...
  const triggerElectricalAnimation = useCallback(() => {
    ...
    const originalColor = theme === "dark" ? "#f5fbf7" : "#111111";
    const electricColor = theme === "dark" ? "#ffffff" : "#2a5c48";
  ...
  return (
    ...
    >
      ...
      <section
        ref={heroSectionRef}
        className="dark7-neat2-hero relative overflow-visible pt-32 md:pt-40 pb-0"
        style={{
      ...
      >
        {theme === "dark" && (
          <NeatHeroBackground containerRef={heroSectionRef} config={neat2GradientConfig} />
        )}
        {theme === "dark" && (
          <div
        ...
        >
          <button onClick={scrollToPortfolio} className="flex flex-col gap-[-2px] cursor-pointer group hover:scale-110 transition-transform duration-300" aria-label="Scroll to next section">
            {[0, 1, 2, 3].map((index) => {
              ...
              return (
                <svg key={index} className={`dark7-neat2-hero-scroll w-4 h-4 md:w-4 md:h-4 transition-colors duration-300 -my-0.5 ${isActive ? "is-active" : ""}`} viewBox="0 0 24 24" fill="none">
