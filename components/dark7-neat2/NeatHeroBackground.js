  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      gradient.horizontalPressure = clamp(
        base.horizontalPressure + currentX * 2.4 * (0.35 + influence * 0.65),
        0,
        10,
      );
      gradient.verticalPressure = clamp(
        base.verticalPressure - currentY * 2.1 * (0.35 + influence * 0.65),
        0,
        10,
      );
      gradient.colorBrightness = base.colorBrightness + influence * 0.28;
      gradient.colorSaturation = base.colorSaturation + influence * 0.85;
      gradient.highlights = clamp(base.highlights + influence * 1.2, 0, 10);
      gradient.shadows = clamp(base.shadows - influence * 0.25, 0, 10);
      gradient.colorBlending = clamp(
        base.colorBlending + influence * 0.6,
        0,
        10,
      );
      gradient.flowDistortionA = base.flowDistortionA + currentX * 0.1 * influence;
