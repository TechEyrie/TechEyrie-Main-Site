...
const SERVICES_WING_ID = "dark7-v11-services-right-wing";

/**
 * ═══════════════════════════════════════════════════════════════════════════
 * SERVICES RIGHT-WING TUNING
 * File: components/dark7-v11/ServicesRightWingScene.js
 * Page: /dark7-v11  →  New Services section (left side)
 *
 ...
 * Note: bird is mirrored (scale.x = -1) so you see the RIGHT wing.
 * ═══════════════════════════════════════════════════════════════════════════
 */
...
export default function ServicesRightWingScene({ sectionRef, className = "" }) {
  ...
  useLayoutEffect(() => {
    ...
    scene.add(new THREE.AmbientLight(0xb8dcc8, 1.25));
    const keyLight = new THREE.DirectionalLight(0xf0fff6, 2.5);
    keyLight.position.set(-2.2, 6, 4);
    scene.add(keyLight);
    scene.add(new THREE.HemisphereLight(0x5ad898, 0x021808, 1.1));
    const jadeLight = new THREE.PointLight(0x00d868, 4.2, 56);
    jadeLight.position.set(-3.5, 2, 4);
    scene.add(jadeLight);
    const rimLight = new THREE.PointLight(0xa8ffe8, 2.6, 48);
    ...
    function applyBirdPose() {
      if (!birdObject) return;
      // Flip on X so the RIGHT wing faces the camera (hero shows the left wing).
      // Do not remove the leading minus unless you want the left wing instead.
    ...
    function createScrollAnimation(section) {
      ...
      gsapCtx = gsap.context(() => {
        ...
        gsap.to(state, {
          progress: 1,
          ease: "none",
          scrollTrigger: Dark7V11ScrollTrigger({
            id: SERVICES_WING_ID,
            trigger: section,
      ...
      requestAnimationFrame(() => refreshDark7V11ScrollTriggers());
      window.setTimeout(() => refreshDark7V11ScrollTriggers(true), 400);
    }
    ...
    gltfLoader.load(
      ...
      (gltf) => {
        ...
        birdMaterials = applyBirdMaterial(birdObject, { featherNormal }, false);
