...
import { Dark7V12ScrollTrigger, subscribeAfterScrollLayout } from "./lenisScrollTrigger";
import {
  DARK7_GRADIENTS,
...
function MobileAirvoirSection({ theme }) {
  ...
  return (
    <div className="relative w-full max-w-full min-w-0 overflow-x-clip -mt-px">
      <section
        className="dark7-v12-airvoir relative overflow-hidden transition-colors duration-500"
        style={bgStyle}
      >
        <SectionOverlays />
...
function DesktopAirvoirSection({ theme }) {
  ...
  useLayoutEffect(() => {
    ...
    const ctx = gsap.context(() => {
      const build = () => {
        ...
        ScrollTrigger.getById("dark7-v12-airvoir")?.kill();

        const mainTl = gsap.timeline({
          scrollTrigger: Dark7V12ScrollTrigger({
            id: "dark7-v12-airvoir",
            trigger: section,
            start: "top top",
            end: "+=140%",
            scrub: 0.6,
  ...
  return (
    <section
      ref={sectionRef}
      className="dark7-v12-airvoir relative -mt-px overflow-hidden transition-colors duration-500 min-h-screen"
      style={bgStyle}
    >
      <SectionOverlays />
