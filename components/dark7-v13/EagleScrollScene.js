...
/**
 * Hero top feathers — sharp dark→light leaf greens (Noomo-style)
 * root/shadow = base of leaf | tip/highlight = outer tip
 */
...
/**
 * Hero bottom / stretched leaves — same dark→light idea, slightly brighter tips
 */
...
const FEATHER_PAINT_STRENGTH = 0.996;
const WING_TIP_ZONE_START = 0.22;
const WING_TIP_BLEND_STRENGTH = 0.85;
...
function applyFeatherSurfaceShader(
  ...
  material.customProgramCacheKey = () =>
    `feather-leaf-smooth-grad-v2-${isWing ? 1 : 0}-${heroWingLook ? 1 : 0}-${axis?.axisIdx ?? "b"}`;
...
float featherNoise(vec2 p) {
  ...
  if (uHeroWingLook > 0.5) {
    // Multi-stop dark → mid → lime → tip along the leaf (root=0 … tip=1)
    float gradPow = mix(0.85, 1.35, leafId);
    float t = pow(wingT, gradPow);
    t = clamp(t + (leafId2 - 0.5) * 0.12, 0.0, 1.0);

    // Extra vertical-ish band variation so downward leaves hold several greens
    float bandShift = (leafId3 - 0.5) * 0.08;
    float t1 = clamp(t + bandShift, 0.0, 1.0);
    float t2 = clamp(t - bandShift * 0.6, 0.0, 1.0);

    vec3 cDeep = mix(uRootColor, uShadowColor, leafId * 0.55);
    vec3 cForest = mix(uAltColor, uStripeColor, leafId2);
    vec3 cMid = mix(uBaseColor, uVaneColor, 0.35 + leafId3 * 0.45);
    vec3 cLime = mix(uHighlightColor, uCoolColor, leafId);
    vec3 cTip = mix(uTipColor, vec3(0.78, 1.0, 0.32), leafId2 * 0.4);
    // Second lime family for bottom leaves (more yellowish)
    vec3 cLime2 = mix(uCoolColor, uTipColor, 0.5 + leafId3 * 0.35);

    // 5-stop gradient along the leaf
    vec3 feather = cDeep;
    feather = mix(feather, cForest, smoothstep(0.0, 0.22, t1));
    feather = mix(feather, cMid, smoothstep(0.18, 0.48, t));
    feather = mix(feather, cLime, smoothstep(0.42, 0.72, t2));
    feather = mix(feather, mix(cLime2, cTip, leafId), smoothstep(0.65, 1.0, t));

    // Soft side-to-side green shift (no hard lines)
    float side = featherNoise(vec2(vObjectPos.x * 2.4 + uMeshSeed, wingT * 3.0));
    feather = mix(feather, cForest, (1.0 - t) * side * 0.18);
    feather = mix(feather, cLime2, t * (1.0 - side) * 0.16);

    // Soft paint flecks only
    feather = mix(feather, cDeep, (1.0 - softGrain) * 0.06 * (1.0 - t));
    feather = mix(feather, cTip, softGrain2 * 0.08 * t);

    feather = mix(feather, cTip, tip * uTipBlendStrength * 0.85);
    feather = mix(feather, cLime, ndl * 0.05 * t);
    feather = mix(feather, cDeep, (1.0 - ndl) * 0.04 * (1.0 - t));
...
export default function EagleScrollScene({
  ...
  useLayoutEffect(() => {
    ...
    function createScrollAnimation(triggerEl) {
      ...
      // Initial camera for embedded hero (dark7-v13).
      ...
      window.setTimeout(() => {
        ...
        const progress =
          getDark7_V13ScrollTop() < 8 ? 0 : (scrollTween?.scrollTrigger?.progress ?? 0);
        applyScrollProgress(progress);
        onScrollProgressRef.current?.(progress <= 0.001 ? 0 : progress);
        if (embeddedScroll) {
          notifyHeroPinReady();
