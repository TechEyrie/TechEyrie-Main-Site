...
/**
 * Hero leaf families — each entry is a distinct leaf personality.
 * root/shadow = dark base | tip/highlight = bright tip
 * Tuned for v14: bigger contrast so every leaf reads separately.
 */
const HERO_TOP_PALETTE = [
  {
    // Deep forest → neon lime
    hue: 0.33,
    color: "#0A7A22",
    shadow: "#032810",
    highlight: "#6EFF3A",
...
/** Stretched / lower leaves — brighter mid & tip so they separate from upper ones */
...
function applyFeatherSurfaceShader(
  ...
  material.customProgramCacheKey = () =>
    `feather-leaf-vivid-v14-${isWing ? 1 : 0}-${heroWingLook ? 1 : 0}-${axis?.axisIdx ?? "b"}`;
...
float featherNoise(vec2 p) {
  ...
  if (uHeroWingLook > 0.5) {
    ...
    vec3 accentWarm = vec3(0.92, 1.0, 0.28);
    vec3 accentCool = vec3(0.35, 0.98, 0.72);
    vec3 accentNeon = vec3(0.45, 1.0, 0.22);
    ...
    feather = mix(feather, cDeep, rim * 0.22);

    // Soft flecks only
    feather = mix(feather, cDeep, (1.0 - softGrain) * 0.08 * (1.0 - t));
...
export default function EagleScrollScene({
  ...
  useLayoutEffect(() => {
    ...
    function createScrollAnimation(triggerEl) {
      ...
      // Initial camera for embedded hero (dark7-v14).
      ...
      ScrollTrigger.getById(DARK7_V14_HERO_PIN_ID)?.kill();
      ...
      gsapCtx = gsap.context(() => {
        ...
        scrollTween = gsap.to(state, {
          ...
          scrollTrigger: Dark7V14ScrollTrigger({
            id: embeddedScroll ? DARK7_V14_HERO_PIN_ID : "eagle-scroll-scene",
            trigger: triggerEl,
            start: "top top",
      ...
      requestAnimationFrame(() => {
        refreshDark7V14ScrollTriggers();
        const progress =
          getDark7V14ScrollTop() < 8 ? 0 : (scrollTween?.scrollTrigger?.progress ?? 0);
      ...
      window.setTimeout(() => {
        ...
        const progress =
          getDARK7_V14ScrollTop() < 8 ? 0 : (scrollTween?.scrollTrigger?.progress ?? 0);
        applyScrollProgress(progress);
        onScrollProgressRef.current?.(progress <= 0.001 ? 0 : progress);
        if (embeddedScroll) {
          notifyHeroPinReady();
