...
/** Lenis + ScrollTrigger scroller for all dark7-v16 sections. */
export const DARK7_V16_SCROLLER =
...
export function getDark7V16ScrollTop() {
  if (lenisInstance) return lenisInstance.scroll;
  if (DARK7_V16_SCROLLER) {
    return ScrollTrigger.getScrollFunc(DARK7_V16_SCROLLER)?.() ?? window.scrollY;
  }
  return window.scrollY;
}

/** True when any pinned ScrollTrigger is currently active. */
export function hasActiveDark7V16Pin() {
  return ScrollTrigger.getAll().some(
    (st) => st.isActive && Boolean(st.vars?.pin || st.pin),
  );
}

/** Merge dark7-v16 scroll defaults into a ScrollTrigger config object. */
export function Dark7V16ScrollTrigger(vars = {}) {
  return {
    scroller: DARK7_V16_SCROLLER,
 ...
 */
function applyDark7V16ScrollerProxy(lenis) {
  if (!DARK7_V16_SCROLLER) return;

  // Lenis always transforms documentElement — "fixed" pins shift downstream triggers.
  const pinType = lenis ? "transform" : DARK7_V16_SCROLLER.style.transform ? "transform" : "fixed";

  ScrollTrigger.scrollerProxy(DARK7_V16_SCROLLER, {
    scrollTop(value) {
      if (arguments.length) {
...
export function initDark7V16LenisScroll(lenis) {
  if (typeof window === "undefined" || !DARK7_V16_SCROLLER) return;

  lenisInstance = lenis;
  layoutSettled = false;

  applyDark7V16ScrollerProxy(lenis);

  const onRefreshInit = () => applyDark7V16ScrollerProxy(lenis);
  ...
  lenis.on("scroll", () => {
    ...
    if (typeof window !== "undefined") {
      window.dispatchEvent(
        new CustomEvent("dark7-v16-scroll", { detail: lenis.scroll }),
      );
    }
  });
...
export function destroyDark7V16LenisScroll(lenis) {
  lenisInstance = null;
  layoutSettled = false;
  removeRefreshInitListener?.();
 ...
 */
export function refreshDark7V16ScrollTriggers(hard = true) {
  if (typeof window === "undefined") return;

  if (hard && layoutSettled && hasActiveDark7V16Pin()) {
    ScrollTrigger.update();
    return;
  }
...
export function markDark7V16ScrollLayoutSettled() {
  layoutSettled = true;
}

export const DARK7_V16_SCROLL_LAYOUT_READY_EVENT = "dark7-v16-scroll-layout-ready";
...
export function notifyScrollLayoutReady() {
  ...
  refreshDark7V16ScrollTriggers(true);
  window.dispatchEvent(new Event(DARK7_V16_SCROLL_LAYOUT_READY_EVENT));

  requestAnimationFrame(() => refreshDark7V16ScrollTriggers(true));
  window.setTimeout(() => refreshDark7V16ScrollTriggers(true), 120);
}
...
export function subscribeAfterScrollLayout(callback) {
  ...
  const run = () => {
    ...
    callback();
    requestAnimationFrame(() => {
      if (!cancelled) refreshDark7V16ScrollTriggers(true);
    });
  };
  ...
  const onReady = () => {
    window.removeEventListener(DARK7_V16_SCROLL_LAYOUT_READY_EVENT, onReady);
    window.clearTimeout(fallbackTimer);
    run();
  };

  window.addEventListener(DARK7_V16_SCROLL_LAYOUT_READY_EVENT, onReady);
...
export const DARK7_V16_HERO_PIN_ID = "dark7-v16-hero-pin";
