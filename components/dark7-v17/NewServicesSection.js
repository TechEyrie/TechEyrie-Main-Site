...
function ServiceArrowLink({ href, label, className = "", onClick }) {
  return (
    <Link
      href={href}
      onClick={onClick}
      className={`relative flex h-6 w-6 sm:h-7 sm:w-7 items-center justify-center overflow-hidden rounded-[4px] dark7-v17-header-arrow-btn transition-all duration-500 ease-out group-hover:scale-110 group-hover:-translate-y-[1px] flex-shrink-0 ${className}`}
      aria-label={label}
    >
...
export default function NewServicesSection({ theme = "dark", sharedBackground = false }) {
  ...
  useEffect(() => {
    ...
    style.innerHTML = `
      ...
      .new-services-card:hover .dark7-v17-header-arrow-btn {
  ...
  return (
    <>
      <section
        ref={sectionRef}
        className="dark7-v17-new-services relative overflow-hidden py-0 bg-transition"
        style={
        ...
        <div className="relative z-10 mx-auto max-w-[1800px] px-4 sm:px-6 md:px-8">
          <div className="new-services-fade mb-6 flex items-center gap-2 sm:mb-8 sm:gap-3">
            <Image
              ...
              aria-hidden
            />
            <span className="dark7-v17-section-eyebrow font-playfair font-normal text-[#162D24]">
              Our services
            </span>
          </div>
