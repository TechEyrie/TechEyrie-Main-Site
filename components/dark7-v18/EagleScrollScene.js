...
import {
  Dark7V18ScrollTrigger,
  getDark7V18ScrollTop,
  refreshDark7V18ScrollTriggers,
  notifyHeroPinReady,
  DARK7_V18_HERO_PIN_ID,
...
/**
 * Hero leaf families — v18 — chartreuse / acid green (high contrast Noomo)
 * root/shadow = dark base | tip/highlight = bright tip
 */
const HERO_TOP_PALETTE = [
  { hue: 0.26, color: "#4AB008", shadow: "#1C4804", highlight: "#B8FF18", emissive: "#0E2802", tip: "#F8FF30", root: "#0A1C02", stripe: "#388808", vane: "#78D010", alt: "#2C6808", cool: "#A0F020" },
  { hue: 0.30, color: "#28B010", shadow: "#104808", highlight: "#98FF28", emissive: "#082804", tip: "#E8FF38", root: "#061C04", stripe: "#1E880C", vane: "#50D018", alt: "#186810", cool: "#80F828" },
  { hue: 0.24, color: "#58A800", shadow: "#244400", highlight: "#C8F800", emissive: "#122400", tip: "#FFFF28", root: "#0E1C00", stripe: "#408000", vane: "#88C800", alt: "#346400", cool: "#B0E810" },
  { hue: 0.32, color: "#10B818", shadow: "#064C0C", highlight: "#80FF30", emissive: "#032C06", tip: "#D0FF40", root: "#022004", stripe: "#0C9014", vane: "#28D820", alt: "#0A7014", cool: "#68F838" },
  { hue: 0.28, color: "#3CB008", shadow: "#184804", highlight: "#A8FF20", emissive: "#0C2802", tip: "#F0FF34", root: "#081C02", stripe: "#2C8808", vane: "#68D012", alt: "#24680A", cool: "#90F024" },
  { hue: 0.34, color: "#08B028", shadow: "#044814", highlight: "#70FF40", emissive: "#022808", tip: "#C8FF50", root: "#011C06", stripe: "#06881C", vane: "#18D030", alt: "#066818", cool: "#58F848" },
];

/** Lower / stretched leaves */
const HERO_BOTTOM_PALETTE = [
  { hue: 0.27, color: "#60C008", shadow: "#285008", highlight: "#D0FF28", emissive: "#142C04", tip: "#FFFF40", root: "#0E2002", stripe: "#489810", vane: "#90E018", alt: "#38780C", cool: "#B8F830" },
  { hue: 0.31, color: "#30C818", shadow: "#14540C", highlight: "#A8FF38", emissive: "#0A3006", tip: "#F0FF50", root: "#062404", stripe: "#24A014", vane: "#58E828", alt: "#1C8014", cool: "#90FF40" },
  { hue: 0.25, color: "#70B800", shadow: "#304C00", highlight: "#E0F800", emissive: "#182800", tip: "#FFFF20", root: "#101C00", stripe: "#589000", vane: "#A0D000", alt: "#447000", cool: "#C8E808" },
  { hue: 0.33, color: "#18C828", shadow: "#0A5414", highlight: "#90FF42", emissive: "#04300A", tip: "#E0FF58", root: "#022406", stripe: "#12A020", vane: "#38E834", alt: "#0E8020", cool: "#78FF4C" },
  { hue: 0.29, color: "#48C810", shadow: "#1C5408", highlight: "#C0FF30", emissive: "#0E3004", tip: "#F8FF44", root: "#0A2402", stripe: "#38A00C", vane: "#78E820", alt: "#2C800C", cool: "#A8F838" },
  { hue: 0.35, color: "#10C838", shadow: "#06541C", highlight: "#88FF55", emissive: "#03300E", tip: "#D8FF70", root: "#022408", stripe: "#0CA02C", vane: "#28E848", alt: "#0A8028", cool: "#70FF60" },
];
...
function applyFeatherSurfaceShader(
  ...
  material.customProgramCacheKey = () =>
    `feather-leaf-acid-v18-${isWing ? 1 : 0}-${heroWingLook ? 1 : 0}-${axis?.axisIdx ?? "b"}`;
...
float featherNoise(vec2 p) {
  ...
  if (uHeroWingLook > 0.5) {
    ...
    vec3 accentWarm = vec3(0.98, 1.0, 0.18);
    vec3 accentCool = vec3(0.70, 1.0, 0.35);
    vec3 accentNeon = vec3(0.85, 1.0, 0.12);
    vec3 accentPick = family < 1.5 ? accentNeon : (family < 3.5 ? accentWarm : accentCool);
    ...
    feather = mix(feather, cDeep, rim * 0.14);

    // Soft flecks only
    feather = mix(feather, cDeep, (1.0 - softGrain) * 0.07 * (1.0 - t));
    feather = mix(feather, cTipHot, softGrain2 * 0.18 * t);
    ...
    feather = mix(vec3(luma), feather, 1.36);
...
export default function EagleScrollScene({
  ...
  useLayoutEffect(() => {
    ...
    function createScrollAnimation(triggerEl) {
      ...
      // Initial camera for embedded hero (dark7-v18).
      ...
      ScrollTrigger.getById(DARK7_V18_HERO_PIN_ID)?.kill();
      ...
      gsapCtx = gsap.context(() => {
        ...
        scrollTween = gsap.to(state, {
          progress: 1,
          ease: "none",
          scrollTrigger: Dark7V18ScrollTrigger({
            id: embeddedScroll ? DARK7_V18_HERO_PIN_ID : "eagle-scroll-scene",
            trigger: triggerEl,
            start: "top top",
      ...
      requestAnimationFrame(() => {
        refreshDark7V18ScrollTriggers();
        const progress =
          getDark7V18ScrollTop() < 8 ? 0 : (scrollTween?.scrollTrigger?.progress ?? 0);
        applyScrollProgress(progress);
        onScrollProgressRef.current?.(progress <= 0.001 ? 0 : progress);
      });

      window.setTimeout(() => {
        refreshDark7V18ScrollTriggers(true);
        const progress =
          getDARK7_V18ScrollTop() < 8 ? 0 : (scrollTween?.scrollTrigger?.progress ?? 0);
        applyScrollProgress(progress);
        onScrollProgressRef.current?.(progress <= 0.001 ? 0 : progress);
        if (embeddedScroll) {
          notifyHeroPinReady();
