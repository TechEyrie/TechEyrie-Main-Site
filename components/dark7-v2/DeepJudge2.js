...
export default function DeepJudge2({ theme }) {
  ...
  const circleBlobStyle = surfaceIsLight || !isDark ? { backgroundColor: `${lightColors.tertiary}B3`, borderColor: `${lightColors.tertiary}66` } : {};
  const subTextColor = surfaceIsLight ? 'text-slate-600' : isDark ? 'text-[#a0a0a0]' : 'text-slate-600';
  const cardBg = surfaceIsLight || !isDark ? 'bg-white/95 border-gray-200' : 'bg-neutral-900/95 border-neutral-800';
  const cardBgStyle = surfaceIsLight || !isDark ? { backgroundColor: `${lightColors.background}F2`, borderColor: `${lightColors.tertiary}80` } : {};
  const cardTitle = surfaceIsLight ? 'text-slate-900' : isDark ? 'text-[#f3f3f3]' : 'text-slate-900';
  const cardDesc = surfaceIsLight ? 'text-slate-500' : isDark ? 'text-[#d0d0d0]' : 'text-slate-500';
  const badgeBg = surfaceIsLight || !isDark ? '' : 'bg-neutral-800 text-white';
  const badgeBgStyle = surfaceIsLight || !isDark ? { backgroundColor: lightColors.tertiary, color: "#000000" } : {};
    ...
    let ctx = gsap.context(() => {
      ...
      mm.add("(min-width: 1024px)", () => {
        ...
        tl.to(cards, {
            ...
            height: cardHeight,
            borderRadius: "16px",
            backgroundColor: surfaceIsLight ? "rgba(255, 255, 255, 0.95)" : isDark ? "rgba(23, 23, 23, 0.95)" : "rgba(255, 255, 255, 0.95)",
            scale: 1,
            left: (i) => {
  ...
  const bgStyle = {
    background:
      "linear-gradient(to bottom, #c6af8d 0%, #c6af8d 10%, #c4a578 35%, #b69668 50%, #c4a578 65%, #f5f1ee 90%, #f5f1ee 100%)",
  };
            ...
            {FEATURE_CARDS.map((card, i) => (
                ...
                >
                    <div className="card-content opacity-0 p-5 sm:p-6 md:p-7 flex flex-col h-full">
                        <div className={`deep-judge-badge-num w-12 h-12 sm:w-14 sm:h-14 ${badgeBg} rounded-xl flex items-center justify-center font-bold mb-4 sm:mb-5 shrink-0 text-lg sm:text-xl !text-black`} style={badgeBgStyle}>
                            {i+1}
                        </div>
                ...
                {FEATURE_CARDS.map((card, i) => (
                  ...
                  >
                    <div className="flex items-start gap-4">
                      <div
                        className={`deep-judge-badge-num flex h-11 w-11 sm:h-12 sm:w-12 shrink-0 items-center justify-center rounded-xl font-bold text-lg !text-black ${badgeBg}`}
                        style={badgeBgStyle}
                      >
                        {i + 1}
                      </div>
