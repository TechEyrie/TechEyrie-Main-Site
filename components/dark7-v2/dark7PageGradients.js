/**
 * Smooth multi-stop gradients + anti-banding noise for dark7-v2 page surfaces.
 */
...
export function buildSmoothLinearGradient(
  ...
  for (let index = 0; index < stops.length - 1; index += 1) {
    ...
    for (let step = 0; step < steps; step += 1) {
      ...
      parts.push(`${lerpColor(start.color, end.color, progress)} ${position.toFixed(2)}%`);
    }
  }
...
function expandWaypointsToStops(waypoints, subdivisions = 4) {
  ...
  for (let index = 0; index < waypoints.length - 1; index += 1) {
    for (let step = 0; step < subdivisions; step += 1) {
      ...
      stops.push({
        color: lerpColor(waypoints[index], waypoints[index + 1], progress),
        at: position,
      });
    }
  }
...
export const DARK7_AIRPLANE_END = "#263b33";
...
/** Hero: #162d24 plateau → mid greens → #8EAC85 (user spec) */
...
/** One paint for hero (100svh) + portfolio — no seam at #8EAC85 */
export function dark7HeroPortfolioContainerStyle() {
  return {
    ...
    backgroundSize: `100% calc(100% - 100svh), 100% 100svh`,
...
export const DARK7_GRADIENTS = {
  ...
  footer: buildSmoothLinearGradient([
    { color: "#1e352c", at: 0 },
    { color: "#1a3028", at: 34 },
    { color: "#152820", at: 68 },
    { color: "#122218", at: 100 },
