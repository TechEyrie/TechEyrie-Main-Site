...
import {
  Dark7V22ScrollTrigger,
  getDark7V22ScrollTop,
  refreshDark7V22ScrollTriggers,
  notifyHeroPinReady,
  DARK7_V22_HERO_PIN_ID,
...
function applyFeatherSurfaceShader(
  ...
  material.customProgramCacheKey = () =>
    `feather-leaf-vein-scatter-v22-${isWing ? 1 : 0}-${heroWingLook ? 1 : 0}-${axis?.axisIdx ?? "b"}`;
...
float featherVeinRidge(float phase, float width) {
  ...
  if (uHeroWingLook > 0.5) {
    ...
    // ── REAL WING-LEAF PATTERN: veins + scattered barbs + patch gradients ──
    float lx = vObjectPos.x;
    float ly = vObjectPos.y;

    // Organic warp (grown, not ruled)
    float warp = (featherNoise(vec2(ly * 1.6 + uMeshSeed, lx * 2.2)) - 0.5) * 0.032;
    float warp2 = (featherNoise(vec2(ly * 4.2, lx * 3.1 + uMeshSeed * 2.0)) - 0.5) * 0.016;
    float warp3 = (featherNoise(vec2(ly * 9.0 + leafId, lx * 7.0)) - 0.5) * 0.008;
    lx += warp + warp2 + warp3;
    ly += (featherNoise(vec2(lx * 2.0, uMeshSeed)) - 0.5) * 0.024;

    // Midrib — curved central spine with slight thickness variation
    float midCurve = sin(ly * 1.9 + uMeshSeed * 3.0) * 0.018
                   + sin(ly * 4.5 + leafId * 6.0) * 0.007
                   + sin(ly * 11.0 + leafId2) * 0.003;
    float midDist = abs(lx - midCurve);
    float midW = 0.05 + leafId * 0.02 + featherNoise(vec2(ly * 3.0, uMeshSeed)) * 0.012;
    float midrib = 1.0 - smoothstep(0.0, midW, midDist);
    float midribCore = 1.0 - smoothstep(0.0, 0.016, midDist);

    float side = sign(lx + 0.0001);
    float away = abs(lx);

    // Secondary veins — denser, slightly scattered spacing/slant (not perfect lattice)
    float slant = mix(2.5, 3.8, leafId);
    float veinDensity = mix(9.0, 14.0, leafId2);
    float scatter = (featherNoise(vec2(ly * veinDensity * 0.55 + uMeshSeed, side * 2.0)) - 0.5);
    float scatter2 = (featherNoise(vec2(floor(ly * veinDensity + 0.5), uMeshSeed * 1.7 + side)) - 0.5);
    float veinPhase = ly * veinDensity
                    - away * (slant + scatter * 0.55)
                    + uMeshSeed * 4.0
                    + side * 0.19
                    + scatter2 * 0.35;
    float veinLane = floor(veinPhase);
    float laneU = fract(veinPhase); // 0..1 across a single panel between two veins
    float vein2 = featherVeinRidge(veinPhase, mix(0.065, 0.1, leafId3));
    vein2 *= smoothstep(0.0, 0.035, away) * (1.0 - smoothstep(0.38, 0.72, away));

    // Extra intermittent / forked veins — a little more scattered than a textbook leaf
    float forkPhase = ly * (veinDensity * 1.35) - away * (slant * 0.85) + leafId * 8.0 + side * 0.4;
    float forkGate = step(0.62, featherHash(vec2(floor(forkPhase), uMeshSeed + side)));
    float veinFork = featherVeinRidge(forkPhase + scatter * 0.2, 0.07) * forkGate;
    veinFork *= smoothstep(0.02, 0.08, away) * (1.0 - smoothstep(0.3, 0.65, away));

    // Tertiary + quaternary — denser hierarchy
    float tertPhase = ly * veinDensity * 2.85 - away * slant * 1.2 + leafId2 * 5.0 + scatter * 0.4;
    float vein3 = featherVeinRidge(tertPhase, 0.05);
    vein3 *= smoothstep(0.01, 0.055, away) * (1.0 - smoothstep(0.42, 0.78, away));

    float quatPhase = ly * veinDensity * 5.6 - away * slant * 1.4 + leafId3 * 7.0;
    float vein4 = featherVeinRidge(quatPhase, 0.038);
    vein4 *= smoothstep(0.012, 0.07, away);

    // Fine capillary lines — more lines, softly scattered
    float capPhase = ly * veinDensity * 9.5 - away * slant * 1.55 + uMeshSeed * 6.0 + scatter2 * 0.6;
    float veinCap = featherVeinRidge(capPhase, 0.032);
    veinCap *= smoothstep(0.02, 0.1, away) * 0.85;

    // Barb / fiber layers — denser + mild scatter so they aren’t perfect parallels
    float barbScatter = (featherNoise(vec2(ly * 8.0, away * 12.0 + uMeshSeed)) - 0.5);
    float barbPhase = ly * mix(52.0, 78.0, leafId) - away * mix(16.0, 26.0, leafId2) + uMeshSeed * 8.0 + barbScatter * 0.45;
    float barb = featherVeinRidge(barbPhase, 0.08);
    float barbFinePhase = ly * mix(110.0, 165.0, leafId3) - away * mix(32.0, 48.0, leafId) + 3.0 + barbScatter * 0.7;
    float barbFine = featherVeinRidge(barbFinePhase, 0.065);
    float barbMicroPhase = ly * mix(240.0, 380.0, leafId2) - away * mix(60.0, 95.0, leafId3) + barbScatter;
    float barbMicro = featherVeinRidge(barbMicroPhase, 0.055);
    float barbHairPhase = ly * mix(420.0, 620.0, leafId) - away * mix(100.0, 150.0, leafId2);
    float barbHair = featherVeinRidge(barbHairPhase + barbScatter * 1.2, 0.05);

    // ── PATCH GRADIENTS (real leaflet lamina between veins) ──
    // Each panel: darker near both veins + midrib, lighter in the belly of the patch,
    // then cools / brightens toward tip and outer edge.
    ...
    // Base family for this patch
    vec3 patchRoot = mix(cDeep, cForest, 0.35 + lanePick * 0.4);
    vec3 patchMid = mix(cPanel, cEmerald, lanePick2);
    vec3 patchLite = mix(cPanelLite, cLime, lanePick);
    vec3 patchTip = mix(cLime, mix(cCyan, cTip, 0.55), 0.45 + lanePick3 * 0.4);

    // Along-leaf gradient inside the patch (root → tip)
    float alongG = smoothstep(0.0, 1.0, t);
    vec3 patchCol = mix(patchRoot, patchMid, smoothstep(0.0, 0.45, alongG));
    patchCol = mix(patchCol, patchLite, smoothstep(0.3, 0.7, alongG));
    patchCol = mix(patchCol, patchTip, smoothstep(0.55, 1.0, alongG));

    // Across-panel: dark at vein edges (laneU ~0 and ~1), bright in center (~0.5)
    float belly = 1.0 - abs(laneU - 0.5) * 2.0;
    belly = smoothstep(0.0, 1.0, belly);
    // Near midrib darker; toward outer edge cooler/lighter
    float radialG = smoothstep(0.02, 0.42, away);
    patchCol = mix(mix(cDeep, patchRoot, 0.55), patchCol, 0.35 + radialG * 0.65);
    patchCol = mix(patchCol, mix(patchLite, patchTip, alongG), belly * 0.55);
    patchCol = mix(patchCol, mix(cCyan, cTip, 0.4), radialG * alongG * 0.35);
    // Soft secondary wash unique per lane
    patchCol = mix(patchCol, mix(cForest, cCyan, lanePick2), (1.0 - belly) * 0.2);

    float panelMask = (1.0 - vein2 * 0.9) * (1.0 - midrib * 0.55) * (1.0 - veinFork * 0.5);
    feather = mix(feather, patchCol, panelMask * 0.72);

    // Every other panel a touch deeper — readable leaflet banding
    float altLane = step(0.5, fract(veinLane * 0.5));
    feather = mix(feather, mix(cDeep, cForest, 0.45), altLane * panelMask * 0.14 * (1.0 - alongG));

    // Vein hierarchy paint
    feather = mix(feather, cVein, midrib * 0.58);
    feather = mix(feather, mix(cDeep, cForest, 0.2), midribCore * 0.5);
    feather = mix(feather, mix(cVein, cEmerald, 0.3), vein2 * 0.52);
    feather = mix(feather, mix(cVein, cForest, 0.4), veinFork * 0.4);
    feather = mix(feather, mix(cForest, cLime, 0.3), vein3 * 0.34 * (0.35 + t));
    feather = mix(feather, mix(cEmerald, cCyan, 0.45), vein4 * 0.24 * t);
    feather = mix(feather, mix(cLime, cCyan, 0.4), veinCap * 0.16 * t);

    // Dense scattered barbs — light ridges / dark grooves following the leaf
    float barbMask = (1.0 - midrib * 0.65) * smoothstep(0.01, 0.05, away);
    feather = mix(feather, mix(cLime, cPanelLite, 0.55), barb * 0.24 * t * barbMask);
    feather = mix(feather, mix(cCyan, cTip, 0.4), barbFine * 0.2 * t * barbMask);
    feather = mix(feather, mix(cEmerald, cLime, 0.5), barbMicro * 0.15 * (0.25 + t) * barbMask);
    feather = mix(feather, mix(cLime, cCyan, 0.35), barbHair * 0.1 * tip * barbMask);
    feather = mix(feather, cDeep, (1.0 - barb) * 0.07 * (1.0 - t) * barbMask);

    // Fine grain that follows barb direction (patterned, not speckled)
    float fiberGrain = featherFbm(vec2(barbPhase * 0.4, away * 20.0 + uMeshSeed));
    float fiberGrain2 = featherFbm(vec2(barbFinePhase * 0.28, ly * 7.0 + leafId));
    feather = mix(feather, patchLite, fiberGrain * 0.11 * t * panelMask);
    feather = mix(feather, cCyan, fiberGrain2 * 0.09 * tip * panelMask);
    feather = mix(feather, patchRoot, (1.0 - fiberGrain) * 0.07 * (1.0 - t) * panelMask);

    // Outer leaflet edge: cooler / brighter like a real wing leaf
    float edge = smoothstep(0.1, 0.52, away);
    feather = mix(feather, mix(cCyan, cTip, 0.35), edge * 0.2 * t);
    ...
    float sheen = smoothstep(0.45, 0.95, ndl) * (0.3 + midrib * 0.4 + vein2 * 0.25 + belly * 0.2);
    feather = mix(feather, mix(cLime, cCyan, 0.55), sheen * 0.13 * t);
...
export default function EagleScrollScene({
  ...
  useLayoutEffect(() => {
    ...
    function createScrollAnimation(triggerEl) {
      ...
      // Initial camera for embedded hero (dark7-v22).
      ...
      ScrollTrigger.getById(DARK7_V22_HERO_PIN_ID)?.kill();
      ...
      gsapCtx = gsap.context(() => {
        ...
        scrollTween = gsap.to(state, {
          progress: 1,
          ease: "none",
          scrollTrigger: Dark7V22ScrollTrigger({
            id: embeddedScroll ? DARK7_V22_HERO_PIN_ID : "eagle-scroll-scene",
            trigger: triggerEl,
            start: "top top",
      ...
      requestAnimationFrame(() => {
        refreshDark7V22ScrollTriggers();
        const progress =
          getDark7V22ScrollTop() < 8 ? 0 : (scrollTween?.scrollTrigger?.progress ?? 0);
        applyScrollProgress(progress);
        onScrollProgressRef.current?.(progress <= 0.001 ? 0 : progress);
      });

      window.setTimeout(() => {
        refreshDark7V22ScrollTriggers(true);
        const progress =
          getDARK7_V22ScrollTop() < 8 ? 0 : (scrollTween?.scrollTrigger?.progress ?? 0);
        applyScrollProgress(progress);
        onScrollProgressRef.current?.(progress <= 0.001 ? 0 : progress);
        if (embeddedScroll) {
          notifyHeroPinReady();
