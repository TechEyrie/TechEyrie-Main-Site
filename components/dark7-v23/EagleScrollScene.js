...
import {
  Dark7V23ScrollTrigger,
  getDark7V23ScrollTop,
  refreshDark7V23ScrollTriggers,
  notifyHeroPinReady,
  DARK7_V23_HERO_PIN_ID,
...
function isWingFeatherMesh(mesh) {
  ...
  if (/wing|feather|cover|primar|second|tert|scap|alula/.test(name)) return true;
  ...
  // Catch more top/covert leaf meshes (slightly less strict)
  return maxDim / (minDim + 1e-5) > 1.65;
}
...
function applyFeatherSurfaceShader(
  ...
  material.customProgramCacheKey = () =>
    `feather-leaf-dense-x10-v23-${isWing ? 1 : 0}-${heroWingLook ? 1 : 0}-${axis?.axisIdx ?? "b"}`;
...
float featherVeinRidge(float phase, float width) {
  ...
  if (uHeroWingLook > 0.5) {
    ...
    // ── X10 DENSE LEAF PATTERN — full coverage top + bottom ──
    float lx = vObjectPos.x;
    float ly = vObjectPos.y;

    // Stronger organic warp so ultra-dense lines still feel grown
    float warp = (featherNoise(vec2(ly * 2.2 + uMeshSeed, lx * 3.0)) - 0.5) * 0.038;
    float warp2 = (featherNoise(vec2(ly * 6.0, lx * 4.5 + uMeshSeed * 2.0)) - 0.5) * 0.018;
    float warp3 = (featherNoise(vec2(ly * 14.0 + leafId, lx * 11.0)) - 0.5) * 0.01;
    lx += warp + warp2 + warp3;
    ly += (featherNoise(vec2(lx * 2.4, uMeshSeed)) - 0.5) * 0.028;

    // Midrib + faint parallel companion ribs (whole leaf coverage)
    float midCurve = sin(ly * 2.1 + uMeshSeed * 3.0) * 0.016
                   + sin(ly * 5.2 + leafId * 6.0) * 0.006
                   + sin(ly * 13.0 + leafId2) * 0.0025;
    float midDist = abs(lx - midCurve);
    float midW = 0.042 + leafId * 0.016 + featherNoise(vec2(ly * 4.0, uMeshSeed)) * 0.01;
    float midrib = 1.0 - smoothstep(0.0, midW, midDist);
    float midribCore = 1.0 - smoothstep(0.0, 0.012, midDist);
    float ribL = 1.0 - smoothstep(0.0, 0.028, abs(lx - midCurve + 0.055));
    float ribR = 1.0 - smoothstep(0.0, 0.028, abs(lx - midCurve - 0.055));

    float side = sign(lx + 0.0001);
    float away = abs(lx);

    // ~10× denser secondary veins — thin ridges, all the way across the leaf
    float slant = mix(3.2, 5.0, leafId);
    float veinDensity = mix(95.0, 145.0, leafId2);
    float scatter = (featherNoise(vec2(ly * veinDensity * 0.08 + uMeshSeed, side * 2.0)) - 0.5);
    float scatter2 = (featherNoise(vec2(floor(ly * veinDensity * 0.12 + 0.5), uMeshSeed * 1.7 + side)) - 0.5);
    float veinPhase = ly * veinDensity
                    - away * (slant + scatter * 0.7)
                    + uMeshSeed * 4.0
                    + side * 0.19
                    + scatter2 * 0.45;
    float veinLane = floor(veinPhase);
    float laneU = fract(veinPhase);
    float vein2 = featherVeinRidge(veinPhase, mix(0.028, 0.045, leafId3));
    // Keep veins alive from midrib to tip edge (full leaf)
    vein2 *= smoothstep(0.0, 0.008, away) * (1.0 - smoothstep(0.55, 0.95, away));

    // Second slant family (crossing / forked feel) — also dense
    float veinDensityB = mix(70.0, 110.0, leafId3);
    float veinPhaseB = ly * veinDensityB
                     - away * (slant * 0.72 + scatter * 0.4)
                     + leafId * 11.0
                     + side * 0.33;
    float vein2b = featherVeinRidge(veinPhaseB + scatter2 * 0.25, 0.032);
    vein2b *= smoothstep(0.0, 0.01, away) * (1.0 - smoothstep(0.5, 0.9, away));

    // Intermittent forks — denser than v22
    float forkPhase = ly * (veinDensity * 0.55) - away * (slant * 0.9) + leafId * 8.0 + side * 0.4;
    float forkGate = step(0.48, featherHash(vec2(floor(forkPhase), uMeshSeed + side)));
    float veinFork = featherVeinRidge(forkPhase + scatter * 0.25, 0.034) * forkGate;
    veinFork *= smoothstep(0.005, 0.04, away) * (1.0 - smoothstep(0.45, 0.85, away));

    // Tertiary / quaternary / capillary — stacked hierarchy at high frequency
    float tertPhase = ly * veinDensity * 2.4 - away * slant * 1.25 + leafId2 * 5.0 + scatter * 0.5;
    float vein3 = featherVeinRidge(tertPhase, 0.024);
    vein3 *= smoothstep(0.0, 0.012, away) * (1.0 - smoothstep(0.58, 0.95, away));

    float quatPhase = ly * veinDensity * 4.8 - away * slant * 1.45 + leafId3 * 7.0;
    float vein4 = featherVeinRidge(quatPhase, 0.02);
    vein4 *= smoothstep(0.0, 0.015, away);

    float capPhase = ly * veinDensity * 8.5 - away * slant * 1.65 + uMeshSeed * 6.0 + scatter2 * 0.7;
    float veinCap = featherVeinRidge(capPhase, 0.016);
    veinCap *= smoothstep(0.0, 0.02, away);

    float microCapPhase = ly * veinDensity * 14.0 - away * slant * 1.85 + leafId * 13.0;
    float veinMicro = featherVeinRidge(microCapPhase + scatter * 0.3, 0.014);
    veinMicro *= smoothstep(0.0, 0.02, away);

    // Ultra-dense barb / fiber stacks (scattered slightly)
    float barbScatter = (featherNoise(vec2(ly * 12.0, away * 18.0 + uMeshSeed)) - 0.5);
    float barbPhase = ly * mix(480.0, 720.0, leafId) - away * mix(140.0, 220.0, leafId2) + uMeshSeed * 8.0 + barbScatter * 0.55;
    float barb = featherVeinRidge(barbPhase, 0.04);
    float barbFinePhase = ly * mix(980.0, 1450.0, leafId3) - away * mix(280.0, 420.0, leafId) + 3.0 + barbScatter * 0.8;
    float barbFine = featherVeinRidge(barbFinePhase, 0.032);
    float barbMicroPhase = ly * mix(2100.0, 3200.0, leafId2) - away * mix(520.0, 820.0, leafId3) + barbScatter;
    float barbMicro = featherVeinRidge(barbMicroPhase, 0.026);
    float barbHairPhase = ly * mix(3800.0, 5600.0, leafId) - away * mix(900.0, 1400.0, leafId2);
    float barbHair = featherVeinRidge(barbHairPhase + barbScatter * 1.4, 0.022);
    float barbNanoPhase = ly * mix(7000.0, 9800.0, leafId3) - away * mix(1600.0, 2400.0, leafId);
    float barbNano = featherVeinRidge(barbNanoPhase, 0.018);

    // ── STRONG PATCH GRADIENTS (many tiny leaflets between dense veins) ──
    float lanePick = featherHash(vec2(veinLane, uMeshSeed * 3.1 + side));
    float lanePick2 = featherHash(vec2(veinLane * 1.7, leafId * 9.0));
    float lanePick3 = featherHash(vec2(veinLane * 0.37 + side, leafId3 * 5.0));
    float lanePick4 = featherHash(vec2(veinLane * 2.3, leafId2 * 4.1 + side));

    vec3 patchRoot = mix(cDeep, cForest, 0.25 + lanePick * 0.5);
    vec3 patchMid = mix(cPanel, cEmerald, lanePick2);
    vec3 patchLite = mix(cPanelLite, cLime, 0.3 + lanePick * 0.5);
    vec3 patchTip = mix(cLime, mix(cCyan, cTip, 0.6), 0.4 + lanePick3 * 0.5);
    vec3 patchCool = mix(cCyan, accentTeal, lanePick4);

    // Along-leaf: deep root → luminous tip (harder stops = clearer leaf read)
    float alongG = clamp(t, 0.0, 1.0);
    vec3 patchCol = mix(patchRoot, patchMid, smoothstep(0.0, 0.28, alongG));
    patchCol = mix(patchCol, patchLite, smoothstep(0.22, 0.55, alongG));
    patchCol = mix(patchCol, patchTip, smoothstep(0.48, 0.82, alongG));
    patchCol = mix(patchCol, patchCool, smoothstep(0.7, 1.0, alongG));

    // Across each tiny panel: dark vein walls → bright belly
    float belly = 1.0 - abs(laneU - 0.5) * 2.0;
    belly = pow(smoothstep(0.0, 1.0, belly), 0.85);
    float radialG = smoothstep(0.0, 0.5, away);
    // Midrib side darker, outer edge cooler — classic leaflet
    patchCol = mix(mix(cDeep, patchRoot, 0.4), patchCol, 0.25 + radialG * 0.75);
    patchCol = mix(patchCol, mix(patchLite, patchTip, alongG), belly * 0.7);
    patchCol = mix(patchCol, mix(cCyan, cTip, 0.45), radialG * alongG * 0.42);
    patchCol = mix(patchCol, mix(cForest, cDeep, 0.4), (1.0 - belly) * 0.35);
    // Extra micro-gradient along the panel (root of panel near midrib → outer)
    float panelAlong = clamp(away * 2.2 + alongG * 0.35, 0.0, 1.0);
    patchCol = mix(patchCol, mix(patchMid, patchCool, lanePick2), panelAlong * belly * 0.28);

    float panelMask = (1.0 - vein2 * 0.85) * (1.0 - midrib * 0.5) * (1.0 - veinFork * 0.4) * (1.0 - vein2b * 0.35);
    feather = mix(feather, patchCol, panelMask * 0.82);

    // Secondary dense-lane banding (readable leaflet mosaic)
    float altLane = step(0.5, fract(veinLane * 0.5));
    float altLane2 = step(0.5, fract(veinLane * 0.25));
    feather = mix(feather, mix(cDeep, cForest, 0.5), altLane * panelMask * 0.16 * (1.0 - alongG));
    feather = mix(feather, mix(cEmerald, cLime, 0.4), altLane2 * panelMask * 0.1 * alongG);

    // Vein hierarchy paint (visible at every scale)
    feather = mix(feather, cVein, midrib * 0.62);
    feather = mix(feather, mix(cDeep, cForest, 0.15), midribCore * 0.55);
    feather = mix(feather, mix(cVein, cForest, 0.35), max(ribL, ribR) * 0.28);
    feather = mix(feather, mix(cVein, cEmerald, 0.25), vein2 * 0.55);
    feather = mix(feather, mix(cVein, cLime, 0.3), vein2b * 0.38);
    feather = mix(feather, mix(cVein, cForest, 0.4), veinFork * 0.42);
    feather = mix(feather, mix(cForest, cLime, 0.35), vein3 * 0.36 * (0.3 + t));
    feather = mix(feather, mix(cEmerald, cCyan, 0.5), vein4 * 0.28 * t);
    feather = mix(feather, mix(cLime, cCyan, 0.45), veinCap * 0.2 * t);
    feather = mix(feather, mix(cCyan, cTip, 0.35), veinMicro * 0.14 * tip);

    // Dense scattered barbs — full leaf surface
    float barbMask = (1.0 - midrib * 0.55) * smoothstep(0.0, 0.02, away);
    feather = mix(feather, mix(cLime, cPanelLite, 0.55), barb * 0.26 * t * barbMask);
    feather = mix(feather, mix(cCyan, cTip, 0.4), barbFine * 0.22 * t * barbMask);
    feather = mix(feather, mix(cEmerald, cLime, 0.5), barbMicro * 0.16 * (0.25 + t) * barbMask);
    feather = mix(feather, mix(cLime, cCyan, 0.4), barbHair * 0.12 * tip * barbMask);
    feather = mix(feather, mix(cCyan, cTip, 0.5), barbNano * 0.08 * tip * barbMask);
    feather = mix(feather, cDeep, (1.0 - barb) * 0.06 * (1.0 - t) * barbMask);

    // Fine grain following barb direction
    float fiberGrain = featherFbm(vec2(barbPhase * 0.2, away * 28.0 + uMeshSeed));
    float fiberGrain2 = featherFbm(vec2(barbFinePhase * 0.15, ly * 10.0 + leafId));
    float fiberGrain3 = featherFbm(vec2(barbMicroPhase * 0.08, away * 40.0));
    feather = mix(feather, patchLite, fiberGrain * 0.12 * t * panelMask);
    feather = mix(feather, cCyan, fiberGrain2 * 0.1 * tip * panelMask);
    feather = mix(feather, patchTip, fiberGrain3 * 0.08 * alongG * panelMask);
    feather = mix(feather, patchRoot, (1.0 - fiberGrain) * 0.07 * (1.0 - t) * panelMask);

    // Outer edge cooler / brighter
    float edge = smoothstep(0.08, 0.55, away);
    feather = mix(feather, mix(cCyan, cTip, 0.4), edge * 0.22 * t);
    feather = mix(feather, cDeep, (1.0 - edge) * 0.05 * (1.0 - t));
    ...
    float sheen = smoothstep(0.45, 0.95, ndl) * (0.28 + midrib * 0.35 + vein2 * 0.22 + belly * 0.25);
    feather = mix(feather, mix(cLime, cCyan, 0.55), sheen * 0.14 * t);
...
export default function EagleScrollScene({
  ...
  useLayoutEffect(() => {
    ...
    function createScrollAnimation(triggerEl) {
      ...
      // Initial camera for embedded hero (dark7-v23).
      ...
      ScrollTrigger.getById(DARK7_V23_HERO_PIN_ID)?.kill();
      ...
      gsapCtx = gsap.context(() => {
        ...
        scrollTween = gsap.to(state, {
          progress: 1,
          ease: "none",
          scrollTrigger: Dark7V23ScrollTrigger({
            id: embeddedScroll ? DARK7_V23_HERO_PIN_ID : "eagle-scroll-scene",
            trigger: triggerEl,
            start: "top top",
      ...
      requestAnimationFrame(() => {
        refreshDark7V23ScrollTriggers();
        const progress =
          getDark7V23ScrollTop() < 8 ? 0 : (scrollTween?.scrollTrigger?.progress ?? 0);
        applyScrollProgress(progress);
        onScrollProgressRef.current?.(progress <= 0.001 ? 0 : progress);
      });

      window.setTimeout(() => {
        refreshDark7V23ScrollTriggers(true);
        const progress =
          getDARK7_V23ScrollTop() < 8 ? 0 : (scrollTween?.scrollTrigger?.progress ?? 0);
        applyScrollProgress(progress);
        onScrollProgressRef.current?.(progress <= 0.001 ? 0 : progress);
        if (embeddedScroll) {
          notifyHeroPinReady();
