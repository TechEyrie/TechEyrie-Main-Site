...
/** Lenis + ScrollTrigger scroller for all dark7-v27 sections. */
export const DARK7_V27_SCROLLER =
...
export function getDark7V27ScrollTop() {
  if (lenisInstance) return lenisInstance.scroll;
  if (DARK7_V27_SCROLLER) {
    return ScrollTrigger.getScrollFunc(DARK7_V27_SCROLLER)?.() ?? window.scrollY;
  }
  return window.scrollY;
}

/** True when any pinned ScrollTrigger is currently active. */
export function hasActiveDark7V27Pin() {
  return ScrollTrigger.getAll().some(
    (st) => st.isActive && Boolean(st.vars?.pin || st.pin),
  );
}

/** Merge dark7-v27 scroll defaults into a ScrollTrigger config object. */
export function Dark7V27ScrollTrigger(vars = {}) {
  return {
    scroller: DARK7_V27_SCROLLER,
 ...
 */
function applyDark7V27ScrollerProxy(lenis) {
  if (!DARK7_V27_SCROLLER) return;

  // Lenis always transforms documentElement — "fixed" pins shift downstream triggers.
  const pinType = lenis ? "transform" : DARK7_V27_SCROLLER.style.transform ? "transform" : "fixed";

  ScrollTrigger.scrollerProxy(DARK7_V27_SCROLLER, {
    scrollTop(value) {
      if (arguments.length) {
...
export function initDark7V27LenisScroll(lenis) {
  if (typeof window === "undefined" || !DARK7_V27_SCROLLER) return;

  lenisInstance = lenis;
  layoutSettled = false;

  applyDark7V27ScrollerProxy(lenis);

  const onRefreshInit = () => applyDark7V27ScrollerProxy(lenis);
  ...
  lenis.on("scroll", () => {
    ...
    if (typeof window !== "undefined") {
      window.dispatchEvent(
        new CustomEvent("dark7-v27-scroll", { detail: lenis.scroll }),
      );
    }
  });
...
export function destroyDark7V27LenisScroll(lenis) {
  lenisInstance = null;
  layoutSettled = false;
  removeRefreshInitListener?.();
 ...
 */
export function refreshDark7V27ScrollTriggers(hard = true) {
  if (typeof window === "undefined") return;

  if (hard && layoutSettled && hasActiveDark7V27Pin()) {
    ScrollTrigger.update();
    return;
  }
...
export function markDark7V27ScrollLayoutSettled() {
  layoutSettled = true;
}

export const DARK7_V27_SCROLL_LAYOUT_READY_EVENT = "dark7-v27-scroll-layout-ready";
...
export function notifyScrollLayoutReady() {
  ...
  refreshDark7V27ScrollTriggers(true);
  window.dispatchEvent(new Event(DARK7_V27_SCROLL_LAYOUT_READY_EVENT));

  requestAnimationFrame(() => refreshDark7V27ScrollTriggers(true));
  window.setTimeout(() => refreshDark7V27ScrollTriggers(true), 120);
}
...
export function subscribeAfterScrollLayout(callback) {
  ...
  const run = () => {
    ...
    callback();
    requestAnimationFrame(() => {
      if (!cancelled) refreshDark7V27ScrollTriggers(true);
    });
  };
  ...
  const onReady = () => {
    window.removeEventListener(DARK7_V27_SCROLL_LAYOUT_READY_EVENT, onReady);
    window.clearTimeout(fallbackTimer);
    run();
  };

  window.addEventListener(DARK7_V27_SCROLL_LAYOUT_READY_EVENT, onReady);
...
export const DARK7_V27_HERO_PIN_ID = "dark7-v27-hero-pin";
