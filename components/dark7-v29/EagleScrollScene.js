...
import {
  ...
  getDark7V29ScrollTop,
  refreshDark7V29ScrollTriggers,
  notifyHeroPinReady,
...
function applyFeatherSurfaceShader(
  ...
  material.customProgramCacheKey = () =>
    `feather-leaf-veins-dark-v29-${isWing ? 1 : 0}-${heroWingLook ? 1 : 0}-${axis?.axisIdx ?? "b"}-${axis?.acrossIdx ?? "a"}`;
...
export default function EagleScrollScene({
  ...
  useLayoutEffect(() => {
    ...
    function createScrollAnimation(triggerEl) {
      ...
      // Initial camera for embedded hero (dark7-v29).
      ...
      ScrollTrigger.getById(DARK7_V29_HERO_PIN_ID)?.kill();
      ...
      gsapCtx = gsap.context(() => {
        ...
        scrollTween = gsap.to(state, {
          progress: 1,
          ease: "none",
          scrollTrigger: Dark7V29ScrollTrigger({
            id: embeddedScroll ? DARK7_V29_HERO_PIN_ID : "eagle-scroll-scene",
            trigger: triggerEl,
            start: "top top",
      ...
      requestAnimationFrame(() => {
        refreshDark7V29ScrollTriggers();
        const progress =
          getDark7V29ScrollTop() < 8 ? 0 : (scrollTween?.scrollTrigger?.progress ?? 0);
      ...
      window.setTimeout(() => {
        ...
        const progress =
          getDARK7_V29ScrollTop() < 8 ? 0 : (scrollTween?.scrollTrigger?.progress ?? 0);
        applyScrollProgress(progress);
        onScrollProgressRef.current?.(progress <= 0.001 ? 0 : progress);
        if (embeddedScroll) {
          notifyHeroPinReady();
