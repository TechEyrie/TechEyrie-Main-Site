...
/** Lenis + ScrollTrigger scroller for all dark7-v30 sections. */
export const DARK7_V30_SCROLLER =
...
export function getDark7V30ScrollTop() {
  if (lenisInstance) return lenisInstance.scroll;
  if (DARK7_V30_SCROLLER) {
    return ScrollTrigger.getScrollFunc(DARK7_V30_SCROLLER)?.() ?? window.scrollY;
  }
  return window.scrollY;
}

/** True when any pinned ScrollTrigger is currently active. */
export function hasActiveDark7V30Pin() {
  return ScrollTrigger.getAll().some(
    (st) => st.isActive && Boolean(st.vars?.pin || st.pin),
  );
}

/** Merge dark7-v30 scroll defaults into a ScrollTrigger config object. */
export function Dark7V30ScrollTrigger(vars = {}) {
  return {
    scroller: DARK7_V30_SCROLLER,
 ...
 */
function applyDark7V30ScrollerProxy(lenis) {
  if (!DARK7_V30_SCROLLER) return;

  // Lenis always transforms documentElement — "fixed" pins shift downstream triggers.
  const pinType = lenis ? "transform" : DARK7_V30_SCROLLER.style.transform ? "transform" : "fixed";

  ScrollTrigger.scrollerProxy(DARK7_V30_SCROLLER, {
    scrollTop(value) {
      if (arguments.length) {
...
export function initDark7V30LenisScroll(lenis) {
  if (typeof window === "undefined" || !DARK7_V30_SCROLLER) return;

  lenisInstance = lenis;
  layoutSettled = false;

  applyDark7V30ScrollerProxy(lenis);

  const onRefreshInit = () => applyDark7V30ScrollerProxy(lenis);
  ...
  lenis.on("scroll", () => {
    ...
    if (typeof window !== "undefined") {
      window.dispatchEvent(
        new CustomEvent("dark7-v30-scroll", { detail: lenis.scroll }),
      );
    }
  });
...
export function destroyDark7V30LenisScroll(lenis) {
  lenisInstance = null;
  layoutSettled = false;
  removeRefreshInitListener?.();
 ...
 */
export function refreshDark7V30ScrollTriggers(hard = true) {
  if (typeof window === "undefined") return;

  if (hard && layoutSettled && hasActiveDark7V30Pin()) {
    ScrollTrigger.update();
    return;
  }
...
export function markDark7V30ScrollLayoutSettled() {
  layoutSettled = true;
}

export const DARK7_V30_SCROLL_LAYOUT_READY_EVENT = "dark7-v30-scroll-layout-ready";
...
export function notifyScrollLayoutReady() {
  ...
  refreshDark7V30ScrollTriggers(true);
  window.dispatchEvent(new Event(DARK7_V30_SCROLL_LAYOUT_READY_EVENT));

  requestAnimationFrame(() => refreshDark7V30ScrollTriggers(true));
  window.setTimeout(() => refreshDark7V30ScrollTriggers(true), 120);
}
...
export function subscribeAfterScrollLayout(callback) {
  ...
  const run = () => {
    ...
    callback();
    requestAnimationFrame(() => {
      if (!cancelled) refreshDark7V30ScrollTriggers(true);
    });
  };
  ...
  const onReady = () => {
    window.removeEventListener(DARK7_V30_SCROLL_LAYOUT_READY_EVENT, onReady);
    window.clearTimeout(fallbackTimer);
    run();
  };

  window.addEventListener(DARK7_V30_SCROLL_LAYOUT_READY_EVENT, onReady);
...
export const DARK7_V30_HERO_PIN_ID = "dark7-v30-hero-pin";
