...
import {
  ...
  getDark7V32ScrollTop,
  refreshDark7V32ScrollTriggers,
  notifyHeroPinReady,
...
/**
 * Hero leaf families — Noomo deep teal → cyan tip
...
function applyFeatherSurfaceShader(
  ...
  material.customProgramCacheKey = () =>
    `feather-leaf-noomo-v32-${isWing ? 1 : 0}-${heroWingLook ? 1 : 0}-${axis?.axisIdx ?? "b"}-${axis?.acrossIdx ?? "a"}`;
...
float featherFiber(float across, float along, float density, float wobble) {
  ...
  if (uHeroWingLook > 0.5) {
    ...
    float veinDensity = mix(11.0, 15.0, leafId2);
...
export function applyBirdMaterial(bird, textures, lightweightOrOptions = false) {
  ...
  bird.traverse((child) => {
    ...
    const material = new THREE.MeshPhysicalMaterial({
      ...
      emissiveIntensity: useHeroWingLook ? (isBottom ? 0.14 : 0.12) : isWing ? 0.0 : 0.004,
      ...
      clearcoat: useHeroWingLook ? 0.72 : 0.0,
      ...
      transmission: useHeroWingLook ? 0.08 : 0.0,
...
export default function EagleScrollScene({
  ...
  useLayoutEffect(() => {
    ...
    function createScrollAnimation(triggerEl) {
      ...
      // Initial camera for embedded hero (dark7-v32).
      ...
      ScrollTrigger.getById(DARK7_V32_HERO_PIN_ID)?.kill();
      ...
      gsapCtx = gsap.context(() => {
        ...
        scrollTween = gsap.to(state, {
          progress: 1,
          ease: "none",
          scrollTrigger: Dark7V32ScrollTrigger({
            id: embeddedScroll ? DARK7_V32_HERO_PIN_ID : "eagle-scroll-scene",
            trigger: triggerEl,
            start: "top top",
      ...
      requestAnimationFrame(() => {
        refreshDark7V32ScrollTriggers();
        const progress =
          getDark7V32ScrollTop() < 8 ? 0 : (scrollTween?.scrollTrigger?.progress ?? 0);
      ...
      window.setTimeout(() => {
        ...
        const progress =
          getDARK7_V32ScrollTop() < 8 ? 0 : (scrollTween?.scrollTrigger?.progress ?? 0);
        applyScrollProgress(progress);
        onScrollProgressRef.current?.(progress <= 0.001 ? 0 : progress);
        if (embeddedScroll) {
          notifyHeroPinReady();
