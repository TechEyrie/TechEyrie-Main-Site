...
import {
  ...
  getDark7V33ScrollTop,
  refreshDark7V33ScrollTriggers,
  notifyHeroPinReady,
...
/**
 * Hero leaf families — Noomo emerald glass
...
function applyFeatherSurfaceShader(
  ...
  material.customProgramCacheKey = () =>
    `feather-leaf-noomo-v33-${isWing ? 1 : 0}-${heroWingLook ? 1 : 0}-${axis?.axisIdx ?? "b"}-${axis?.acrossIdx ?? "a"}`;
...
float featherFiber(float across, float along, float density, float wobble) {
  ...
  if (uHeroWingLook > 0.5) {
    ...
    float veinDensity = mix(12.0, 16.0, leafId2);
...
export function applyBirdMaterial(bird, textures, lightweightOrOptions = false) {
  ...
  bird.traverse((child) => {
    ...
    const material = new THREE.MeshPhysicalMaterial({
      ...
      emissiveIntensity: useHeroWingLook ? (isBottom ? 0.16 : 0.14) : isWing ? 0.0 : 0.004,
      ...
      transmission: useHeroWingLook ? 0.12 : 0.0,
...
export default function EagleScrollScene({
  ...
  useLayoutEffect(() => {
    ...
    function createScrollAnimation(triggerEl) {
      ...
      // Initial camera for embedded hero (dark7-v33).
      ...
      ScrollTrigger.getById(DARK7_V33_HERO_PIN_ID)?.kill();
      ...
      gsapCtx = gsap.context(() => {
        ...
        scrollTween = gsap.to(state, {
          progress: 1,
          ease: "none",
          scrollTrigger: Dark7V33ScrollTrigger({
            id: embeddedScroll ? DARK7_V33_HERO_PIN_ID : "eagle-scroll-scene",
            trigger: triggerEl,
            start: "top top",
      ...
      requestAnimationFrame(() => {
        refreshDark7V33ScrollTriggers();
        const progress =
          getDark7V33ScrollTop() < 8 ? 0 : (scrollTween?.scrollTrigger?.progress ?? 0);
      ...
      window.setTimeout(() => {
        ...
        const progress =
          getDARK7_V33ScrollTop() < 8 ? 0 : (scrollTween?.scrollTrigger?.progress ?? 0);
        applyScrollProgress(progress);
        onScrollProgressRef.current?.(progress <= 0.001 ? 0 : progress);
        if (embeddedScroll) {
          notifyHeroPinReady();
