...
/**
 * Hero leaf families — Noomo cool cyan-periwinkle green
...
function applyFeatherSurfaceShader(
  ...
  material.customProgramCacheKey = () =>
    `feather-leaf-noomo-v35-${isWing ? 1 : 0}-${heroWingLook ? 1 : 0}-${axis?.axisIdx ?? "b"}-${axis?.acrossIdx ?? "a"}`;
...
float featherFiber(float across, float along, float density, float wobble) {
  ...
  if (uHeroWingLook > 0.5) {
    ...
    float veinDensity = mix(11.0, 15.5, leafId2);
...
export function applyBirdMaterial(bird, textures, lightweightOrOptions = false) {
  ...
  bird.traverse((child) => {
    ...
    const material = new THREE.MeshPhysicalMaterial({
      ...
      emissiveIntensity: useHeroWingLook ? (isBottom ? 0.17 : 0.15) : isWing ? 0.0 : 0.004,
      ...
      clearcoat: useHeroWingLook ? 0.80 : 0.0,
      ...
      transmission: useHeroWingLook ? 0.11 : 0.0,
...
export default function EagleScrollScene({
  ...
  useLayoutEffect(() => {
    ...
    function createScrollAnimation(triggerEl) {
      ...
      // Initial camera for embedded hero (dark7-v35).
      ...
      window.setTimeout(() => {
        ...
        const progress =
          getDARK7_V35ScrollTop() < 8 ? 0 : (scrollTween?.scrollTrigger?.progress ?? 0);
        applyScrollProgress(progress);
        onScrollProgressRef.current?.(progress <= 0.001 ? 0 : progress);
        if (embeddedScroll) {
          notifyHeroPinReady();
