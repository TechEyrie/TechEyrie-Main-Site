...
/**
 * Hero leaf families — Noomo bottle green → cyan tip
...
function applyFeatherSurfaceShader(
  ...
  material.customProgramCacheKey = () =>
    `feather-leaf-noomo-v37-${isWing ? 1 : 0}-${heroWingLook ? 1 : 0}-${axis?.axisIdx ?? "b"}-${axis?.acrossIdx ?? "a"}`;
...
float featherFiber(float across, float along, float density, float wobble) {
  ...
  if (uHeroWingLook > 0.5) {
    ...
    float veinDensity = mix(10.5, 14.5, leafId2);
...
export function applyBirdMaterial(bird, textures, lightweightOrOptions = false) {
  ...
  bird.traverse((child) => {
    ...
    const material = new THREE.MeshPhysicalMaterial({
      ...
      emissiveIntensity: useHeroWingLook ? (isBottom ? 0.13 : 0.11) : isWing ? 0.0 : 0.004,
      ...
      clearcoat: useHeroWingLook ? 0.75 : 0.0,
      ...
      transmission: useHeroWingLook ? 0.09 : 0.0,
...
export default function EagleScrollScene({
  ...
  useLayoutEffect(() => {
    ...
    function createScrollAnimation(triggerEl) {
      ...
      // Initial camera for embedded hero (dark7-v37).
      ...
      ScrollTrigger.getById(DARK7_V37_HERO_PIN_ID)?.kill();
      ...
      gsapCtx = gsap.context(() => {
        ...
        scrollTween = gsap.to(state, {
          ...
          scrollTrigger: Dark7V37ScrollTrigger({
            id: embeddedScroll ? DARK7_V37_HERO_PIN_ID : "eagle-scroll-scene",
            trigger: triggerEl,
            start: "top top",
      ...
      requestAnimationFrame(() => {
        refreshDark7V37ScrollTriggers();
        const progress =
          getDark7V37ScrollTop() < 8 ? 0 : (scrollTween?.scrollTrigger?.progress ?? 0);
      ...
      window.setTimeout(() => {
        ...
        const progress =
          getDARK7_V37ScrollTop() < 8 ? 0 : (scrollTween?.scrollTrigger?.progress ?? 0);
        applyScrollProgress(progress);
        onScrollProgressRef.current?.(progress <= 0.001 ? 0 : progress);
        if (embeddedScroll) {
          notifyHeroPinReady();
