...
/** Lenis + ScrollTrigger scroller for all dark7-v38 sections. */
export const DARK7_V38_SCROLLER =
...
export function getDark7V38ScrollTop() {
  if (lenisInstance) return lenisInstance.scroll;
  if (DARK7_V38_SCROLLER) {
    return ScrollTrigger.getScrollFunc(DARK7_V38_SCROLLER)?.() ?? window.scrollY;
  }
  return window.scrollY;
}

/** True when any pinned ScrollTrigger is currently active. */
export function hasActiveDark7V38Pin() {
  return ScrollTrigger.getAll().some(
    (st) => st.isActive && Boolean(st.vars?.pin || st.pin),
  );
}

/** Merge dark7-v38 scroll defaults into a ScrollTrigger config object. */
export function Dark7V38ScrollTrigger(vars = {}) {
  return {
    scroller: DARK7_V38_SCROLLER,
 ...
 */
function applyDark7V38ScrollerProxy(lenis) {
  if (!DARK7_V38_SCROLLER) return;

  // Lenis always transforms documentElement — "fixed" pins shift downstream triggers.
  const pinType = lenis ? "transform" : DARK7_V38_SCROLLER.style.transform ? "transform" : "fixed";

  ScrollTrigger.scrollerProxy(DARK7_V38_SCROLLER, {
    scrollTop(value) {
      if (arguments.length) {
...
export function initDark7V38LenisScroll(lenis) {
  if (typeof window === "undefined" || !DARK7_V38_SCROLLER) return;

  lenisInstance = lenis;
  layoutSettled = false;

  applyDark7V38ScrollerProxy(lenis);

  const onRefreshInit = () => applyDark7V38ScrollerProxy(lenis);
  ...
  lenis.on("scroll", () => {
    ...
    if (typeof window !== "undefined") {
      window.dispatchEvent(
        new CustomEvent("dark7-v38-scroll", { detail: lenis.scroll }),
      );
    }
  });
  ...
  requestAnimationFrame(() => {
    applyDark7V38ScrollerProxy(lenis);
    refreshDark7V38ScrollTriggers(true);
  });
}

export function destroyDark7V38LenisScroll(lenis) {
  lenisInstance = null;
  layoutSettled = false;
  removeRefreshInitListener?.();
 ...
 */
export function refreshDark7V38ScrollTriggers(hard = true) {
  if (typeof window === "undefined") return;

  if (hard && layoutSettled && hasActiveDark7V38Pin()) {
    ScrollTrigger.update();
    return;
  }
...
export function markDark7V38ScrollLayoutSettled() {
  layoutSettled = true;
}

export const DARK7_V38_SCROLL_LAYOUT_READY_EVENT = "dark7-v38-scroll-layout-ready";
...
export function notifyScrollLayoutReady() {
  ...
  refreshDark7V38ScrollTriggers(true);
  window.dispatchEvent(new Event(DARK7_V38_SCROLL_LAYOUT_READY_EVENT));

  requestAnimationFrame(() => refreshDark7V38ScrollTriggers(true));
  window.setTimeout(() => refreshDark7V38ScrollTriggers(true), 120);
}
...
export function subscribeAfterScrollLayout(callback) {
  ...
  const run = () => {
    ...
    callback();
    requestAnimationFrame(() => {
      if (!cancelled) refreshDark7V38ScrollTriggers(true);
    });
  };
  ...
  const onReady = () => {
    window.removeEventListener(DARK7_V38_SCROLL_LAYOUT_READY_EVENT, onReady);
    window.clearTimeout(fallbackTimer);
    run();
  };

  window.addEventListener(DARK7_V38_SCROLL_LAYOUT_READY_EVENT, onReady);
...
export const DARK7_V38_HERO_PIN_ID = "dark7-v38-hero-pin";
