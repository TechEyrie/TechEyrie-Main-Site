...
import {
  ...
  getDark7V39ScrollTop,
  refreshDark7V39ScrollTriggers,
  notifyHeroPinReady,
...
/**
 * Hero leaf families — Noomo forest + holographic cool tips
...
function applyFeatherSurfaceShader(
  ...
  material.customProgramCacheKey = () =>
    `feather-leaf-noomo-v39-${isWing ? 1 : 0}-${heroWingLook ? 1 : 0}-${axis?.axisIdx ?? "b"}-${axis?.acrossIdx ?? "a"}`;
...
float featherFiber(float across, float along, float density, float wobble) {
  ...
  if (uHeroWingLook > 0.5) {
    ...
    float veinDensity = mix(12.0, 16.5, leafId2);
...
export function applyBirdMaterial(bird, textures, lightweightOrOptions = false) {
  ...
  bird.traverse((child) => {
    ...
    const material = new THREE.MeshPhysicalMaterial({
      ...
      clearcoat: useHeroWingLook ? 0.82 : 0.0,
...
export default function EagleScrollScene({
  ...
  useLayoutEffect(() => {
    ...
    function createScrollAnimation(triggerEl) {
      ...
      // Initial camera for embedded hero (dark7-v39).
      ...
      ScrollTrigger.getById(DARK7_V39_HERO_PIN_ID)?.kill();
      ...
      gsapCtx = gsap.context(() => {
        ...
        scrollTween = gsap.to(state, {
          ...
          scrollTrigger: Dark7V39ScrollTrigger({
            id: embeddedScroll ? DARK7_V39_HERO_PIN_ID : "eagle-scroll-scene",
            trigger: triggerEl,
            start: "top top",
      ...
      requestAnimationFrame(() => {
        refreshDark7V39ScrollTriggers();
        const progress =
          getDark7V39ScrollTop() < 8 ? 0 : (scrollTween?.scrollTrigger?.progress ?? 0);
      ...
      window.setTimeout(() => {
        ...
        const progress =
          getDARK7_V39ScrollTop() < 8 ? 0 : (scrollTween?.scrollTrigger?.progress ?? 0);
        applyScrollProgress(progress);
        onScrollProgressRef.current?.(progress <= 0.001 ? 0 : progress);
        if (embeddedScroll) {
          notifyHeroPinReady();
