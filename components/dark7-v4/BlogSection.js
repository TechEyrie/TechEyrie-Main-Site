...
const BlogsSection = ({ theme = "light", sharedBackground = false }) => {
  ...
  const triggerElectricalAnimation = useCallback(() => {
    ...
    const originalColor = theme === "dark" ? "#FFFFFF" : "#000000";
    const accentOriginalColor = theme === "dark" ? "#FFFFFF" : "#000000";
  ...
  const sectionBgStyle = dark7GradientSurfaceStyle(DARK7_GRADIENTS.blog, "#efe7da");
  ...
  const bodyColor = theme === "dark" ? "text-white" : "text-black";
  const metaColor = "text-black/70";
  const postTitleColor = "text-black";
  const tagBorder = "border-black/30 text-black";

  return (
    <section 
      ref={sectionRef}
      className="dark7-v4-blogs relative w-full py-12 sm:py-16 md:py-20 lg:py-24 xl:py-32 overflow-hidden transition-colors duration-500"
      style={bgStyle}
    >
      ...
      <div className="relative z-[2] w-full max-w-[1500px] mx-auto px-4 sm:px-6 md:px-8">
        ...
        <div className="w-full mx-auto mb-10 sm:mb-14 md:mb-16 lg:mb-20 xl:mb-24">
          ...
          <div ref={titleRef} className="mb-6 sm:mb-8 md:mb-10 text-center">
            <h2
              className={`blogs-heading font-italiana font-light text-[32px] sm:text-[40px] md:text-[48px] lg:text-[56px] xl:text-[64px] 2xl:text-[72px] 3xl:text-[80px] leading-[1.08] tracking-[0.01em] transition-colors duration-500 ${headingColor}`}
            >
              <span className="insights-by-text block">Not Built for Everyone</span>
              <span className={`new-engen-text block ${accentColor}`}>
          ...
          <div className="max-w-3xl mx-auto text-center">
            <p
              className={`blogs-body font-merriweather text-[12px] sm:text-[13px] md:text-[14px] lg:text-[15px] xl:text-[15px] font-regular leading-relaxed tracking-tight transition-colors duration-500 ${bodyColor}`}
            >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 lg:gap-10 xl:gap-12 mb-10 sm:mb-12 md:mb-14 lg:mb-16">
          {loading ? (
            <div className="col-span-full font-merriweather text-[12px] sm:text-[13px] md:text-[14px] text-center py-8 text-black">
              Loading…
            </div>
          ) : (
            ...
            <Link key={post.id} href={post.slug ? `/blog/${post.slug}` : '/blog'} className="block group">
              <article className="group cursor-pointer">
                ...
                <div className="flex flex-wrap items-center gap-2 sm:gap-3 mb-3 sm:mb-4">
                  ...
                  <span
                    key={tagIndex}
                    className={`inline-flex items-center px-3 py-1.5 rounded-full border text-[11px] sm:text-[12px] font-medium leading-none transition-colors duration-300 ${tagBorder}`}
                  >
                    {tag}
                  </span>
                ))}
                <span
                  className={`text-[11px] sm:text-[12px] font-normal leading-snug ml-auto transition-colors duration-500 ${metaColor}`}
                >
                  {post.date}
                </span>
              </div>
              ...
              <h3
                className={`font-merriweather text-[12px] sm:text-[13px] md:text-[14px] lg:text-[15px] xl:text-[15px] font-light leading-snug transition-colors duration-500 ${postTitleColor}`}
              >
                <span
