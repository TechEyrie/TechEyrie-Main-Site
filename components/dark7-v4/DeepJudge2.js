...
import { dark7V4ScrollTrigger, refreshDark7V4ScrollTriggers, subscribeAfterHeroPin } from "./lenisScrollTrigger";
import {
  DARK7_GRADIENTS,
...
function Phase3CardBody({ card, index }) {
  return (
    <>
      ...
      <h3 className="deep-judge-phase3-title mb-3 font-merriweather text-[20px] font-bold leading-snug text-black sm:text-[22px]">
        {card.title}
      </h3>
      <p className="deep-judge-phase3-body font-merriweather text-[14px] font-normal leading-[1.55] text-black sm:text-[15px]">
        {card.desc}
      </p>
    </>
  );
...
export default function DeepJudge2({ theme }) {
  ...
  // Light cream surface on dark7-v4 (theme prop stays "dark" for page shell)
    ...
    let ctx = gsap.context(() => {
      ...
      mm.add("(min-width: 1024px)", () => {
        ...
        const build = () => {
          ...
          ScrollTrigger.getById("dark7-v4-deep-judge")?.kill();

          const tl = gsap.timeline({
            scrollTrigger: dark7V4ScrollTrigger({
              id: "dark7-v4-deep-judge",
  ...
  return (
    <main
      ref={containerRef}
      style={bgStyle}
      className="dark7-v4-deep-judge relative min-h-screen overflow-x-hidden transition-colors duration-500 isolate"
    >
      <div
            ...
            <div className={`space-y-10 sm:space-y-12 border-t pt-14 sm:pt-16 md:pt-20 ${surfaceIsLight ? "border-black/10" : isDark ? "border-white/10" : "border-black/10"}`}>
              ...
              <div className="-mx-4 flex gap-5 overflow-x-auto px-4 pb-2 snap-x snap-mandatory sm:mx-0 sm:px-0">
                {FEATURE_CARDS.map((card, i) => (
                  <article
                    key={card.title}
                    className="deep-judge-phase3-card min-w-[260px] shrink-0 snap-start rounded-[20px] border border-black bg-[#F2F0EC] p-7 sm:min-w-[272px] sm:p-8"
                  >
                    <Phase3CardBody card={card} index={i} />
                  </article>
                ))}
