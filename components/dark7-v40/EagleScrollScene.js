...
import {
  ...
  getDark7V40ScrollTop,
  refreshDark7V40ScrollTriggers,
  notifyHeroPinReady,
...
/**
 * Hero leaf families — Noomo richest teal glass (closest match)
 * Tuned toward https://storytelling.noomoagency.com/ wing feel
 */
...
function applyFeatherSurfaceShader(
  ...
  material.customProgramCacheKey = () =>
    `feather-leaf-noomo-v40-${isWing ? 1 : 0}-${heroWingLook ? 1 : 0}-${axis?.axisIdx ?? "b"}-${axis?.acrossIdx ?? "a"}`;
...
export function applyBirdMaterial(bird, textures, lightweightOrOptions = false) {
  ...
  bird.traverse((child) => {
    ...
    const material = new THREE.MeshPhysicalMaterial({
      ...
      clearcoat: useHeroWingLook ? 0.92 : 0.0,
      ...
      transmission: useHeroWingLook ? 0.15 : 0.0,
...
export default function EagleScrollScene({
  ...
  useLayoutEffect(() => {
    ...
    function createScrollAnimation(triggerEl) {
      ...
      // Initial camera for embedded hero (dark7-v40).
      ...
      ScrollTrigger.getById(DARK7_V40_HERO_PIN_ID)?.kill();
      ...
      gsapCtx = gsap.context(() => {
        ...
        scrollTween = gsap.to(state, {
          progress: 1,
          ease: "none",
          scrollTrigger: Dark7V40ScrollTrigger({
            id: embeddedScroll ? DARK7_V40_HERO_PIN_ID : "eagle-scroll-scene",
            trigger: triggerEl,
            start: "top top",
      ...
      requestAnimationFrame(() => {
        refreshDark7V40ScrollTriggers();
        const progress =
          getDark7V40ScrollTop() < 8 ? 0 : (scrollTween?.scrollTrigger?.progress ?? 0);
      ...
      window.setTimeout(() => {
        ...
        const progress =
          getDARK7_V40ScrollTop() < 8 ? 0 : (scrollTween?.scrollTrigger?.progress ?? 0);
        applyScrollProgress(progress);
        onScrollProgressRef.current?.(progress <= 0.001 ? 0 : progress);
        if (embeddedScroll) {
          notifyHeroPinReady();
