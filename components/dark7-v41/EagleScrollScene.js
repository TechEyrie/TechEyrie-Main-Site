...
import {
  Dark7V41ScrollTrigger,
  getDark7V41ScrollTop,
  refreshDark7V41ScrollTriggers,
  notifyHeroPinReady,
  DARK7_V41_HERO_PIN_ID,
...
/**
 * Hero leaf families — site dark → medium green (#162d24 → #8EAC85)
 * Crystalline structure kept; no blue/teal.
 */
...
/** Outer / tip leaves — medium greens toward #8EAC85 */
...
function applyFeatherSurfaceShader(
  ...
  material.customProgramCacheKey = () =>
    `feather-leaf-green-crystal-v41e-${isWing ? 1 : 0}-${heroWingLook ? 1 : 0}-${axis?.axisIdx ?? "b"}-${axis?.acrossIdx ?? "a"}`;
...
float crystalHex(vec2 p) {
  ...
  if (uHeroWingLook > 0.5) {
    // Site dark→medium green + crystalline facets + hardcore patches
    ...
    // Pure green accents — no blue
    vec3 cVein = mix(cDeep, vec3(0.04, 0.14, 0.06), 0.4);
    vec3 cPatchDark = mix(cDeep, cMid, 0.28);
    vec3 cMoss = mix(cMid, vec3(0.16, 0.36, 0.14), 0.4);
    vec3 cSap = mix(cLite, vec3(0.42, 0.68, 0.28), 0.3);
    vec3 cLeaf = mix(cBody, vec3(0.22, 0.48, 0.22), 0.25);
    ...
    // HARDCORE PATCH DETAIL between diagonals
    ...
    vec3 patchRoot = mix(cPatchDark, cDeep, 0.35 + lanePick * 0.6);
    vec3 patchMid = mix(mix(cMoss, cLeaf, 0.45), cBody, lanePick2);
    vec3 patchLite = mix(cBody, mix(cLite, cSap, 0.35), 0.3 + lanePick * 0.5);
    vec3 patchTip = mix(cLite, mix(cTip, cSap, 0.35), 0.35 + lanePick3 * 0.5);
    // Extra mid-green family per lane so patches read clearly
    vec3 patchAlt = mix(cMid, cMoss, lanePick4);

    vec3 patchCol = mix(patchRoot, patchMid, smoothstep(0.0, 0.28, t));
    patchCol = mix(patchCol, patchAlt, smoothstep(0.18, 0.48, t) * 0.55);
    patchCol = mix(patchCol, patchLite, smoothstep(0.35, 0.68, t));
    patchCol = mix(patchCol, patchTip, smoothstep(0.58, 1.0, t));

    float belly = 1.0 - abs(laneU - 0.5) * 2.0;
    belly = pow(smoothstep(0.0, 1.0, belly), 0.85);
    float radial = smoothstep(0.02, 0.55, away);
    patchCol = mix(mix(cDeep, patchRoot, 0.35), patchCol, 0.22 + radial * 0.78);
    patchCol = mix(patchCol, mix(patchLite, patchTip, t), belly * 0.78);
    patchCol = mix(patchCol, cPatchDark, (1.0 - belly) * 0.58);
    patchCol = mix(patchCol, mix(cLite, cSap, 0.25), radial * t * 0.42);

    // Multi-scale mottling — real wing blotch detail
    ...
    patchCol = mix(patchCol, mix(cMoss, cLeaf, mott), 0.22 * belly * laneAA);
    patchCol = mix(patchCol, mix(cDeep, cMid, mott2), 0.16 * (1.0 - belly) * laneAA);
    patchCol = mix(patchCol, mix(cSap, cLite, mott3), 0.12 * t * belly * laneAA);

    // Sub-panel gradient bands inside each vein lane
    float subBand = smoothstep(0.15, 0.45, laneU) * (1.0 - smoothstep(0.55, 0.85, laneU));
    patchCol = mix(patchCol, mix(cLeaf, cLite, t), subBand * 0.28 * laneAA);
    patchCol = mix(patchCol, mix(cDeep, cMoss, 0.4), (1.0 - subBand) * 0.18 * (1.0 - t) * laneAA);

    float panelMask = (1.0 - diag * 0.92) * (1.0 - midrib * 0.68) * (1.0 - diagFork * 0.4);
    feather = mix(feather, patchCol, panelMask * 0.94);

    // CRYSTALLINE STRUCTURE — geometry only, same green palette
    vec2 facetUV = vec2(
      along * mix(3.6, 5.8, leafId) + leafId2 * 1.6,
      acrossN * mix(2.8, 4.6, leafId3) + side * 0.55 + uMeshSeed * 0.3
    );
    facetUV.x += away * 0.85 + scatter * 0.45;
    facetUV.y += (featherNoise(vec2(along * 2.2, uMeshSeed)) - 0.5) * 0.55;
    facetUV = mat2(1.08, 0.34, -0.28, 0.92) * facetUV;
    ...
    float facetAA = 1.0 - smoothstep(0.12, 0.38, facetFw);
    float facetMask = panelMask * smoothstep(0.008, 0.06, away) * facetAA;

    // Hard stepped face colors from existing greens
    vec3 crystalCol = mix(cDeep, cMid, step(0.33, facetId));
    crystalCol = mix(crystalCol, mix(cBody, cLeaf, 0.45), step(0.55, facetId));
    crystalCol = mix(crystalCol, mix(cLite, cTip, t), step(0.78, facetId));
    crystalCol *= mix(0.62, 1.32, smoothstep(0.1, 0.92, facetShade));

    // Facets dominate the panel
    feather = mix(feather, crystalCol, facetMask * 0.92 * (0.55 + belly * 0.45));

    // Thick dark gem seams
    float seam = pow(1.0 - facetEdge, 1.35);
    feather = mix(feather, mix(cVein, cDeep, 0.65), seam * facetMask * 0.95);
    // Bright cut edges
    float rimCut = pow(smoothstep(0.28, 0.98, facetEdge), 1.8) * (1.0 - seam);
    feather = mix(feather, mix(cLite, cTip, 0.4), rimCut * facetMask * 0.62 * (0.35 + t));

    float facetLit = pow(facetEdge, 0.95) * mix(0.65, 1.3, facetShade);
    feather = mix(feather, mix(cLite, cSap, 0.3), facetLit * facetMask * 0.55 * (0.4 + t));

    // Diamond lattice — stronger
    vec2 diaUV = vec2(along * mix(6.0, 10.0, leafId2) - away * 2.4, acrossN * mix(4.8, 7.8, leafId) + side);
    diaUV = mat2(0.9, 0.45, -0.4, 1.1) * diaUV;
    float diamond = crystalDiamond(diaUV);
    float diaAA = 1.0 - smoothstep(0.1, 0.32, fwidth(diaUV.x) + fwidth(diaUV.y));
    feather = mix(feather, mix(cDeep, cVein, 0.5), (1.0 - diamond) * facetMask * 0.38 * diaAA);
    feather = mix(feather, mix(cLite, cTip, 0.3), diamond * facetMask * 0.45 * t * diaAA);

    // Hex shard overlay
    vec2 hexUV = facetUV * 1.35 + vec2(leafId * 2.0, side * 0.7);
    float hex = crystalHex(hexUV) * diaAA;
    feather = mix(feather, mix(cVein, cDeep, 0.4), hex * facetMask * 0.42);
    feather = mix(feather, mix(cLite, cSap, 0.25), (1.0 - hex) * facetMask * facetLit * 0.18 * t);

    // Mid + fine crystal grain
    vec3 facet2 = crystalFacet(facetUV * 2.05 + vec2(leafId3 * 3.0, 2.0));
    float micro = pow(facet2.x, 1.15) * (1.0 - smoothstep(0.1, 0.34, facetFw * 1.25));
    feather = mix(feather, mix(cBody, cLite, facet2.y), micro * facetMask * 0.35 * t);
    float microSeam = pow(1.0 - facet2.x, 1.6);
    feather = mix(feather, cDeep, microSeam * facetMask * 0.32 * diaAA);

    vec3 facet3 = crystalFacet(facetUV * 3.4 + vec2(1.9, leafId2 * 4.0));
    float nano = pow(facet3.x, 1.4) * (1.0 - smoothstep(0.12, 0.36, facetFw * 1.6));
    feather = mix(feather, mix(cLite, cTip, facet3.y), nano * facetMask * 0.22 * t);
    feather = mix(feather, cDeep, pow(1.0 - facet3.x, 2.0) * facetMask * 0.16 * diaAA);
    ...
    feather = mix(feather, cDeep, altLane * panelMask * 0.3 * (1.0 - t * 0.35) * laneAA);
    feather = mix(feather, cMoss, altLane2 * panelMask * 0.18 * t * laneAA);
    feather = mix(feather, cLeaf, altLane3 * panelMask * 0.12 * belly * laneAA);
    ...
    feather = mix(feather, cDeep, (1.0 - fiber) * 0.08 * (1.0 - t) * fiberMask);

    float edge = smoothstep(0.18, 0.88, away);
    feather = mix(feather, mix(cLite, cTip, 0.35), edge * 0.2 * t);
    feather = mix(feather, cDeep, (1.0 - edge) * 0.12 * (1.0 - t));
    ...
    feather = mix(feather, mix(cLite, cTip, 0.35), rim * 0.12 * t);
    feather = mix(feather, mix(cLite, cTip, 0.45), rim * rimCut * facetMask * 0.16 * t);

    float sheen = smoothstep(0.48, 0.98, ndl) * (0.22 + midrib * 0.3 + diag * 0.2 + belly * 0.18 + facetLit * 0.55);
    feather = mix(feather, mix(cLite, cTip, 0.4), sheen * 0.16 * t);
    ...
    feather = mix(feather, cDeep, (1.0 - ndl) * 0.1 * (1.0 - t));
...
export function applyBirdMaterial(bird, textures, lightweightOrOptions = false) {
  ...
  bird.traverse((child) => {
    ...
    const material = new THREE.MeshPhysicalMaterial({
      ...
      clearcoatRoughness: useHeroWingLook ? 0.12 : 1.0,
      ...
      specularIntensity: useHeroWingLook ? 0.58 : 0.12,
...
export default function EagleScrollScene({
  ...
  useLayoutEffect(() => {
    ...
    function createScrollAnimation(triggerEl) {
      ...
      // Initial camera for embedded hero (dark7-v41).
      ...
      ScrollTrigger.getById(DARK7_V41_HERO_PIN_ID)?.kill();
      ...
      gsapCtx = gsap.context(() => {
        ...
        scrollTween = gsap.to(state, {
          progress: 1,
          ease: "none",
          scrollTrigger: Dark7V41ScrollTrigger({
            id: embeddedScroll ? DARK7_V41_HERO_PIN_ID : "eagle-scroll-scene",
            trigger: triggerEl,
            start: "top top",
      ...
      requestAnimationFrame(() => {
        refreshDark7V41ScrollTriggers();
        const progress =
          getDark7V41ScrollTop() < 8 ? 0 : (scrollTween?.scrollTrigger?.progress ?? 0);
        applyScrollProgress(progress);
        onScrollProgressRef.current?.(progress <= 0.001 ? 0 : progress);
      });

      window.setTimeout(() => {
        refreshDark7V41ScrollTriggers(true);
        const progress =
          getDARK7_V41ScrollTop() < 8 ? 0 : (scrollTween?.scrollTrigger?.progress ?? 0);
        applyScrollProgress(progress);
        onScrollProgressRef.current?.(progress <= 0.001 ? 0 : progress);
        if (embeddedScroll) {
          notifyHeroPinReady();
