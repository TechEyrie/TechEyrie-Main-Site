...
/** Lenis + ScrollTrigger scroller for all dark7-v41 sections. */
export const DARK7_V41_SCROLLER =
...
export function getDark7V41ScrollTop() {
  if (lenisInstance) return lenisInstance.scroll;
  if (DARK7_V41_SCROLLER) {
    return ScrollTrigger.getScrollFunc(DARK7_V41_SCROLLER)?.() ?? window.scrollY;
  }
  return window.scrollY;
}

/** True when any pinned ScrollTrigger is currently active. */
export function hasActiveDark7V41Pin() {
  return ScrollTrigger.getAll().some(
    (st) => st.isActive && Boolean(st.vars?.pin || st.pin),
  );
}

/** Merge dark7-v41 scroll defaults into a ScrollTrigger config object. */
export function Dark7V41ScrollTrigger(vars = {}) {
  return {
    scroller: DARK7_V41_SCROLLER,
 ...
 */
function applyDark7V41ScrollerProxy(lenis) {
  if (!DARK7_V41_SCROLLER) return;

  // Lenis always transforms documentElement — "fixed" pins shift downstream triggers.
  const pinType = lenis ? "transform" : DARK7_V41_SCROLLER.style.transform ? "transform" : "fixed";

  ScrollTrigger.scrollerProxy(DARK7_V41_SCROLLER, {
    scrollTop(value) {
      if (arguments.length) {
...
export function initDark7V41LenisScroll(lenis) {
  if (typeof window === "undefined" || !DARK7_V41_SCROLLER) return;

  lenisInstance = lenis;
  layoutSettled = false;

  applyDark7V41ScrollerProxy(lenis);

  const onRefreshInit = () => applyDark7V41ScrollerProxy(lenis);
  ...
  lenis.on("scroll", () => {
    ...
    if (typeof window !== "undefined") {
      window.dispatchEvent(
        new CustomEvent("dark7-v41-scroll", { detail: lenis.scroll }),
      );
    }
  });
...
export function destroyDark7V41LenisScroll(lenis) {
  lenisInstance = null;
  layoutSettled = false;
  removeRefreshInitListener?.();
 ...
 */
export function refreshDark7V41ScrollTriggers(hard = true) {
  if (typeof window === "undefined") return;

  if (hard && layoutSettled && hasActiveDark7V41Pin()) {
    ScrollTrigger.update();
    return;
  }
...
export function markDark7V41ScrollLayoutSettled() {
  layoutSettled = true;
}

export const DARK7_V41_SCROLL_LAYOUT_READY_EVENT = "dark7-v41-scroll-layout-ready";
...
export function notifyScrollLayoutReady() {
  ...
  refreshDark7V41ScrollTriggers(true);
  window.dispatchEvent(new Event(DARK7_V41_SCROLL_LAYOUT_READY_EVENT));

  requestAnimationFrame(() => refreshDark7V41ScrollTriggers(true));
  window.setTimeout(() => refreshDark7V41ScrollTriggers(true), 120);
}
...
export function subscribeAfterScrollLayout(callback) {
  ...
  const run = () => {
    ...
    callback();
    requestAnimationFrame(() => {
      if (!cancelled) refreshDark7V41ScrollTriggers(true);
    });
  };
  ...
  const onReady = () => {
    window.removeEventListener(DARK7_V41_SCROLL_LAYOUT_READY_EVENT, onReady);
    window.clearTimeout(fallbackTimer);
    run();
  };

  window.addEventListener(DARK7_V41_SCROLL_LAYOUT_READY_EVENT, onReady);
...
export const DARK7_V41_HERO_PIN_ID = "dark7-v41-hero-pin";
