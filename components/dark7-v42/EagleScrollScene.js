...
import {
  Dark7V42ScrollTrigger,
  getDark7V42ScrollTop,
  refreshDark7V42ScrollTriggers,
  notifyHeroPinReady,
  DARK7_V42_HERO_PIN_ID,
...
function applyFeatherSurfaceShader(
  ...
  material.customProgramCacheKey = () =>
    `feather-leaf-green-crystal-v42c-${isWing ? 1 : 0}-${heroWingLook ? 1 : 0}-${axis?.axisIdx ?? "b"}-${axis?.acrossIdx ?? "a"}`;
...
// Crystalline Voronoi — hard angular shards (edge, cellId, shade)
vec3 crystalFacet(vec2 p) {
  ...
  for (int y = -1; y <= 1; y++) {
    for (int x = -1; x <= 1; x++) {
      ...
      // Snap to corners → gem cuts, not blobs
      o = mix(o, step(0.5, o), 0.72);
      vec2 r = g + o - f;
      float dE = length(r * vec2(1.18, 0.82));
      float dC = max(abs(r.x * 1.25), abs(r.y * 0.85));
      float dM = abs(r.x) * 1.1 + abs(r.y) * 0.95;
      float d = mix(mix(dE, dC, 0.72), dM, 0.28);
      if (d < minD) {
        midD = minD;
        minD = d;
        minI = i + g;
  ...
  // Harder face-to-face contrast
  float shade = mix(0.28, 1.28, step(0.5, cellId) * 0.55 + cellId * 0.45);
  return vec3(edge, cellId, shade);
}
// Diamond lattice ridges — extra shard geometry
float crystalDiamond(vec2 p) {
  vec2 q = abs(fract(p) - 0.5);
  float d = (q.x + q.y * 1.2) * 2.0;
  float fw = fwidth(d);
  float ridge = 1.0 - smoothstep(0.0, max(0.06, fw * 1.8), abs(d - 0.68));
  float face = smoothstep(0.98, 0.28, d);
  return ridge * 0.7 + face * 0.3;
}
// Hex shard cuts
float crystalHex(vec2 p) {
  vec2 q = abs(fract(p) - 0.5);
  float d = max(q.x * 1.15 + q.y * 0.35, q.y * 1.05);
  float fw = fwidth(d);
  return 1.0 - smoothstep(0.0, max(0.05, fw * 2.0), abs(d - 0.42));
}
// Tri shard cuts
float crystalTri(vec2 p) {
  vec2 q = fract(p) - 0.5;
  float d = abs(q.x) * 1.4 + abs(q.y + q.x * 0.35) * 0.95;
  float fw = fwidth(d);
  return 1.0 - smoothstep(0.0, max(0.045, fw * 1.8), abs(d - 0.55));
}`
      )
      .replace(
  ...
  if (uHeroWingLook > 0.5) {
    // Green transitions + max crystalline structure
    ...
    float t = pow(along, mix(0.7, 1.18, leafId));
    t = clamp(t + (leafId2 - 0.5) * 0.05, 0.0, 1.0);

    // Base family from uniforms
    ...
    vec3 cVein = mix(cDeep, vec3(0.04, 0.14, 0.06), 0.4);
    vec3 cPatchDark = mix(cDeep, cMid, 0.28);

    // Distinct green transition stops (all green, different hues)
    vec3 gForest = mix(cDeep, vec3(0.08, 0.22, 0.12), 0.45);
    vec3 gMoss   = mix(cMid,  vec3(0.18, 0.38, 0.14), 0.5);
    vec3 gOlive  = mix(cBody, vec3(0.28, 0.36, 0.12), 0.45);
    vec3 gLeaf   = mix(cBody, vec3(0.2, 0.48, 0.22), 0.4);
    vec3 gSage   = mix(cLite, vec3(0.42, 0.58, 0.38), 0.4);
    vec3 gBottle = mix(cMid,  vec3(0.1, 0.32, 0.22), 0.45);
    vec3 gSap    = mix(cLite, vec3(0.4, 0.66, 0.26), 0.35);
    vec3 gLime   = mix(cTip,  vec3(0.52, 0.72, 0.32), 0.3);

    // Along-wing green transition (root forest → tip sage/lime)
    float gt = smoothstep(0.0, 1.0, t);
    vec3 greenA = mix(gForest, gBottle, smoothstep(0.0, 0.35, gt));
    greenA = mix(greenA, gMoss, smoothstep(0.2, 0.55, gt));
    greenA = mix(greenA, gLeaf, smoothstep(0.4, 0.75, gt));
    greenA = mix(greenA, mix(gSage, gLime, leafId), smoothstep(0.65, 1.0, gt));

    // Across-leaf green transition (midrib deep → edge lighter)
    float gx = smoothstep(0.0, 1.0, away);
    vec3 greenB = mix(gForest, gOlive, gx * 0.55);
    greenB = mix(greenB, mix(gSage, gSap, leafId2), gx * gt * 0.65);

    // Noise-driven soft green family flips
    float gShift = featherFbm(vec2(along * 3.2, acrossN * 2.4 + uMeshSeed));
    float gShift2 = featherFbm(vec2(along * 6.5 + leafId, acrossN * 4.0));
    vec3 greenTrans = mix(greenA, greenB, 0.35 + gShift * 0.3);
    greenTrans = mix(greenTrans, mix(gOlive, gMoss, gShift2), 0.18);
    greenTrans = mix(greenTrans, mix(gBottle, gLeaf, leafId3), 0.12 * (1.0 - gt));

    vec3 feather = mix(cDeep, greenTrans, 0.85);
    feather = mix(feather, cMid, smoothstep(0.12, 0.4, t) * 0.35);
    feather = mix(feather, cBody, smoothstep(0.32, 0.6, t) * 0.3);
    feather = mix(feather, cLite, smoothstep(0.52, 0.8, t) * 0.35);
    feather = mix(feather, cTip, smoothstep(0.7, 1.0, t) * 0.55);
    ...
    float veinDensity = mix(11.5, 15.5, leafId2);
    ...
    // HARDCORE PATCHES with green-family transitions per lane
    ...
    float lanePick4 = mix(0.5, featherHash(vec2(veinLane * 2.3 + leafId2, side * 3.1)), laneAA);

    vec3 patchRoot = mix(gForest, cDeep, 0.3 + lanePick * 0.55);
    vec3 patchMid = mix(mix(gMoss, gOlive, lanePick2), gLeaf, lanePick);
    vec3 patchLite = mix(gLeaf, mix(gSage, gSap, lanePick3), 0.4 + lanePick * 0.4);
    vec3 patchTip = mix(gSage, mix(cTip, gLime, 0.45), 0.35 + lanePick4 * 0.5);
    vec3 patchAlt = mix(gBottle, gOlive, lanePick4);

    vec3 patchCol = mix(patchRoot, patchMid, smoothstep(0.0, 0.28, t));
    patchCol = mix(patchCol, patchAlt, smoothstep(0.15, 0.5, t) * 0.6);
    patchCol = mix(patchCol, patchLite, smoothstep(0.35, 0.68, t));
    patchCol = mix(patchCol, patchTip, smoothstep(0.55, 1.0, t));
    // Soft blend into the overall green transition
    patchCol = mix(patchCol, greenTrans, 0.28);

    float belly = 1.0 - abs(laneU - 0.5) * 2.0;
    belly = pow(smoothstep(0.0, 1.0, belly), 0.85);
    float radial = smoothstep(0.02, 0.55, away);
    patchCol = mix(mix(cDeep, patchRoot, 0.35), patchCol, 0.2 + radial * 0.8);
    patchCol = mix(patchCol, mix(patchLite, patchTip, t), belly * 0.8);
    patchCol = mix(patchCol, mix(gForest, gBottle, 0.4), (1.0 - belly) * 0.55);
    patchCol = mix(patchCol, mix(gSap, gLime, 0.3), radial * t * 0.4);

    float mott = featherFbm(vec2(along * 5.2, acrossN * 3.6 + uMeshSeed));
    float mott2 = featherFbm(vec2(along * 9.0 + leafId, acrossN * 6.5 - uMeshSeed));
    float mott3 = featherFbm(vec2(along * 14.0 + side, acrossN * 10.0 + leafId3));
    patchCol = mix(patchCol, mix(gMoss, gLeaf, mott), 0.24 * belly * laneAA);
    patchCol = mix(patchCol, mix(gForest, gOlive, mott2), 0.18 * (1.0 - belly) * laneAA);
    patchCol = mix(patchCol, mix(gSap, gSage, mott3), 0.14 * t * belly * laneAA);

    float subBand = smoothstep(0.15, 0.45, laneU) * (1.0 - smoothstep(0.55, 0.85, laneU));
    patchCol = mix(patchCol, mix(gLeaf, gSage, t), subBand * 0.32 * laneAA);
    patchCol = mix(patchCol, mix(gForest, gMoss, 0.4), (1.0 - subBand) * 0.2 * (1.0 - t) * laneAA);

    float panelMask = (1.0 - diag * 0.9) * (1.0 - midrib * 0.65) * (1.0 - diagFork * 0.38);
    feather = mix(feather, patchCol, panelMask * 0.95);

    // —— MAX CRYSTALLINE ——
    vec2 facetUV = vec2(
      along * mix(3.2, 5.4, leafId) + leafId2 * 1.5,
      acrossN * mix(2.5, 4.2, leafId3) + side * 0.5 + uMeshSeed * 0.28
    );
    facetUV.x += away * 0.9 + scatter * 0.5;
    facetUV.y += (featherNoise(vec2(along * 2.0, uMeshSeed)) - 0.5) * 0.6;
    facetUV = mat2(1.12, 0.38, -0.32, 0.9) * facetUV;
    ...
    float facetAA = 1.0 - smoothstep(0.12, 0.4, facetFw);
    float facetMask = panelMask * smoothstep(0.006, 0.05, away) * facetAA;

    // Crystal faces pick different greens (transition, not flat)
    vec3 crystalCol = mix(gForest, gBottle, step(0.2, facetId));
    crystalCol = mix(crystalCol, gMoss, step(0.4, facetId));
    crystalCol = mix(crystalCol, mix(gOlive, gLeaf, 0.5), step(0.6, facetId));
    crystalCol = mix(crystalCol, mix(gSage, gLime, t), step(0.8, facetId));
    crystalCol = mix(crystalCol, greenTrans, 0.25);
    crystalCol *= mix(0.55, 1.4, smoothstep(0.08, 0.95, facetShade));

    feather = mix(feather, crystalCol, facetMask * 0.95 * (0.6 + belly * 0.4));

    float seam = pow(1.0 - facetEdge, 1.2);
    feather = mix(feather, mix(cVein, gForest, 0.5), seam * facetMask * 1.0);
    float rimCut = pow(smoothstep(0.22, 0.98, facetEdge), 1.6) * (1.0 - seam);
    feather = mix(feather, mix(gSage, cTip, 0.4), rimCut * facetMask * 0.7 * (0.35 + t));

    float facetLit = pow(facetEdge, 0.85) * mix(0.6, 1.35, facetShade);
    feather = mix(feather, mix(gSap, gLime, 0.35), facetLit * facetMask * 0.62 * (0.4 + t));

    vec2 diaUV = vec2(along * mix(5.5, 9.5, leafId2) - away * 2.5, acrossN * mix(4.4, 7.2, leafId) + side);
    diaUV = mat2(0.88, 0.5, -0.45, 1.12) * diaUV;
    float diamond = crystalDiamond(diaUV);
    float diaAA = 1.0 - smoothstep(0.1, 0.34, fwidth(diaUV.x) + fwidth(diaUV.y));
    feather = mix(feather, mix(gForest, cVein, 0.45), (1.0 - diamond) * facetMask * 0.45 * diaAA);
    feather = mix(feather, mix(gSage, cTip, 0.3), diamond * facetMask * 0.52 * t * diaAA);

    vec2 hexUV = facetUV * 1.4 + vec2(leafId * 2.2, side * 0.8);
    float hex = crystalHex(hexUV) * diaAA;
    feather = mix(feather, mix(cVein, gForest, 0.4), hex * facetMask * 0.5);
    feather = mix(feather, mix(gLeaf, gSap, 0.3), (1.0 - hex) * facetMask * facetLit * 0.22 * t);

    float tri = crystalTri(facetUV * 1.7 + vec2(2.3, leafId3)) * diaAA;
    feather = mix(feather, mix(gOlive, gMoss, 0.4), tri * facetMask * 0.35);
    feather = mix(feather, mix(gSage, gLime, 0.3), (1.0 - tri) * facetMask * 0.15 * t);

    vec3 facet2 = crystalFacet(facetUV * 2.2 + vec2(leafId3 * 3.0, 2.1));
    float micro = pow(facet2.x, 1.05) * (1.0 - smoothstep(0.1, 0.35, facetFw * 1.2));
    feather = mix(feather, mix(gLeaf, gSage, facet2.y), micro * facetMask * 0.4 * t);
    feather = mix(feather, gForest, pow(1.0 - facet2.x, 1.5) * facetMask * 0.35 * diaAA);

    vec3 facet3 = crystalFacet(facetUV * 3.6 + vec2(2.0, leafId2 * 4.2));
    float nano = pow(facet3.x, 1.25) * (1.0 - smoothstep(0.12, 0.38, facetFw * 1.55));
    feather = mix(feather, mix(gSap, cTip, facet3.y), nano * facetMask * 0.28 * t);
    feather = mix(feather, gForest, pow(1.0 - facet3.x, 1.8) * facetMask * 0.2 * diaAA);

    // Extra shard layer rotated
    vec2 facetUV2 = mat2(0.7, 0.7, -0.7, 0.7) * facetUV * 1.55;
    vec3 facet4 = crystalFacet(facetUV2 + vec2(leafId, 3.3));
    float crossCut = pow(1.0 - facet4.x, 1.4) * facetAA;
    feather = mix(feather, mix(gBottle, gOlive, facet4.y), crossCut * facetMask * 0.3);
    feather = mix(feather, mix(gSage, gLime, 0.4), pow(facet4.x, 1.2) * facetMask * 0.25 * t);
    ...
    feather = mix(feather, gForest, altLane * panelMask * 0.28 * (1.0 - t * 0.35) * laneAA);
    feather = mix(feather, gMoss, altLane2 * panelMask * 0.2 * t * laneAA);
    feather = mix(feather, gOlive, altLane3 * panelMask * 0.14 * belly * laneAA);

    feather = mix(feather, cVein, midribHalo * 0.4);
    feather = mix(feather, gForest, midrib * 0.8);
    feather = mix(feather, mix(cDeep, vec3(0.03, 0.1, 0.05), 0.4), midribCore * 0.75);
    feather = mix(feather, mix(gSap, gSage, 0.35), midHighlight * 0.38 * (0.4 + t));

    feather = mix(feather, gForest, diag * 0.72);
    feather = mix(feather, mix(cVein, gMoss, 0.25), diagFork * 0.5);
    feather = mix(feather, mix(cVein, gOlive, 0.3), diag2 * 0.42);
    feather = mix(feather, mix(gMoss, gLeaf, 0.35), diag3 * 0.28 * (0.35 + t));
    feather = mix(feather, mix(gLeaf, gSage, 0.3), diagCap * 0.18 * t);

    float diagEdge = abs(fract(veinPhase) - 0.5);
    float diagLit = (1.0 - smoothstep(0.0, max(0.04, fwidth(veinPhase) * 1.5), diagEdge)) * diag * 0.4;
    feather = mix(feather, mix(gSap, gLime, 0.35), diagLit * 0.2 * t);
    ...
    feather = mix(feather, mix(gLeaf, gSage, 0.4), fiber * 0.14 * t * fiberMask);
    feather = mix(feather, mix(gSap, gLime, 0.3), fiber2 * 0.1 * t * fiberMask);
    feather = mix(feather, gForest, (1.0 - fiber) * 0.08 * (1.0 - t) * fiberMask);

    float edge = smoothstep(0.18, 0.88, away);
    feather = mix(feather, mix(gSage, cTip, 0.35), edge * 0.22 * t);
    feather = mix(feather, gForest, (1.0 - edge) * 0.12 * (1.0 - t));

    float viewN = clamp(dot(n, normalize(cameraPosition - vFeatherWorldPos)), 0.0, 1.0);
    float rim = pow(1.0 - viewN, 2.0);
    feather = mix(feather, gForest, rim * 0.1);
    feather = mix(feather, mix(gSage, cTip, 0.35), rim * 0.14 * t);
    feather = mix(feather, mix(gSap, cTip, 0.4), rim * rimCut * facetMask * 0.22 * t);

    float sheen = smoothstep(0.45, 0.98, ndl) * (0.24 + midrib * 0.28 + diag * 0.18 + belly * 0.16 + facetLit * 0.6);
    feather = mix(feather, mix(gSage, cTip, 0.4), sheen * 0.18 * t);

    feather = mix(feather, cTip, tip * uTipBlendStrength * 0.68);
    feather = mix(feather, mix(cTip, gLime, 0.35), tip * 0.14);
    feather = mix(feather, gSap, ndl * 0.05 * t);
    feather = mix(feather, gForest, (1.0 - ndl) * 0.1 * (1.0 - t));
...
export function applyBirdMaterial(bird, textures, lightweightOrOptions = false) {
  ...
  bird.traverse((child) => {
    ...
    const material = new THREE.MeshPhysicalMaterial({
      ...
      specularIntensity: useHeroWingLook ? 0.65 : 0.12,
      ...
      flatShading: false,
      envMapIntensity: useHeroWingLook ? 0.9 : isWing ? 0.02 : 0.015,
...
export default function EagleScrollScene({
  ...
  useLayoutEffect(() => {
    ...
    function createScrollAnimation(triggerEl) {
      ...
      // Initial camera for embedded hero (dark7-v42).
      ...
      ScrollTrigger.getById(DARK7_V42_HERO_PIN_ID)?.kill();
      ...
      gsapCtx = gsap.context(() => {
        ...
        scrollTween = gsap.to(state, {
          progress: 1,
          ease: "none",
          scrollTrigger: Dark7V42ScrollTrigger({
            id: embeddedScroll ? DARK7_V42_HERO_PIN_ID : "eagle-scroll-scene",
            trigger: triggerEl,
            start: "top top",
      ...
      requestAnimationFrame(() => {
        refreshDark7V42ScrollTriggers();
        const progress =
          getDark7V42ScrollTop() < 8 ? 0 : (scrollTween?.scrollTrigger?.progress ?? 0);
        applyScrollProgress(progress);
        onScrollProgressRef.current?.(progress <= 0.001 ? 0 : progress);
      });

      window.setTimeout(() => {
        refreshDark7V42ScrollTriggers(true);
        const progress =
          getDARK7_V42ScrollTop() < 8 ? 0 : (scrollTween?.scrollTrigger?.progress ?? 0);
        applyScrollProgress(progress);
        onScrollProgressRef.current?.(progress <= 0.001 ? 0 : progress);
        if (embeddedScroll) {
          notifyHeroPinReady();
