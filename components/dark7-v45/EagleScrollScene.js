...
import {
  Dark7V45ScrollTrigger,
  getDark7V45ScrollTop,
  refreshDark7V45ScrollTriggers,
  notifyHeroPinReady,
  DARK7_V45_HERO_PIN_ID,
...
/**
 * Hero leaf families — near-black dark greens + punchy crystal accents
 * Deepest pine/ink greens with emerald / chartreuse cut highlights.
 */
...
/** Outer tips — still dark base, accents punch through */
...
function applyFeatherSurfaceShader(
  ...
  material.customProgramCacheKey = () =>
    `feather-leaf-green-crystal-v45a-${isWing ? 1 : 0}-${heroWingLook ? 1 : 0}-${axis?.axisIdx ?? "b"}-${axis?.acrossIdx ?? "a"}`;
...
// Crystalline Voronoi — darkest ink gems (edge, cellId, shade)
vec3 crystalFacet(vec2 p) {
  ...
  for (int y = -1; y <= 1; y++) {
    for (int x = -1; x <= 1; x++) {
      ...
      o = mix(o, step(0.5, o), 0.97);
      vec2 r = g + o - f;
      float dC = max(abs(r.x * 1.65), abs(r.y * 0.68));
      float dM = abs(r.x) * 1.4 + abs(r.y) * 0.72;
      float d = mix(dC, dM, 0.55);
      if (d < minD) { midD = minD; minD = d; minI = i + g; }
      else if (d < midD) { midD = d; }
    }
  }
  float edge = clamp((midD - minD) * 12.0, 0.0, 1.0);
  float cellId = featherHash(minI);
  float shade = mix(0.08, 1.85, step(0.25, cellId) * 0.3 + step(0.5, cellId) * 0.3 + step(0.75, cellId) * 0.25 + cellId * 0.15);
  return vec3(edge, cellId, shade);
}
float crystalDiamond(vec2 p) {
  vec2 q = abs(fract(p) - 0.5);
  float d = (q.x + q.y * 1.4) * 2.0;
  float fw = fwidth(d);
  float ridge = 1.0 - smoothstep(0.0, max(0.025, fw * 1.1), abs(d - 0.55));
  float face = smoothstep(1.0, 0.1, d);
  return ridge * 0.88 + face * 0.12;
}
float crystalHex(vec2 p) {
  vec2 q = abs(fract(p) - 0.5);
  float d = max(q.x * 1.35 + q.y * 0.48, q.y * 1.18);
  float fw = fwidth(d);
  return 1.0 - smoothstep(0.0, max(0.022, fw * 1.15), abs(d - 0.34));
}
float crystalTri(vec2 p) {
  vec2 q = fract(p) - 0.5;
  float d = abs(q.x) * 1.6 + abs(q.y + q.x * 0.45) * 0.95;
  float fw = fwidth(d);
  return 1.0 - smoothstep(0.0, max(0.022, fw * 1.15), abs(d - 0.48));
}
float crystalSlash(vec2 p) {
  float s1 = abs(fract(p.x * 1.35 + p.y * 0.9) - 0.5);
  float s2 = abs(fract(p.x * 0.75 - p.y * 1.4) - 0.5);
  float s3 = abs(fract(p.x * 1.9 + p.y * 0.35) - 0.5);
  float fw = fwidth(s1) + fwidth(s2);
  float r1 = 1.0 - smoothstep(0.0, max(0.025, fw), s1);
  float r2 = 1.0 - smoothstep(0.0, max(0.025, fw), s2);
  float r3 = 1.0 - smoothstep(0.0, max(0.025, fw), s3);
  return max(r1, max(r2 * 0.9, r3 * 0.7));
}
float crystalChip(vec2 p) {
  ...
  float d = max(abs(r.x) * 1.4, abs(r.y) * 1.1);
  float fw = fwidth(d);
  float edge = 1.0 - smoothstep(0.0, max(0.03, fw * 1.4), abs(d - 0.42));
  float face = smoothstep(0.55, 0.15, d);
  return edge * 0.7 + face * 0.3 * h;
}`
      )
      .replace(
  ...
  if (uHeroWingLook > 0.5) {
    // v45: darkest ink greens + max crystal detail
    ...
    acrossN += (featherNoise(vec2(along * 1.8, uMeshSeed)) - 0.5) * 0.02;
    acrossN = clamp(acrossN, -1.35, 1.35);
    float away = abs(acrossN);
    float side = sign(acrossN + 1e-5);

    float t = pow(along, mix(0.65, 1.1, leafId));
    t = clamp(t + (leafId2 - 0.5) * 0.035, 0.0, 1.0);

    vec3 cDeep = mix(uRootColor, uShadowColor, 0.75 + leafId * 0.22);
    vec3 cMid = mix(uAltColor, uBaseColor, 0.3 + leafId2 * 0.4);
    vec3 cBody = mix(uBaseColor, uVaneColor, 0.28 + leafId3 * 0.45);
    vec3 cLite = mix(uHighlightColor, uCoolColor, 0.28 + leafId * 0.35);
    vec3 cTip = mix(uTipColor, uCoolColor, 0.15);
    vec3 cVein = mix(cDeep, vec3(0.01, 0.04, 0.015), 0.75);

    // Near-black families + colorful crystal accents
    vec3 gInk     = mix(cDeep, vec3(0.01, 0.035, 0.015), 0.85);
    vec3 gPine    = mix(cDeep, vec3(0.02, 0.07, 0.03), 0.8);
    vec3 gForest  = mix(cDeep, vec3(0.035, 0.12, 0.055), 0.65);
    vec3 gBottle  = mix(cMid,  vec3(0.04, 0.18, 0.12), 0.65);
    vec3 gMoss    = mix(cMid,  vec3(0.09, 0.26, 0.07), 0.65);
    vec3 gOlive   = mix(cBody, vec3(0.18, 0.24, 0.05), 0.6);
    vec3 gEmerald = mix(cBody, vec3(0.05, 0.32, 0.18), 0.6);
    vec3 gLeaf    = mix(cBody, vec3(0.1, 0.36, 0.12), 0.55);
    vec3 gJade    = mix(cLite, vec3(0.12, 0.42, 0.28), 0.55);
    vec3 gSage    = mix(cLite, vec3(0.3, 0.48, 0.28), 0.5);
    vec3 gChart   = mix(cTip,  vec3(0.42, 0.62, 0.12), 0.45);
    vec3 gSap     = mix(cLite, vec3(0.28, 0.55, 0.16), 0.5);
    vec3 gLime    = mix(cTip,  vec3(0.42, 0.62, 0.2), 0.4);

    float gt = smoothstep(0.0, 1.0, t);
    vec3 greenA = mix(gInk, gPine, smoothstep(0.0, 0.18, gt));
    greenA = mix(greenA, gForest, smoothstep(0.1, 0.32, gt));
    greenA = mix(greenA, gBottle, smoothstep(0.22, 0.45, gt));
    greenA = mix(greenA, mix(gMoss, gOlive, leafId), smoothstep(0.35, 0.58, gt));
    greenA = mix(greenA, mix(gEmerald, gLeaf, leafId2), smoothstep(0.48, 0.72, gt));
    greenA = mix(greenA, mix(gJade, gSage, leafId3), smoothstep(0.62, 0.88, gt));
    greenA = mix(greenA, mix(gChart, gLime, leafId), smoothstep(0.8, 1.0, gt));
    float gx = smoothstep(0.0, 1.0, away);
    vec3 greenTrans = mix(greenA, mix(gOlive, gEmerald, gx), 0.28);

    vec3 feather = mix(gInk, greenTrans, 0.88);

    float midrib = 1.0 - smoothstep(0.0, 0.05, away);
    float midribCore = 1.0 - smoothstep(0.0, 0.018, away);
    float midribHalo = 1.0 - smoothstep(0.0, 0.1, away);
    float midEdge = abs(away - 0.035);
    float midHighlight = (1.0 - smoothstep(0.0, max(0.012, fwidth(away) * 1.8), midEdge))
                       * smoothstep(0.006, 0.03, away)
                       * (1.0 - smoothstep(0.055, 0.1, away));

    float slant = mix(2.5, 4.0, leafId);
    float veinDensity = mix(9.5, 14.5, leafId2);
    float scatter = (featherNoise(vec2(along * 2.8 + uMeshSeed, side * 2.0)) - 0.5) * 0.28;

    float veinPhase = along * veinDensity - away * (slant + scatter) + uMeshSeed * 3.0 + side * 0.32;
    float veinLane = floor(veinPhase);
    float laneU = fract(veinPhase);
    float diag = featherVeinRidge(veinPhase, mix(0.07, 0.105, leafId3));
    diag *= smoothstep(0.008, 0.05, away) * (1.0 - smoothstep(0.76, 1.2, away));

    float diag2 = featherVeinRidge(along * veinDensity * 1.8 - away * slant * 1.22 + leafId2 * 4.0, 0.05);
    diag2 *= smoothstep(0.015, 0.07, away) * (1.0 - smoothstep(0.8, 1.24, away));

    float diag3 = featherVeinRidge(along * veinDensity * 2.7 - away * slant * 1.32 + leafId3 * 5.5, 0.04);
    diag3 *= smoothstep(0.02, 0.085, away) * (1.0 - smoothstep(0.84, 1.28, away));

    float diag4 = featherVeinRidge(along * veinDensity * 3.5 - away * slant * 1.4 + uMeshSeed * 4.0, 0.035);
    diag4 *= smoothstep(0.03, 0.1, away) * (1.0 - smoothstep(0.88, 1.32, away));
    ...
    vec3 patchCol = mix(gInk, gPine, lanePick);
    patchCol = mix(patchCol, mix(gForest, gBottle, lanePick2), smoothstep(0.08, 0.35, t));
    patchCol = mix(patchCol, mix(gMoss, gOlive, lanePick3), smoothstep(0.25, 0.55, t));
    patchCol = mix(patchCol, mix(gEmerald, gLeaf, lanePick4), smoothstep(0.4, 0.72, t));
    patchCol = mix(patchCol, mix(gJade, mix(gSage, gChart, t), lanePick), smoothstep(0.55, 1.0, t));
    patchCol = mix(patchCol, greenTrans, 0.14);

    float belly = pow(1.0 - abs(laneU - 0.5) * 2.0, 0.7);
    patchCol = mix(mix(gInk, gPine, 0.45), patchCol, 0.12 + belly * 0.88);

    float mott = featherFbm(vec2(along * 6.0, acrossN * 4.5 + uMeshSeed));
    float mott2 = featherFbm(vec2(along * 12.0 + leafId, acrossN * 8.5));
    float mott3 = featherFbm(vec2(along * 20.0 + side, acrossN * 14.0 + leafId3));
    float mott4 = featherFbm(vec2(along * 28.0, acrossN * 18.0 - uMeshSeed));
    patchCol = mix(patchCol, mix(gMoss, gEmerald, mott), 0.3 * belly * laneAA);
    patchCol = mix(patchCol, mix(gInk, gOlive, mott2), 0.26 * (1.0 - belly) * laneAA);
    patchCol = mix(patchCol, mix(gChart, gSap, mott3), 0.18 * t * belly * laneAA);
    patchCol = mix(patchCol, mix(gPine, gForest, mott4), 0.14 * (1.0 - t) * laneAA);

    float subBand = smoothstep(0.1, 0.4, laneU) * (1.0 - smoothstep(0.6, 0.9, laneU));
    patchCol = mix(patchCol, mix(gLeaf, gJade, t), subBand * 0.42 * laneAA);
    patchCol = mix(patchCol, mix(gInk, gMoss, 0.5), (1.0 - subBand) * 0.3 * (1.0 - t) * laneAA);

    float panelMask = (1.0 - diag * 0.75) * (1.0 - midrib * 0.45);
    feather = mix(feather, patchCol, panelMask * 0.78);

    // —— MAX DARK CRYSTAL ——
    vec2 facetUV = vec2(
      along * mix(1.2, 2.0, leafId) + leafId2 * 0.75,
      acrossN * mix(1.05, 1.8, leafId3) + side * 0.28 + uMeshSeed * 0.15
    );
    facetUV.x += away * 0.55 + scatter * 0.3;
    facetUV = mat2(1.15, 0.4, -0.34, 0.9) * facetUV;
    ...
    float facetAA = 1.0 - smoothstep(0.15, 0.48, facetFw);
    float facetMask = smoothstep(0.0, 0.025, away) * mix(0.85, 1.0, facetAA);

    // 8 posterized near-black → accent gem faces
    vec3 face0 = mix(gInk, vec3(0.008, 0.02, 0.01), 0.65);
    vec3 face1 = mix(gPine, gForest, 0.4);
    vec3 face2 = mix(gForest, gBottle, 0.45);
    vec3 face3 = mix(gMoss, gOlive, 0.5);
    vec3 face4 = mix(gEmerald, gLeaf, 0.55);
    vec3 face5 = mix(gJade, gSage, 0.5);
    vec3 face6 = mix(gSap, gChart, 0.45);
    vec3 face7 = mix(gChart, gLime, 0.5 + t * 0.3);
    vec3 crystalCol = face0;
    crystalCol = mix(crystalCol, face1, step(0.125, facetId));
    crystalCol = mix(crystalCol, face2, step(0.25, facetId));
    crystalCol = mix(crystalCol, face3, step(0.375, facetId));
    crystalCol = mix(crystalCol, face4, step(0.5, facetId));
    crystalCol = mix(crystalCol, face5, step(0.625, facetId));
    crystalCol = mix(crystalCol, face6, step(0.75, facetId));
    crystalCol = mix(crystalCol, face7, step(0.875, facetId));
    crystalCol *= mix(0.22, 1.9, smoothstep(0.03, 0.97, facetShade));
    crystalCol = mix(crystalCol, greenTrans, 0.06);

    feather = mix(feather, crystalCol, facetMask * 0.998);

    // Ink-black gem seams
    float seam = pow(1.0 - facetEdge, 0.55);
    feather = mix(feather, mix(vec3(0.005, 0.015, 0.008), gInk, 0.15), seam * facetMask * 1.0);

    float rimCut = pow(smoothstep(0.08, 1.0, facetEdge), 1.05) * (1.0 - seam);
    feather = mix(feather, mix(gSage, cTip, 0.65), rimCut * facetMask * 1.0 * (0.4 + t));

    float facetLit = pow(facetEdge, 0.45) * mix(0.45, 1.75, facetShade);
    feather = mix(feather, mix(gSap, gChart, 0.5), facetLit * facetMask * 0.95 * (0.45 + t));

    // Multi-scale inner gems
    vec3 facet2 = crystalFacet(facetUV * 2.7 + vec2(leafId3 * 2.0, 1.4));
    float innerSeam = pow(1.0 - facet2.x, 0.8) * (1.0 - smoothstep(0.12, 0.42, facetFw));
    feather = mix(feather, mix(gInk, cVein, 0.3), innerSeam * facetMask * 0.95);
    feather = mix(feather, mix(gLeaf, gJade, facet2.y), pow(facet2.x, 0.7) * facetMask * 0.62 * t);

    vec3 facet2b = crystalFacet(facetUV * 3.7 + vec2(1.0, leafId * 2.8));
    feather = mix(feather, mix(gPine, gMoss, facet2b.y), pow(facet2b.x, 0.75) * facetMask * 0.48 * t);
    feather = mix(feather, gInk, pow(1.0 - facet2b.x, 1.0) * facetMask * 0.55 * facetAA);

    vec3 facet2c = crystalFacet(facetUV * 4.6 + vec2(leafId2 * 1.8, 2.5));
    feather = mix(feather, mix(gForest, gEmerald, facet2c.y), pow(facet2c.x, 0.8) * facetMask * 0.38 * t);
    feather = mix(feather, gPine, pow(1.0 - facet2c.x, 1.05) * facetMask * 0.4 * facetAA);

    // Cut lattices
    vec2 diaUV = vec2(along * mix(4.0, 6.8, leafId2) - away * 2.0, acrossN * mix(3.0, 5.0, leafId) + side);
    diaUV = mat2(0.85, 0.52, -0.45, 1.12) * diaUV;
    float diamond = crystalDiamond(diaUV);
    float diaAA = 1.0 - smoothstep(0.11, 0.38, fwidth(diaUV.x) + fwidth(diaUV.y));
    feather = mix(feather, mix(gInk, cVein, 0.4), (1.0 - diamond) * facetMask * 0.72 * diaAA);
    feather = mix(feather, mix(gSage, cTip, 0.5), diamond * facetMask * 0.8 * t * diaAA);

    float hex = crystalHex(facetUV * 1.85 + vec2(leafId * 1.5, side * 0.5)) * diaAA;
    feather = mix(feather, mix(cVein, gInk, 0.25), hex * facetMask * 0.8);
    feather = mix(feather, mix(gEmerald, gSap, 0.45), (1.0 - hex) * facetMask * facetLit * 0.4 * t);

    float tri = crystalTri(facetUV * 2.15 + vec2(1.9, leafId3)) * diaAA;
    feather = mix(feather, mix(gOlive, gMoss, 0.55), tri * facetMask * 0.65);
    feather = mix(feather, mix(gChart, gLime, 0.45), (1.0 - tri) * facetMask * 0.32 * t);

    float slash = crystalSlash(facetUV * 2.6 + vec2(leafId2 * 1.4, side)) * diaAA;
    feather = mix(feather, mix(gInk, cVein, 0.35), slash * facetMask * 0.58);
    feather = mix(feather, mix(gJade, gSap, 0.4), (1.0 - slash) * facetMask * 0.25 * t);

    float chip = crystalChip(facetUV * 3.2 + vec2(leafId3 * 2.0, leafId)) * diaAA;
    feather = mix(feather, mix(gPine, gForest, 0.4), (1.0 - chip) * facetMask * 0.35);
    feather = mix(feather, mix(gLeaf, gChart, 0.4), chip * facetMask * 0.4 * t);

    // Nano + micro sparkle
    vec3 facet3 = crystalFacet(facetUV * 5.2 + vec2(2.5, leafId2 * 4.0));
    float nano = pow(facet3.x, 0.85) * (1.0 - smoothstep(0.14, 0.45, facetFw * 1.3));
    feather = mix(feather, mix(gSap, cTip, facet3.y), nano * facetMask * 0.55 * t);
    feather = mix(feather, gInk, pow(1.0 - facet3.x, 1.15) * facetMask * 0.42 * diaAA);

    vec3 facet3b = crystalFacet(facetUV * 7.0 + vec2(leafId3 * 2.2, 4.2));
    float micro = pow(facet3b.x, 0.95) * (1.0 - smoothstep(0.18, 0.52, facetFw * 1.5));
    feather = mix(feather, mix(gChart, gLime, facet3b.y), micro * facetMask * 0.38 * t);

    vec3 facet3c = crystalFacet(facetUV * 9.0 + vec2(3.3, leafId * 2.0));
    float dust = pow(facet3c.x, 1.05) * (1.0 - smoothstep(0.22, 0.58, facetFw * 1.7));
    feather = mix(feather, mix(gSage, cTip, facet3c.y), dust * facetMask * 0.25 * t);

    // Triple rotated shard layers
    vec2 facetUV2 = mat2(0.68, 0.72, -0.72, 0.68) * facetUV * 1.95;
    vec3 facet4 = crystalFacet(facetUV2 + vec2(leafId, 2.4));
    float crossCut = pow(1.0 - facet4.x, 0.9) * facetAA;
    feather = mix(feather, mix(gBottle, gOlive, facet4.y), crossCut * facetMask * 0.58);
    feather = mix(feather, mix(gJade, gChart, 0.55), pow(facet4.x, 0.85) * facetMask * 0.45 * t);

    vec2 facetUV3 = mat2(0.82, -0.55, 0.55, 0.82) * facetUV * 2.35;
    vec3 facet5 = crystalFacet(facetUV3 + vec2(3.0, leafId2));
    feather = mix(feather, mix(gInk, gEmerald, facet5.y), pow(1.0 - facet5.x, 1.0) * facetMask * 0.45 * facetAA);
    feather = mix(feather, mix(gSage, gLime, 0.45), pow(facet5.x, 0.9) * facetMask * 0.32 * t);

    vec2 facetUV4 = mat2(0.5, 0.86, -0.86, 0.5) * facetUV * 2.7;
    vec3 facet6 = crystalFacet(facetUV4 + vec2(leafId3, 1.7));
    feather = mix(feather, mix(gPine, gMoss, facet6.y), pow(1.0 - facet6.x, 1.05) * facetMask * 0.35 * facetAA);
    feather = mix(feather, mix(gLeaf, gSap, 0.4), pow(facet6.x, 0.95) * facetMask * 0.28 * t);
    ...
    feather = mix(feather, gInk, altLane * panelMask * 0.28 * (1.0 - t) * laneAA);
    feather = mix(feather, gOlive, altLane2 * panelMask * 0.18 * t * laneAA);
    feather = mix(feather, gEmerald, altLane3 * panelMask * 0.12 * belly * laneAA);

    // Deep ink vein grooves
    feather = mix(feather, mix(cVein, gInk, 0.25), midribHalo * 0.45);
    feather = mix(feather, gInk, midrib * 0.85);
    feather = mix(feather, mix(cDeep, vec3(0.008, 0.025, 0.01), 0.7), midribCore * 0.92);
    feather = mix(feather, mix(gSap, gJade, 0.5), midHighlight * 0.65 * (0.45 + t));

    feather = mix(feather, mix(gInk, cVein, 0.3), diag * 0.7);
    feather = mix(feather, mix(cVein, gOlive, 0.35), diag2 * 0.45);
    feather = mix(feather, mix(gMoss, gEmerald, 0.4), diag3 * 0.32 * (0.4 + t));
    feather = mix(feather, mix(gLeaf, gJade, 0.35), diag4 * 0.2 * t);
    float diagEdge = abs(fract(veinPhase) - 0.5);
    float diagLit = (1.0 - smoothstep(0.0, max(0.028, fwidth(veinPhase) * 1.2), diagEdge)) * diag * 0.6;
    feather = mix(feather, mix(gSap, gChart, 0.5), diagLit * 0.4 * t);

    float fiberAlong = along * mix(15.0, 24.0, leafId) - away * mix(6.0, 10.0, leafId2);
    float fiber = featherVeinRidge(fiberAlong + scatter * 0.3, 0.09);
    float fiber2 = featherVeinRidge(fiberAlong * 1.8 + leafId3, 0.075);
    float fiberMask = panelMask * smoothstep(0.012, 0.06, away) * (1.0 - facetMask * 0.4);
    feather = mix(feather, mix(gLeaf, gJade, 0.45), fiber * 0.18 * t * fiberMask);
    feather = mix(feather, mix(gSap, gChart, 0.35), fiber2 * 0.12 * t * fiberMask);

    float edge = smoothstep(0.12, 0.9, away);
    feather = mix(feather, mix(gSage, cTip, 0.5), edge * 0.35 * t);
    feather = mix(feather, gInk, (1.0 - edge) * 0.18 * (1.0 - t));

    float viewN = clamp(dot(n, normalize(cameraPosition - vFeatherWorldPos)), 0.0, 1.0);
    float rim = pow(1.0 - viewN, 1.4);
    feather = mix(feather, gInk, rim * 0.12);
    feather = mix(feather, mix(gSage, cTip, 0.55), rim * 0.32 * t);
    feather = mix(feather, mix(gSap, cTip, 0.6), rim * rimCut * facetMask * 0.58 * t);

    float sheen = smoothstep(0.28, 0.98, ndl) * (0.35 + midrib * 0.15 + diag * 0.1 + facetLit * 0.95);
    feather = mix(feather, mix(gJade, cTip, 0.6), sheen * 0.4 * t);

    feather = mix(feather, cTip, tip * uTipBlendStrength * 0.52);
    feather = mix(feather, mix(cTip, gChart, 0.5), tip * 0.22);
    feather = mix(feather, gSap, ndl * 0.08 * t);
    feather = mix(feather, gInk, (1.0 - ndl) * 0.2 * (1.0 - t));

    float luma = dot(feather, vec3(0.299, 0.587, 0.114));
    feather = mix(vec3(luma), feather, 1.38);
...
export function applyBirdMaterial(bird, textures, lightweightOrOptions = false) {
  ...
  bird.traverse((child) => {
    ...
    const material = new THREE.MeshPhysicalMaterial({
      ...
      // Hard crystal glow
      emissiveIntensity: useHeroWingLook ? (isBottom ? 0.2 : 0.14) : isWing ? 0.0 : 0.004,
...
export default function EagleScrollScene({
  ...
  useLayoutEffect(() => {
    ...
    function createScrollAnimation(triggerEl) {
      ...
      // Initial camera for embedded hero (dark7-v45).
      ...
      ScrollTrigger.getById(DARK7_V45_HERO_PIN_ID)?.kill();
      ...
      gsapCtx = gsap.context(() => {
        ...
        scrollTween = gsap.to(state, {
          progress: 1,
          ease: "none",
          scrollTrigger: Dark7V45ScrollTrigger({
            id: embeddedScroll ? DARK7_V45_HERO_PIN_ID : "eagle-scroll-scene",
            trigger: triggerEl,
            start: "top top",
      ...
      requestAnimationFrame(() => {
        refreshDark7V45ScrollTriggers();
        const progress =
          getDark7V45ScrollTop() < 8 ? 0 : (scrollTween?.scrollTrigger?.progress ?? 0);
        applyScrollProgress(progress);
        onScrollProgressRef.current?.(progress <= 0.001 ? 0 : progress);
      });

      window.setTimeout(() => {
        refreshDark7V45ScrollTriggers(true);
        const progress =
          getDARK7_V45ScrollTop() < 8 ? 0 : (scrollTween?.scrollTrigger?.progress ?? 0);
        applyScrollProgress(progress);
        onScrollProgressRef.current?.(progress <= 0.001 ? 0 : progress);
        if (embeddedScroll) {
          notifyHeroPinReady();
