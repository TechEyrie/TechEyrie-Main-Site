...
/** Lenis + ScrollTrigger scroller for all dark7-v46 sections. */
export const DARK7_V46_SCROLLER =
...
export function getDark7V46ScrollTop() {
  if (lenisInstance) return lenisInstance.scroll;
  if (DARK7_V46_SCROLLER) {
    return ScrollTrigger.getScrollFunc(DARK7_V46_SCROLLER)?.() ?? window.scrollY;
  }
  return window.scrollY;
}

/** True when any pinned ScrollTrigger is currently active. */
export function hasActiveDark7V46Pin() {
  return ScrollTrigger.getAll().some(
    (st) => st.isActive && Boolean(st.vars?.pin || st.pin),
  );
}

/** Merge dark7-v46 scroll defaults into a ScrollTrigger config object. */
export function Dark7V46ScrollTrigger(vars = {}) {
  return {
    scroller: DARK7_V46_SCROLLER,
 ...
 */
function applyDark7V46ScrollerProxy(lenis) {
  if (!DARK7_V46_SCROLLER) return;

  // Lenis always transforms documentElement — "fixed" pins shift downstream triggers.
  const pinType = lenis ? "transform" : DARK7_V46_SCROLLER.style.transform ? "transform" : "fixed";

  ScrollTrigger.scrollerProxy(DARK7_V46_SCROLLER, {
    scrollTop(value) {
      if (arguments.length) {
...
export function initDark7V46LenisScroll(lenis) {
  if (typeof window === "undefined" || !DARK7_V46_SCROLLER) return;

  lenisInstance = lenis;
  layoutSettled = false;

  applyDark7V46ScrollerProxy(lenis);

  const onRefreshInit = () => applyDark7V46ScrollerProxy(lenis);
  ...
  lenis.on("scroll", () => {
    ...
    if (typeof window !== "undefined") {
      window.dispatchEvent(
        new CustomEvent("dark7-v46-scroll", { detail: lenis.scroll }),
      );
    }
  });
...
export function destroyDark7V46LenisScroll(lenis) {
  lenisInstance = null;
  layoutSettled = false;
  removeRefreshInitListener?.();
 ...
 */
export function refreshDark7V46ScrollTriggers(hard = true) {
  if (typeof window === "undefined") return;

  if (hard && layoutSettled && hasActiveDark7V46Pin()) {
    ScrollTrigger.update();
    return;
  }
...
export function markDark7V46ScrollLayoutSettled() {
  layoutSettled = true;
}

export const DARK7_V46_SCROLL_LAYOUT_READY_EVENT = "dark7-v46-scroll-layout-ready";
...
export function notifyScrollLayoutReady() {
  ...
  refreshDark7V46ScrollTriggers(true);
  window.dispatchEvent(new Event(DARK7_V46_SCROLL_LAYOUT_READY_EVENT));

  requestAnimationFrame(() => refreshDark7V46ScrollTriggers(true));
  window.setTimeout(() => refreshDark7V46ScrollTriggers(true), 120);
}
...
export function subscribeAfterScrollLayout(callback) {
  ...
  const run = () => {
    ...
    callback();
    requestAnimationFrame(() => {
      if (!cancelled) refreshDark7V46ScrollTriggers(true);
    });
  };
  ...
  const onReady = () => {
    window.removeEventListener(DARK7_V46_SCROLL_LAYOUT_READY_EVENT, onReady);
    window.clearTimeout(fallbackTimer);
    run();
  };

  window.addEventListener(DARK7_V46_SCROLL_LAYOUT_READY_EVENT, onReady);
...
export const DARK7_V46_HERO_PIN_ID = "dark7-v46-hero-pin";
