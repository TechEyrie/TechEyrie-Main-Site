...
/** Lenis + ScrollTrigger scroller for all dark7-v47 sections. */
export const DARK7_V47_SCROLLER =
...
export function getDark7V47ScrollTop() {
  if (lenisInstance) return lenisInstance.scroll;
  if (DARK7_V47_SCROLLER) {
    return ScrollTrigger.getScrollFunc(DARK7_V47_SCROLLER)?.() ?? window.scrollY;
  }
  return window.scrollY;
}

/** True when any pinned ScrollTrigger is currently active. */
export function hasActiveDark7V47Pin() {
  return ScrollTrigger.getAll().some(
    (st) => st.isActive && Boolean(st.vars?.pin || st.pin),
  );
}

/** Merge dark7-v47 scroll defaults into a ScrollTrigger config object. */
export function Dark7V47ScrollTrigger(vars = {}) {
  return {
    scroller: DARK7_V47_SCROLLER,
 ...
 */
function applyDark7V47ScrollerProxy(lenis) {
  if (!DARK7_V47_SCROLLER) return;

  // Lenis always transforms documentElement — "fixed" pins shift downstream triggers.
  const pinType = lenis ? "transform" : DARK7_V47_SCROLLER.style.transform ? "transform" : "fixed";

  ScrollTrigger.scrollerProxy(DARK7_V47_SCROLLER, {
    scrollTop(value) {
      if (arguments.length) {
...
export function initDark7V47LenisScroll(lenis) {
  if (typeof window === "undefined" || !DARK7_V47_SCROLLER) return;

  lenisInstance = lenis;
  layoutSettled = false;

  applyDark7V47ScrollerProxy(lenis);

  const onRefreshInit = () => applyDark7V47ScrollerProxy(lenis);
  ...
  lenis.on("scroll", () => {
    ...
    if (typeof window !== "undefined") {
      window.dispatchEvent(
        new CustomEvent("dark7-v47-scroll", { detail: lenis.scroll }),
      );
    }
  });
  ...
  requestAnimationFrame(() => {
    applyDark7V47ScrollerProxy(lenis);
    refreshDark7V47ScrollTriggers(true);
  });
}

export function destroyDark7V47LenisScroll(lenis) {
  lenisInstance = null;
  layoutSettled = false;
  removeRefreshInitListener?.();
 ...
 */
export function refreshDark7V47ScrollTriggers(hard = true) {
  if (typeof window === "undefined") return;

  if (hard && layoutSettled && hasActiveDark7V47Pin()) {
    ScrollTrigger.update();
    return;
  }
...
export function markDark7V47ScrollLayoutSettled() {
  layoutSettled = true;
}

export const DARK7_V47_SCROLL_LAYOUT_READY_EVENT = "dark7-v47-scroll-layout-ready";
...
export function notifyScrollLayoutReady() {
  ...
  refreshDark7V47ScrollTriggers(true);
  window.dispatchEvent(new Event(DARK7_V47_SCROLL_LAYOUT_READY_EVENT));

  requestAnimationFrame(() => refreshDark7V47ScrollTriggers(true));
  window.setTimeout(() => refreshDark7V47ScrollTriggers(true), 120);
}
...
export function subscribeAfterScrollLayout(callback) {
  ...
  const run = () => {
    ...
    callback();
    requestAnimationFrame(() => {
      if (!cancelled) refreshDark7V47ScrollTriggers(true);
    });
  };
  ...
  const onReady = () => {
    window.removeEventListener(DARK7_V47_SCROLL_LAYOUT_READY_EVENT, onReady);
    window.clearTimeout(fallbackTimer);
    run();
  };

  window.addEventListener(DARK7_V47_SCROLL_LAYOUT_READY_EVENT, onReady);
...
export const DARK7_V47_HERO_PIN_ID = "dark7-v47-hero-pin";
