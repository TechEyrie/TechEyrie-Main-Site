...
import {
  Dark7V48ScrollTrigger,
  getDark7V48ScrollTop,
  refreshDark7V48ScrollTriggers,
  notifyHeroPinReady,
  DARK7_V48_HERO_PIN_ID,
...
/**
 * v48 — depth + veins on glass mosaic
 * Top darker → bottom lighter, readable midrib/diagonal veins, AO depth
 * Builds on v47 crystalline patches
 */
...
function applyFeatherSurfaceShader(
  ...
  material.customProgramCacheKey = () =>
    `feather-leaf-depth-vein-v48a-${isWing ? 1 : 0}-${heroWingLook ? 1 : 0}-${axis?.axisIdx ?? "b"}-${axis?.acrossIdx ?? "a"}`;
...
float glassDiagVein(float across, float along, float density, float skew) {
  ...
  if (uHeroWingLook > 0.5) {
    // v48 — depth + veins + top-dark / bottom-light (keep crystalline patches)
    ...
    float tipFade = tip * 0.12 + heightLite * 0.04;
    gl_FragColor.a *= mix(1.0, 0.9, tipFade);
  } else {
...
export default function EagleScrollScene({
  ...
  useLayoutEffect(() => {
    ...
    function createScrollAnimation(triggerEl) {
      ...
      // Initial camera for embedded hero (dark7-v48).
      ...
      ScrollTrigger.getById(DARK7_V48_HERO_PIN_ID)?.kill();
      ...
      gsapCtx = gsap.context(() => {
        ...
        scrollTween = gsap.to(state, {
          progress: 1,
          ease: "none",
          scrollTrigger: Dark7V48ScrollTrigger({
            id: embeddedScroll ? DARK7_V48_HERO_PIN_ID : "eagle-scroll-scene",
            trigger: triggerEl,
            start: "top top",
      ...
      requestAnimationFrame(() => {
        refreshDark7V48ScrollTriggers();
        const progress =
          getDark7V48ScrollTop() < 8 ? 0 : (scrollTween?.scrollTrigger?.progress ?? 0);
        applyScrollProgress(progress);
        onScrollProgressRef.current?.(progress <= 0.001 ? 0 : progress);
      });

      window.setTimeout(() => {
        refreshDark7V48ScrollTriggers(true);
        const progress =
          getDARK7_V48ScrollTop() < 8 ? 0 : (scrollTween?.scrollTrigger?.progress ?? 0);
        applyScrollProgress(progress);
        onScrollProgressRef.current?.(progress <= 0.001 ? 0 : progress);
        if (embeddedScroll) {
          notifyHeroPinReady();
