...
import { Dark7V48ScrollTrigger } from "./lenisScrollTrigger";
import "./HeroProblemServicesCombined.css";
...
export default function HeroProblemServicesCombined({ theme = "light" }) {
  ...
  useLayoutEffect(() => {
    ...
    gsap.set(heroOrbRef.current, { opacity: 0.02 });
  ...
  return (
    <main
      ref={wrapperRef}
      className="hero-problem-services-combined relative overflow-x-hidden transition-colors duration-500"
      style={bgStyle}
    >
      {theme === "dark" && (
        <>
          <div
            className="pointer-events-none absolute inset-x-0 top-0 z-[2] h-24 sm:h-28 md:h-32"
            style={{
              background:
                "linear-gradient(to bottom, rgba(0,81,96,0.38) 0%, rgba(0,81,96,0.12) 45%, rgba(0,81,96,0) 100%)",
            }}
          />
          <div
            className="pointer-events-none absolute inset-x-0 bottom-0 z-[2] h-24 sm:h-28 md:h-32"
            style={{
              background:
                "linear-gradient(to top, rgba(22,45,36,0.55) 0%, rgba(22,45,36,0.18) 50%, rgba(22,45,36,0) 100%)",
            }}
          />
        </>
      )}
      ...
      <div className="relative z-10">
        {theme === "dark" && (
          <>
            ...
            <div
              ref={midOrbRef}
              className="pointer-events-none absolute left-1/2 z-[2] rounded-full"
              style={{
                top: "62%",
                width: "440px",
                height: "440px",
                transform: "translate(-50%, -50%)",
                backgroundImage:
                  "radial-gradient(circle at center, rgba(116,245,161,0.62) 0%, rgba(0,81,96,0.26) 35%, rgba(22,45,36,0) 72%)",
                mixBlendMode: "screen",
                opacity: 0.12,
                filter: "blur(18px)",
              }}
        ...
        <div ref={heroWrapRef} className="relative z-30">
          {theme === "dark" && (
            <>
              <div
                ref={heroNoiseRef}
              ...
              <div
                ...
                style={{
                  ...
                  transform: "translate(-50%, -50%)",
                  backgroundImage:
                    "radial-gradient(circle at center, rgba(116,245,161,0.70) 0%, rgba(0,81,96,0.30) 35%, rgba(22,45,36,0) 72%)",
                  mixBlendMode: "screen",
                  opacity: 0,
                  filter: "blur(18px)",
                }}
