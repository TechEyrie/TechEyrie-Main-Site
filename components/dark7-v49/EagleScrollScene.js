...
/**
 * v49 — Noomo push: big patches, hairline veins, rim bloom, clearer top→bottom
 * Builds on v48 depth/veins + v47 glass mosaic
 */
...
function applyFeatherSurfaceShader(
  ...
  material.customProgramCacheKey = () =>
    `feather-leaf-noomo-push-v49a-${isWing ? 1 : 0}-${heroWingLook ? 1 : 0}-${axis?.axisIdx ?? "b"}-${axis?.acrossIdx ?? "a"}`;
...
// ——— v49: Noomo glass sheets — patches + hairline veins + rim bloom ———
vec4 glassCell(vec2 p) {
  ...
  float edge = clamp((midD - minD) * 7.5, 0.0, 1.0);
  float id = featherHash(minI);
  float shade = mix(0.28, 1.45, id);
  return vec4(edge, id, shade, minD);
}
// Bigger soft blotch cells (Noomo color islands)
vec4 glassBlotch(vec2 p) {
  vec2 i = floor(p);
  vec2 f = fract(p);
  float minD = 8.0;
  float midD = 8.0;
  ...
  for (int y = -1; y <= 1; y++) {
    for (int x = -1; x <= 1; x++) {
      vec2 g = vec2(float(x), float(y));
      vec2 o = vec2(featherHash(i + g + 19.1), featherHash(i + g + vec2(7.3, 41.2)));
      o = 0.15 + 0.7 * o;
      vec2 r = (g + o - f) * vec2(0.85, 1.15);
      float d = length(r);
      if (d < minD) { midD = minD; minD = d; minI = i + g; }
      else if (d < midD) { midD = d; }
    }
  }
  float edge = clamp((midD - minD) * 3.2, 0.0, 1.0);
  return vec4(edge, featherHash(minI), mix(0.5, 1.3, featherHash(minI + 2.0)), minD);
}
float glassFacet(vec2 p) {
  vec2 q = p * vec2(1.05, 1.65);
  vec2 iq = floor(q);
  vec2 fq = fract(q) - 0.5;
  ...
  float d = max(abs(r.x) * 1.25 + abs(r.y) * 0.5, abs(r.y) * 1.35);
  float face = 1.0 - smoothstep(0.12, 0.42, d);
  float ridge = 1.0 - smoothstep(0.0, 0.055, abs(d - 0.28));
  float cut = 1.0 - smoothstep(0.0, 0.04, abs(abs(r.x) - abs(r.y) * 0.7));
  float aa = 1.0 - smoothstep(0.07, 0.26, fwidth(q.x) + fwidth(q.y));
  return (face * mix(0.6, 1.35, id) + ridge * 1.7 + cut * 0.55) * aa;
}
float glassBarb(float across, float along, float density) {
  float w = featherFbm(vec2(along * 3.2, across * 1.6)) * 0.65;
  float phase = across * density + along * 1.35 + w;
  float s = 0.5 + 0.5 * sin(phase * 6.2831853);
  float aa = max(0.08, fwidth(phase) * 1.8);
  return smoothstep(aa, 1.0, pow(s, 2.2));
}
float glassSparkle(vec2 p) {
  float n = featherFbm(p);
  float n2 = featherNoise(p * 3.8 + 11.0);
  float n3 = featherNoise(p * 8.2 + 3.7);
  return pow(smoothstep(0.5, 0.9, n) * smoothstep(0.45, 0.85, n2) * smoothstep(0.4, 0.82, n3), 1.05);
}
float glassDiagVein(float across, float along, float density, float skew) {
  float wob = featherFbm(vec2(along * 1.8, across * 0.85)) * 0.42;
  float phase = across * density + along * skew + wob;
  return featherVeinRidge(phase, 0.032);
}
// Ultra-fine hairline veins (Noomo filament look)
float glassHairline(float across, float along, float density, float skew) {
  float wob = featherFbm(vec2(along * 2.4, across * 1.1)) * 0.55;
  float phase = across * density + along * skew + wob;
  return featherVeinRidge(phase, 0.018);
}`
      )
      .replace(
  ...
  vec3 lightDir = normalize(vec3(0.18, 1.1, 0.28));
  ...
  if (uHeroWingLook > 0.5) {
    // v49 — push to Noomo: big patches, hairline veins, rim bloom, clear top→bottom
    ...
    acrossN += (featherNoise(vec2(along * 2.0, uMeshSeed)) - 0.5) * 0.028;
    acrossN = clamp(acrossN, -1.4, 1.4);
    float away = abs(acrossN);
    float side = sign(acrossN + 1e-5);

    float t = pow(along, mix(0.8, 1.25, leafId));
    t = clamp(t + (leafId2 - 0.5) * 0.03, 0.0, 1.0);

    // Stronger vertical split: TOP ink forest → BOTTOM mint wash
    float relY = clamp((vFeatherWorldPos.y - uBirdOrigin.y + 0.75) / 2.35, 0.0, 1.0);
    float heightDark = pow(1.0 - relY, 0.78);
    float heightLite = pow(relY, 0.82);

    vec3 cDeep = mix(uRootColor, uShadowColor, 0.75 + leafId * 0.2);
    vec3 cMid = mix(uAltColor, uBaseColor, 0.2 + leafId2 * 0.4);
    vec3 cBody = mix(uBaseColor, uVaneColor, 0.2 + leafId3 * 0.4);
    vec3 cLite = mix(uHighlightColor, uCoolColor, 0.2 + leafId * 0.3);
    vec3 cTip = mix(uTipColor, uCoolColor, 0.12);

    vec3 gInk    = mix(cDeep, vec3(0.015, 0.055, 0.028), 0.6);
    vec3 gDeep   = mix(cDeep, vec3(0.035, 0.12, 0.065), 0.55);
    vec3 gForest = mix(cMid,  vec3(0.08, 0.26, 0.14), 0.5);
    vec3 gEmerald= mix(cBody, vec3(0.14, 0.44, 0.26), 0.45);
    vec3 gSage   = mix(cLite, vec3(0.36, 0.62, 0.44), 0.45);
    vec3 gMint   = mix(cTip,  vec3(0.58, 0.9, 0.7), 0.45);
    vec3 gGlow   = mix(cTip,  vec3(0.82, 0.99, 0.9), 0.55);
    vec3 gRim    = mix(gGlow, vec3(0.95, 1.0, 0.96), 0.55);

    // Height-led wash (clear top dark / bottom light)
    float wash = clamp(heightLite * 0.78 + tip * 0.42 + t * 0.2 - heightDark * 0.42, 0.0, 1.0);
    wash = pow(wash, 0.88);

    vec3 feather = mix(gInk, gDeep, smoothstep(0.0, 0.2, wash));
    feather = mix(feather, gForest, smoothstep(0.1, 0.4, wash));
    feather = mix(feather, gEmerald, smoothstep(0.32, 0.62, wash));
    feather = mix(feather, gSage, smoothstep(0.5, 0.8, wash));
    feather = mix(feather, mix(gMint, gGlow, tip), smoothstep(0.68, 1.0, wash));
    feather = mix(feather, gInk, heightDark * 0.58 * (1.0 - tip * 0.5));
    feather = mix(feather, mix(gMint, gGlow, 0.4), heightLite * 0.38 * (0.4 + tip));

    // Layer depth AO + translucent sheet lift
    float depthAO = featherFbm(vec2(along * 2.2 + uMeshSeed, acrossN * 1.6));
    float depthAO2 = featherFbm(vec2(along * 5.8, acrossN * 4.5 + leafId));
    float occlude = pow(1.0 - ndl, 1.35) * 0.5 + depthAO * 0.4 + depthAO2 * 0.28;
    occlude *= mix(1.15, 0.3, wash);
    feather = mix(feather, gInk, occlude * 0.5 * (0.5 + heightDark));
    float lift = smoothstep(0.35, 0.95, depthAO2);
    feather = mix(feather, mix(gEmerald, gMint, heightLite), lift * 0.28 * (0.35 + wash));
    // Extra contact shadow near root of leaf stack
    feather = mix(feather, gInk, (1.0 - along) * 0.18 * heightDark * (1.0 - tip));

    // ——— BIG soft color patches (primary Noomo language) ———
    vec2 blotchUV = vec2(
      along * mix(1.4, 2.4, leafId) + leafId2,
      acrossN * mix(1.1, 1.9, leafId3) + side * 0.35 + uMeshSeed * 0.2
    );
    blotchUV = mat2(1.1, 0.25, -0.2, 1.0) * blotchUV;
    vec4 blotch = glassBlotch(blotchUV);
    vec4 blotch2 = glassBlotch(blotchUV * 1.7 + vec2(2.1, leafId));
    float blotchAA = 1.0 - smoothstep(0.08, 0.3, fwidth(blotchUV.x) + fwidth(blotchUV.y));

    vec3 bA = mix(gInk, gDeep, 0.4);
    vec3 bB = mix(gDeep, gForest, 0.55);
    vec3 bC = mix(gForest, gEmerald, 0.65);
    vec3 bD = mix(gEmerald, mix(gSage, gMint, heightLite + tip * 0.5), 0.55);
    vec3 blotchCol = mix(bA, bB, smoothstep(0.0, 0.4, blotch.y));
    blotchCol = mix(blotchCol, bC, smoothstep(0.3, 0.7, blotch.y));
    blotchCol = mix(blotchCol, bD, smoothstep(0.55, 1.0, blotch.y) * (0.35 + wash));
    blotchCol *= mix(0.55, 1.35, blotch.z);
    blotchCol = mix(blotchCol, gInk, heightDark * 0.45 * (1.0 - blotch.y));
    blotchCol = mix(blotchCol, gMint, heightLite * 0.25 * blotch.y);

    float blotchMask = blotchAA * smoothstep(0.0, 0.05, away + 0.03);
    feather = mix(feather, blotchCol, blotchMask * 0.88);
    // Soft patch boundaries (not hard cracks)
    feather = mix(feather, gInk, pow(1.0 - blotch.x, 1.6) * blotchMask * 0.28 * heightDark);
    feather = mix(feather, mix(gSage, gMint, wash), pow(blotch.x, 2.0) * blotchMask * 0.22 * (0.3 + heightLite));
    // Secondary patch layer
    vec3 blotchCol2 = mix(mix(gForest, gEmerald, blotch2.y), mix(gSage, gMint, wash), blotch2.y);
    feather = mix(feather, blotchCol2, blotch2.x * blotchMask * 0.35 * (0.4 + wash));

    // ——— Veins: midrib + diagonals + dense hairlines ———
    float midribWob = (featherNoise(vec2(along * 4.0, uMeshSeed)) - 0.5) * 0.035;
    float midDist = abs(acrossN - midribWob);
    float midribCore = 1.0 - smoothstep(0.0, 0.028, midDist);
    float midrib = 1.0 - smoothstep(0.0, 0.07, midDist);
    float midribRim = midrib * (1.0 - midribCore);
    feather = mix(feather, gInk, midribCore * 0.72 * (0.6 + heightDark * 0.4));
    feather = mix(feather, mix(gEmerald, gSage, wash), midribRim * 0.65);
    feather = mix(feather, gRim, midribRim * tip * 0.45);

    float dens1 = mix(6.5, 10.0, leafId);
    float dens2 = mix(11.0, 16.0, leafId2);
    float dens3 = mix(18.0, 28.0, leafId3);
    float v1 = glassDiagVein(acrossN, along, dens1, mix(1.6, 2.6, leafId2));
    float v2 = glassDiagVein(acrossN, along + 0.14, dens2, mix(2.2, 3.4, leafId3));
    float v3 = glassDiagVein(acrossN * 1.2, along * 0.95, dens3, mix(2.8, 4.0, leafId));
    float veinBody = (1.0 - tip * 0.3) * smoothstep(0.015, 0.18, away);
    feather = mix(feather, gInk, (v1 * 0.9 + v2 * 0.6) * veinBody * 0.55 * (0.5 + heightDark * 0.5));
    feather = mix(feather, mix(gForest, gEmerald, wash), (v1 * 0.4 + v2 * 0.55) * veinBody * 0.42);
    feather = mix(feather, mix(gSage, gMint, 0.45), v3 * veinBody * 0.35 * (0.35 + wash));

    // Dense hairline filaments (the Noomo "detail")
    float h1 = glassHairline(acrossN, along, mix(28.0, 42.0, leafId), mix(2.5, 3.8, leafId2));
    float h2 = glassHairline(acrossN * 1.15, along * 1.08 + 0.2, mix(40.0, 58.0, leafId3), mix(3.2, 4.8, leafId));
    float h3 = glassHairline(acrossN * 0.9, along * 0.85, mix(22.0, 34.0, leafId2), mix(1.8, 2.8, leafId3));
    float hairZone = smoothstep(0.04, 0.85, away) * (1.0 - tip * 0.2);
    feather = mix(feather, gInk, h1 * hairZone * 0.42 * (0.55 + heightDark));
    feather = mix(feather, mix(gDeep, gForest, wash), h2 * hairZone * 0.32);
    feather = mix(feather, mix(gEmerald, gSage, wash), h3 * hairZone * 0.28 * (0.4 + wash));
    // Luminous hairline highlights
    feather = mix(feather, mix(gMint, gRim, tip), (h1 * 0.35 + h2 * 0.5) * hairZone * 0.22 * (0.3 + wash + tip));

    float fiber = featherFiber(acrossN, along, mix(16.0, 26.0, leafId), 0.6);
    float fiberZone = smoothstep(0.06, 0.8, away) * (1.0 - tip * 0.2);
    feather = mix(feather, mix(gDeep, gEmerald, wash), fiber * fiberZone * 0.3);
    feather = mix(feather, gInk, (1.0 - fiber) * fiberZone * 0.14 * heightDark);

    // ——— Crystalline facets inside patches ———
    vec2 mosaicUV = vec2(
      along * mix(4.2, 6.8, leafId) + leafId2 * 1.4,
      acrossN * mix(3.4, 5.4, leafId3) + side * 0.55 + uMeshSeed * 0.35
    );
    mosaicUV.x += away * 0.45;
    mosaicUV = mat2(1.1, 0.24, -0.18, 1.04) * mosaicUV;

    vec4 cell = glassCell(mosaicUV);
    float cellFw = fwidth(mosaicUV.x) + fwidth(mosaicUV.y);
    float cellAA = 1.0 - smoothstep(0.09, 0.35, cellFw);
    float veinProtect = 1.0 - clamp(midrib * 0.9 + v1 * 0.75 + v2 * 0.5 + h1 * 0.35, 0.0, 1.0);
    float mosaicMask = cellAA * smoothstep(0.0, 0.05, away + 0.03) * veinProtect;

    vec3 patchA = mix(gInk, gDeep, 0.3);
    vec3 patchB = mix(gDeep, gForest, 0.5);
    vec3 patchC = mix(gForest, gEmerald, 0.6);
    vec3 patchD = mix(gEmerald, mix(gSage, gMint, tip + heightLite), 0.55 + wash * 0.3);
    vec3 patchCol = mix(patchA, patchB, smoothstep(0.0, 0.35, cell.y));
    patchCol = mix(patchCol, patchC, smoothstep(0.25, 0.65, cell.y));
    patchCol = mix(patchCol, patchD, smoothstep(0.55, 1.0, cell.y) * (0.35 + wash));
    patchCol *= mix(0.5, 1.4, cell.z);
    patchCol = mix(patchCol, gInk, heightDark * 0.38 * (1.0 - cell.y * 0.4));
    feather = mix(feather, patchCol, mosaicMask * 0.72);

    float seam = pow(1.0 - cell.x, 1.25);
    feather = mix(feather, gInk, seam * mosaicMask * 0.55 * (0.55 + heightDark));
    float rimCut = pow(smoothstep(0.22, 0.98, cell.x), 1.45) * (1.0 - seam);
    feather = mix(feather, mix(gEmerald, mix(gMint, gRim, tip), 0.5 + heightLite * 0.3), rimCut * mosaicMask * 0.78);

    float facet1 = glassFacet(mosaicUV * 1.6 + vec2(leafId * 2.0, 0.6));
    float facet2 = glassFacet(mosaicUV * 3.4 + vec2(1.4, leafId2 * 3.2));
    float facet3 = glassFacet(mosaicUV * 6.8 + vec2(leafId3 * 4.2, along * 2.2));
    float facet4 = glassFacet(mosaicUV * 10.5 + vec2(3.1, tip * 2.0));
    float facetMask = mosaicMask * (1.0 - smoothstep(0.1, 0.38, cellFw));
    feather = mix(feather, mix(gDeep, gEmerald, 0.6), facet1 * facetMask * 0.55);
    feather = mix(feather, gInk, (1.0 - clamp(facet1, 0.0, 1.0)) * facetMask * 0.22);
    feather = mix(feather, mix(gEmerald, gSage, 0.55), facet2 * facetMask * 0.5 * (0.4 + wash));
    feather = mix(feather, mix(gMint, gGlow, 0.45), facet3 * facetMask * 0.42 * (0.3 + tip + heightLite * 0.5));
    feather = mix(feather, gRim, facet4 * facetMask * 0.28 * (0.25 + wash + tip));

    float spark = glassSparkle(mosaicUV * 5.0 + vec2(uMeshSeed, along * 3.2));
    float spark2 = glassSparkle(mosaicUV * 9.5 + vec2(2.4, leafId));
    feather = mix(feather, mix(gMint, gRim, 0.5), spark * facetMask * 0.58);
    feather = mix(feather, gRim, spark2 * facetMask * 0.32 * (0.35 + tip + heightLite * 0.3));

    // Edge barbs + HOT rim bloom (Noomo signature)
    float edgeZone = smoothstep(0.25, 0.95, away);
    float barb = glassBarb(acrossN, along, mix(30.0, 46.0, leafId));
    float barb2 = glassBarb(acrossN * 1.4, along * 1.3 + leafId2, mix(40.0, 60.0, leafId3));
    feather = mix(feather, mix(gForest, gSage, 0.5), barb * edgeZone * 0.4);
    feather = mix(feather, mix(gSage, gMint, 0.55), barb2 * edgeZone * 0.35 * (0.4 + wash));
    feather = mix(feather, gInk, (1.0 - barb) * edgeZone * 0.16 * heightDark);

    float leafEdge = smoothstep(0.42, 1.02, away);
    float viewN = clamp(dot(n, normalize(cameraPosition - vFeatherWorldPos)), 0.0, 1.0);
    float fresnel = pow(1.0 - viewN, 1.45);
    // Hot rim bloom
    float rim = pow(leafEdge, 1.25) * (0.55 + fresnel * 0.85);
    feather = mix(feather, mix(gMint, gRim, 0.6), rim * 0.55 * (0.35 + wash + heightLite * 0.4 + tip * 0.4));
    feather = mix(feather, gRim, rim * rim * 0.4 * (0.3 + tip + heightLite * 0.35));
    feather = mix(feather, gInk, (1.0 - leafEdge) * 0.08 * heightDark);

    feather = mix(feather, gInk, fresnel * 0.14 * heightDark);
    feather = mix(feather, mix(gEmerald, gMint, 0.45 + heightLite * 0.35), fresnel * 0.32 * (0.3 + wash));
    feather = mix(feather, gRim, fresnel * fresnel * 0.22 * tip);

    float sheen = smoothstep(0.4, 0.98, ndl);
    feather = mix(feather, mix(gEmerald, gSage, 0.45), sheen * 0.18 * (0.35 + wash));
    feather = mix(feather, gInk, (1.0 - ndl) * 0.22 * heightDark);

    feather = mix(feather, mix(gSage, gMint, 0.6), tip * uTipBlendStrength * 0.45);
    feather = mix(feather, mix(gMint, gGlow, 0.55), heightLite * tip * 0.28);
    feather = mix(feather, gRim, tip * tip * 0.16);

    float luma = dot(feather, vec3(0.299, 0.587, 0.114));
    feather = mix(vec3(luma), feather, 1.28);
    feather *= mix(0.62, 1.12, 0.22 + wash * 0.55 + heightLite * 0.4);

    gl_FragColor.rgb = mix(gl_FragColor.rgb, feather, uPaintStrength);

    // Soft glass alpha — more ethereal on tips / lower wash
    float tipFade = tip * 0.18 + heightLite * 0.08 + wash * 0.04;
    gl_FragColor.a *= mix(1.0, 0.82, tipFade);
  } else {
...
export function applyBirdMaterial(bird, textures, lightweightOrOptions = false) {
  ...
  bird.traverse((child) => {
    ...
    const material = new THREE.MeshPhysicalMaterial({
      ...
      emissiveIntensity: useHeroWingLook ? (isBottom ? 0.26 : 0.14) : isWing ? 0.0 : 0.004,
      ...
      roughness: useHeroWingLook ? 0.2 : isWing ? 0.97 : 0.92,
      ...
      thickness: useHeroWingLook ? 0.85 : 0,
      ior: useHeroWingLook ? 1.42 : 1.5,
      transparent: useHeroWingLook,
      opacity: useHeroWingLook ? 0.94 : 1,
      flatShading: false,
...
export function applyHeroBirdMaterialLook(materials, bloom = 0) {
  ...
  materials.forEach((mat, index) => {
    ...
    if (isHeroWingLook && "transmission" in mat) {
      mat.transmission = THREE.MathUtils.lerp(0.2, 0.24, t);
      mat.opacity = THREE.MathUtils.lerp(0.92, 0.94, t);
      mat.thickness = THREE.MathUtils.lerp(0.7, 0.85, t);
      mat.ior = 1.42;
    }
...
export default function EagleScrollScene({
  ...
  useLayoutEffect(() => {
    ...
    renderer.toneMappingExposure = backgroundOnly ? 1.05 : 0.96;
    ...
    if (!backgroundOnly) {
      ...
      rimLight = new THREE.PointLight(0xf5fff5, 1.65, 55);
    ...
    if (backgroundOnly) {
      bottomLight = new THREE.DirectionalLight(0xb8f0c0, 0.85);
      bottomLight.position.set(0.4, -7, 2.5);
      scene.add(bottomLight);

      topShadeLight = new THREE.DirectionalLight(0x0a1810, 0.38);
    ...
    function applyScrollMaterialEffects(t) {
      ...
      if (bottomLight) bottomLight.intensity = THREE.MathUtils.lerp(0.75, 0.95, bloom);
      if (topShadeLight) topShadeLight.intensity = THREE.MathUtils.lerp(0.35, 0.42, bloom);
      if (limeAccent) limeAccent.intensity = THREE.MathUtils.lerp(1.1, 1.4, bloom);
      if (rimLight) rimLight.intensity = THREE.MathUtils.lerp(1.45, 1.75, bloom);
