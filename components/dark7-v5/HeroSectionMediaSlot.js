...
import { dark7V5ScrollTrigger, notifyScrollLayoutReady } from "./lenisScrollTrigger";
...
export default function HeroSectionMediaSlot({
  ...
  const getHeroStackX = useCallback(
    (index, currentActive = activeCardRef.current) => {
      ...
      if (index < currentActive) {
        stackIndex = mediaAssets.length - currentActive + index;
      } else {
        stackIndex = index - currentActive;
      }
      return stackIndex * heroStackOffset;
    },
    [mediaAssets.length, heroStackOffset],
  );
  ...
  // Card stacking GSAP (disabled while scrolling — morph owns transforms)
  useEffect(() => {
    ...
    heroCardsRef.current.forEach((card, index) => {
      if (!card) return;
      gsap.to(card, {
        x: getHeroStackX(index),
        y: 0,
        scaleX: 1,
        scaleY: 1,
        zIndex: index === activeCard ? 100 : 50 - index,
        transformOrigin: "0 0",
        duration: 0.5,
        ease: "power2.out",
      });
    });
  }, [activeCard, isDesktop, isScrolling, getHeroStackX]);

  // Initial stack positions (GSAP owns transforms — no inline transform)
  useLayoutEffect(() => {
    if (!isDesktop || isScrolling) return;
    heroCardsRef.current.forEach((card, index) => {
      if (!card) return;
  ...
  useLayoutEffect(() => {
    ...
    const applyMorph = (self) => {
      const morphProgress = self.progress;
      ...
      mediaAssets.forEach((_, index) => {
        ...
        const startX = getHeroStackX(index);
        const startY = 0;
  ...
  }, [isDesktop, screenSize, mediaAssets, heroStackOffset, heroCardHeight, getHeroStackX]);
  ...
  return (
    ...
    >
      ...
      <section
        ref={heroSectionRef}
        className={`dark7-v5-hero relative z-30 overflow-visible pb-0${
          theme === "dark" ? " dark7-v5-hero-dark" : ""
        }`}
        style={theme === "dark" && !sharedBackground ? dark7HeroSurfaceStyle() : undefined}
      >
        <div className="dark7-v5-hero-pin relative z-[1] h-[100svh] min-h-[100svh] w-full overflow-hidden">
          <div
            ref={heroContentRef}
            className="dark7-v5-hero-content absolute inset-0 z-20 flex flex-col pt-32 md:pt-40"
          >
            <div className="relative z-10 mx-auto flex h-full w-full max-w-[1800px] flex-col px-4 md:px-6 lg:px-10">
              <div className="flex min-h-0 flex-col pt-8 md:pt-10 lg:max-w-[900px]">
                <div
                  ref={titleContainerRef}
                  className="dark7-v5-hero-title max-w-full lg:max-w-[1600px] xl:max-w-[1800px]"
                >
                  <div className="hero-badge mb-8 flex items-center gap-2 sm:mb-10 sm:gap-3 md:mb-12">
                    <Image
                      src="/feather-heading.png"
                      alt=""
                      width={40}
                      height={40}
                      className={`hero-badge-feather h-5 w-auto shrink-0 sm:h-6 md:h-7 ${
                        theme === "dark" ? "brightness-0 invert" : ""
                      }`}
                      aria-hidden
                    />
                    <span
                      className={`font-merriweather text-[13px] font-light uppercase tracking-[0.16em] md:text-[15px] ${
                ...
                <div className="hero-body relative z-[250] max-w-full pt-14 sm:pt-16 md:pt-20 lg:pt-24">
                  <p
                ...
                <div className="hero-cta-row relative z-[250] mt-8 sm:mt-9 md:mt-10">
                  {heroCtaButton}
                </div>
              </div>
                ...
                {mediaAssets.map((asset, index) => (
                  ...
                  >
                    <div className="card-inner-content relative h-full w-full overflow-hidden rounded-xl">
                      ...
                      <div className="card-overlay pointer-events-none absolute inset-0 z-15 bg-gradient-to-t from-black/70 via-transparent to-transparent">
                        <div className="absolute bottom-3 left-3 right-3">
                          <h3 className="mb-1 font-merriweather text-sm font-bold text-white">
                            {asset.title}
                          </h3>
                          <p className="text-xs text-white/80">{asset.subtitle}</p>
                        </div>
                      </div>
      ...
      <section
        ...
        id="discover"
        className="dark7-v5-portfolio relative z-20 w-full overflow-visible py-0"
      >
        <div className="relative z-10 mx-auto w-full max-w-[1800px] px-4 sm:px-6 lg:px-8">
          <header className="mb-12 text-center sm:mb-16">
            <p
              className={`dark7-v5-portfolio-eyebrow mb-4 flex items-center justify-center gap-2 font-playfair text-base sm:mb-6 sm:gap-3 sm:text-lg md:text-xl lg:text-2xl ${
            ...
            <h2 className="dark7-v5-portfolio-title font-italiana text-3xl leading-tight text-[#162D24] sm:text-4xl md:text-5xl lg:text-6xl xl:text-7xl">
              <span className="dark7-v5-portfolio-title-lead font-light text-[#162D24]">
                Creating impact for businesses in{" "}
              </span>
              <span className="dark7-v5-portfolio-city-pill relative -top-[2px] inline-flex min-w-[6ch] items-center justify-center overflow-hidden rounded-xl bg-[#1D322740] px-3 py-1 align-middle text-[#F7F3F0]">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {mediaAssets.map((item, index) => (
              <div key={index} className="flex flex-col items-center">
                <div
                  className={`dark7-v5-portfolio-card relative flex flex-col w-full ${
                  ...
                  >
                    <Link href={item.link} className="block h-full">
                      <div className="px-4 sm:px-6 py-5 sm:py-6 flex flex-col justify-center h-full">
                        <h3 className="dark7-v5-portfolio-card-title font-merriweather font-light leading-snug tracking-tight mb-2 transition-colors duration-300">
                          {item.title}
                        </h3>
                        <div className="flex flex-wrap gap-2">
                          {item.buttons.map((button, btnIndex) => {
                            ...
                            <span
                              key={btnIndex}
                              className={`dark7-v5-portfolio-card-tag font-merriweather border rounded-full transition-colors duration-300 ${
