...
import {
  Dark7V50ScrollTrigger,
  getDark7V50ScrollTop,
  refreshDark7V50ScrollTriggers,
  notifyHeroPinReady,
  DARK7_V50_HERO_PIN_ID,
...
/**
 * v50 — v48 depth/veins look, but transparent glass sheets (layered see-through)
 */
...
function applyFeatherSurfaceShader(
  ...
  material.customProgramCacheKey = () =>
    `feather-leaf-depth-vein-glass-v50b-${isWing ? 1 : 0}-${heroWingLook ? 1 : 0}-${axis?.axisIdx ?? "b"}-${axis?.acrossIdx ?? "a"}`;
...
// ——— v48: glass mosaic + proper veins + depth AO ———
...
// Diagonal vein sweep from midrib (readable rachis barbs)
float glassDiagVein(float across, float along, float density, float skew) {
  float wob = featherFbm(vec2(along * 1.6, across * 0.7)) * 0.35;
  float phase = across * density + along * skew + wob;
  return featherVeinRidge(phase, 0.045);
}`
      )
      .replace(
  ...
  if (uHeroWingLook > 0.5) {
    // v50 — v48 depth/veins, transparent glass sheets
    ...
    float t = pow(along, mix(0.85, 1.2, leafId));
    t = clamp(t + (leafId2 - 0.5) * 0.03, 0.0, 1.0);

    // World height: TOP dark, BOTTOM light (primary gradient)
    float relY = clamp((vFeatherWorldPos.y - uBirdOrigin.y + 0.65) / 2.25, 0.0, 1.0);
    float heightDark = pow(1.0 - relY, 0.85);
    float heightLite = pow(relY, 0.9);
    ...
    vec3 gInk    = mix(cDeep, vec3(0.01, 0.05, 0.03), 0.55);
    vec3 gDeep   = mix(cDeep, vec3(0.03, 0.11, 0.06), 0.5);
    vec3 gForest = mix(cMid,  vec3(0.07, 0.24, 0.13), 0.45);
    vec3 gEmerald= mix(cBody, vec3(0.12, 0.4, 0.24), 0.4);
    vec3 gSage   = mix(cLite, vec3(0.32, 0.58, 0.4), 0.4);
    vec3 gMint   = mix(cTip,  vec3(0.52, 0.85, 0.65), 0.4);
    vec3 gGlow   = mix(cTip,  vec3(0.72, 0.96, 0.84), 0.45);

    // Explicit top→bottom wash (height drives most of it)
    float wash = clamp(
      heightLite * 0.72 + tip * 0.38 + t * 0.18 - heightDark * 0.35,
      0.0, 1.0
    );
    wash = pow(wash, 0.92);

    vec3 feather = mix(gInk, gDeep, smoothstep(0.0, 0.22, wash));
    feather = mix(feather, gForest, smoothstep(0.12, 0.42, wash));
    feather = mix(feather, gEmerald, smoothstep(0.35, 0.68, wash));
    feather = mix(feather, gSage, smoothstep(0.55, 0.85, wash));
    feather = mix(feather, mix(gMint, gGlow, tip), smoothstep(0.72, 1.0, wash));
    feather = mix(feather, gInk, heightDark * 0.48 * (1.0 - tip * 0.55));
    feather = mix(feather, mix(gSage, gMint, 0.45), heightLite * 0.28 * (0.35 + tip));

    // --- Depth AO (layering / valleys) ---
    float depthAO = featherFbm(vec2(along * 2.4 + uMeshSeed, acrossN * 1.8));
    float depthAO2 = featherFbm(vec2(along * 5.5, acrossN * 4.2 + leafId));
    float occlude = pow(1.0 - ndl, 1.4) * 0.55 + depthAO * 0.35 + depthAO2 * 0.25;
    occlude *= mix(1.0, 0.35, wash);
    feather = mix(feather, gInk, occlude * 0.42 * (0.55 + heightDark));
    float lift = smoothstep(0.4, 0.95, depthAO2) * heightLite;
    feather = mix(feather, mix(gEmerald, gMint, 0.4), lift * 0.22);

    // --- Readable veins (midrib + diagonals + fine fibers) ---
    float midribWob = (featherNoise(vec2(along * 3.5, uMeshSeed)) - 0.5) * 0.04;
    float midDist = abs(acrossN - midribWob);
    float midrib = 1.0 - smoothstep(0.0, 0.085, midDist);
    float midribCore = 1.0 - smoothstep(0.0, 0.035, midDist);
    float midribRim = midrib * (1.0 - midribCore);
    feather = mix(feather, gInk, midribCore * 0.62 * (0.55 + heightDark * 0.45));
    feather = mix(feather, mix(gEmerald, gSage, wash), midribRim * 0.55 * (0.4 + wash));
    feather = mix(feather, mix(gMint, gGlow, tip), midribRim * tip * 0.35);

    float dens1 = mix(7.5, 11.5, leafId);
    float dens2 = mix(12.0, 18.0, leafId2);
    float dens3 = mix(20.0, 30.0, leafId3);
    float skew1 = mix(1.8, 2.8, leafId2);
    float skew2 = mix(2.4, 3.6, leafId3);
    float v1 = glassDiagVein(acrossN, along, dens1, skew1);
    float v2 = glassDiagVein(acrossN, along + 0.17, dens2, skew2);
    float v3 = glassDiagVein(acrossN * 1.15, along * 0.92, dens3, mix(3.0, 4.2, leafId));
    float veinBody = (1.0 - tip * 0.35) * smoothstep(0.02, 0.2, away);
    float veinDark = (v1 * 0.85 + v2 * 0.55) * veinBody;
    float veinLite = (v1 * 0.35 + v2 * 0.55 + v3 * 0.7) * veinBody;
    feather = mix(feather, gInk, veinDark * 0.48 * (0.45 + heightDark * 0.55));
    feather = mix(feather, mix(gForest, gEmerald, wash), veinLite * 0.38);
    feather = mix(feather, mix(gSage, gMint, 0.5), v3 * veinBody * 0.28 * (0.3 + wash));

    float fiber = featherFiber(acrossN, along, mix(14.0, 22.0, leafId), 0.55);
    float fiber2 = featherFiber(acrossN * 1.2, along * 1.1 + leafId2, mix(22.0, 34.0, leafId3), 0.4);
    float fiberZone = smoothstep(0.08, 0.75, away) * (1.0 - tip * 0.25);
    feather = mix(feather, mix(gDeep, gForest, wash), fiber * fiberZone * 0.28);
    feather = mix(feather, mix(gEmerald, gSage, wash), fiber2 * fiberZone * 0.22 * (0.35 + wash));
    feather = mix(feather, gInk, (1.0 - fiber) * fiberZone * 0.12 * heightDark);

    // --- Mosaic patches (between veins) ---
    vec2 mosaicUV = vec2(
      along * mix(3.6, 5.6, leafId) + leafId2 * 1.3,
      acrossN * mix(2.8, 4.4, leafId3) + side * 0.5 + uMeshSeed * 0.3
    );
    mosaicUV.x += away * 0.4;
    ...
    float veinProtect = 1.0 - clamp(midrib * 0.85 + v1 * 0.7 + v2 * 0.45, 0.0, 1.0);

    vec3 patchA = mix(gInk, gDeep, 0.35);
    vec3 patchB = mix(gDeep, gForest, 0.55);
    vec3 patchC = mix(gForest, gEmerald, 0.6);
    vec3 patchD = mix(gEmerald, mix(gSage, gMint, tip + heightLite * 0.5), 0.5 + wash * 0.35);
    vec3 patchCol = mix(patchA, patchB, smoothstep(0.0, 0.35, cellId));
    patchCol = mix(patchCol, patchC, smoothstep(0.25, 0.65, cellId));
    patchCol = mix(patchCol, patchD, smoothstep(0.55, 1.0, cellId) * (0.3 + wash * 0.7));
    patchCol *= mix(0.55, 1.22, cellShade);
    patchCol = mix(patchCol, gInk, heightDark * 0.4 * (1.0 - cellId * 0.45));
    patchCol = mix(patchCol, gMint, heightLite * 0.18 * cellId);

    float mosaicMask = cellAA * smoothstep(0.0, 0.06, away + 0.04) * veinProtect;
    feather = mix(feather, patchCol, mosaicMask * 0.78);

    float seam = pow(1.0 - cellEdge, 1.35);
    feather = mix(feather, gInk, seam * mosaicMask * 0.5 * (0.5 + heightDark));
    float rimCut = pow(smoothstep(0.28, 0.98, cellEdge), 1.6) * (1.0 - seam);
    feather = mix(feather, mix(gEmerald, mix(gSage, gMint, heightLite), 0.45), rimCut * mosaicMask * 0.62);
    ...
    feather = mix(feather, mix(gDeep, gEmerald, 0.55), facet1 * facetMask * 0.48);
    feather = mix(feather, mix(gInk, gForest, 0.4), (1.0 - facet1) * facetMask * 0.2);
    feather = mix(feather, mix(gEmerald, gSage, 0.5), facet2 * facetMask * 0.42 * (0.35 + wash));
    feather = mix(feather, mix(gMint, gGlow, 0.4), facet3 * facetMask * 0.35 * (0.25 + tip + heightLite * 0.5));

    vec4 cell2 = glassCell(mosaicUV * 2.8 + vec2(leafId3 * 2.2, 1.7));
    feather = mix(feather, mix(gEmerald, gSage, cell2.y), pow(cell2.x, 1.2) * facetMask * 0.28);
    feather = mix(feather, gInk, pow(1.0 - cell2.x, 2.0) * facetMask * 0.26 * heightDark);

    float spark = glassSparkle(mosaicUV * 4.5 + vec2(uMeshSeed, along * 3.0));
    float spark2 = glassSparkle(mosaicUV * 8.0 + vec2(2.1, leafId));
    feather = mix(feather, mix(gMint, gGlow, 0.55), spark * facetMask * 0.48);
    feather = mix(feather, gGlow, spark2 * facetMask * 0.24 * (0.3 + tip + heightLite * 0.3));

    float edgeZone = smoothstep(0.28, 0.95, away);
    ...
    feather = mix(feather, mix(gForest, gSage, 0.45), barb * edgeZone * 0.38);
    feather = mix(feather, mix(gSage, gMint, 0.5), barb2 * edgeZone * 0.3 * (0.35 + wash + heightLite * 0.3));
    feather = mix(feather, gInk, (1.0 - barb) * edgeZone * 0.12 * heightDark);

    float leafEdge = smoothstep(0.5, 1.05, away);
    feather = mix(feather, mix(gSage, gMint, 0.5), leafEdge * 0.3 * (0.25 + wash + heightLite * 0.4));

    float viewN = clamp(dot(n, normalize(cameraPosition - vFeatherWorldPos)), 0.0, 1.0);
    float fresnel = pow(1.0 - viewN, 1.55);
    feather = mix(feather, gInk, fresnel * 0.16 * heightDark);
    feather = mix(feather, mix(gEmerald, gMint, 0.4 + heightLite * 0.3), fresnel * 0.26 * (0.25 + wash));
    feather = mix(feather, gGlow, fresnel * fresnel * 0.16 * tip);

    float sheen = smoothstep(0.45, 0.98, ndl);
    feather = mix(feather, mix(gEmerald, gSage, 0.4), sheen * 0.14 * (0.3 + wash));
    feather = mix(feather, gInk, (1.0 - ndl) * 0.2 * heightDark);

    feather = mix(feather, mix(gSage, gMint, 0.55), tip * uTipBlendStrength * 0.38);
    feather = mix(feather, mix(gMint, gGlow, 0.5), heightLite * tip * 0.22);
    feather = mix(feather, gGlow, tip * tip * 0.1);

    float luma = dot(feather, vec3(0.299, 0.587, 0.114));
    feather = mix(vec3(luma), feather, 1.2);
    feather *= mix(0.68, 1.05, 0.25 + wash * 0.55 + heightLite * 0.35);

    gl_FragColor.rgb = mix(gl_FragColor.rgb, feather, uPaintStrength);

    // Ghost only the BOTTOM half — upper mass stays mostly solid
    float viewN2 = clamp(dot(n, normalize(cameraPosition - vFeatherWorldPos)), 0.0, 1.0);
    float ghostZone = smoothstep(0.36, 0.72, relY); // 0 top, 1 bottom
    ghostZone = max(ghostZone, tip * 0.7 * smoothstep(0.28, 0.7, relY));
    float rimHold = pow(1.0 - viewN2, 1.35) * 0.1;
    float glassAlpha = mix(0.96, 0.34, ghostZone) + rimHold;
    gl_FragColor.a *= clamp(glassAlpha, 0.28, 1.0);
  } else {
...
export function applyBirdMaterial(bird, textures, lightweightOrOptions = false) {
  ...
  bird.traverse((child) => {
    ...
    const material = new THREE.MeshPhysicalMaterial({
      ...
      // Upper leaves more solid; bottom leaves ghostly glass
      ...
      roughness: useHeroWingLook ? (isBottom ? 0.16 : 0.26) : isWing ? 0.97 : 0.92,
      metalness: useHeroWingLook ? 0.0 : 0.0,
      clearcoat: useHeroWingLook ? 1.0 : 0.0,
      clearcoatRoughness: useHeroWingLook ? (isBottom ? 0.08 : 0.16) : 1.0,
      reflectivity: useHeroWingLook ? 0.5 : 0.008,
      sheen: useHeroWingLook ? (isBottom ? 0.75 : 0.45) : 0.0,
      sheenRoughness: useHeroWingLook ? 0.28 : 1.0,
      sheenColor: new THREE.Color(swatch.cool ?? swatch.tip),
      specularIntensity: useHeroWingLook ? 0.9 : 0.12,
      transmission: useHeroWingLook ? (isBottom ? 0.52 : 0.1) : 0.0,
      thickness: useHeroWingLook ? (isBottom ? 0.32 : 0.55) : 0,
      ior: useHeroWingLook ? (isBottom ? 1.34 : 1.42) : 1.5,
      transparent: useHeroWingLook,
      opacity: useHeroWingLook ? (isBottom ? 0.58 : 0.94) : 1,
      flatShading: false,
      envMapIntensity: useHeroWingLook ? (isBottom ? 1.2 : 0.85) : isWing ? 0.02 : 0.015,
...
export function applyHeroBirdMaterialLook(materials, bloom = 0) {
  ...
  materials.forEach((mat, index) => {
    ...
    mat.roughness = isHeroWingLook
      ? (isBottomFeather
        ? THREE.MathUtils.lerp(0.18, 0.16, t)
        : THREE.MathUtils.lerp(0.28, 0.26, t))
    ...
    if (isHeroWingLook && "transmission" in mat) {
      ...
      mat.opacity = isBottomFeather
        ? THREE.MathUtils.lerp(0.54, 0.58, t)
        : THREE.MathUtils.lerp(0.92, 0.94, t);
      mat.thickness = isBottomFeather
        ? THREE.MathUtils.lerp(0.26, 0.32, t)
        : THREE.MathUtils.lerp(0.48, 0.55, t);
      mat.ior = isBottomFeather ? 1.34 : 1.42;
...
export default function EagleScrollScene({
  ...
  useLayoutEffect(() => {
    ...
    function createScrollAnimation(triggerEl) {
      ...
      // Initial camera for embedded hero (dark7-v50).
      ...
      ScrollTrigger.getById(DARK7_V50_HERO_PIN_ID)?.kill();
      ...
      gsapCtx = gsap.context(() => {
        ...
        scrollTween = gsap.to(state, {
          progress: 1,
          ease: "none",
          scrollTrigger: Dark7V50ScrollTrigger({
            id: embeddedScroll ? DARK7_V50_HERO_PIN_ID : "eagle-scroll-scene",
            trigger: triggerEl,
            start: "top top",
      ...
      requestAnimationFrame(() => {
        refreshDark7V50ScrollTriggers();
        const progress =
          getDark7V50ScrollTop() < 8 ? 0 : (scrollTween?.scrollTrigger?.progress ?? 0);
        applyScrollProgress(progress);
        onScrollProgressRef.current?.(progress <= 0.001 ? 0 : progress);
      });

      window.setTimeout(() => {
        refreshDark7V50ScrollTriggers(true);
        const progress =
          getDARK7_V50ScrollTop() < 8 ? 0 : (scrollTween?.scrollTrigger?.progress ?? 0);
        applyScrollProgress(progress);
        onScrollProgressRef.current?.(progress <= 0.001 ? 0 : progress);
        if (embeddedScroll) {
          notifyHeroPinReady();
