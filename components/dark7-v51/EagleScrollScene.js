...
import {
  Dark7V51ScrollTrigger,
  getDark7V51ScrollTop,
  refreshDark7V51ScrollTriggers,
  notifyHeroPinReady,
  DARK7_V51_HERO_PIN_ID,
...
/**
 * v51 — stained-glass volume (Beer-Lambert + faceted normals)
...
function applyFeatherSurfaceShader(
  ...
  material.customProgramCacheKey = () =>
    `feather-stained-glass-volume-v51d-${isWing ? 1 : 0}-${heroWingLook ? 1 : 0}-${axis?.axisIdx ?? "b"}-${axis?.acrossIdx ?? "a"}`;
...
// ——— v51 NEW: stained-glass volume (Beer-Lambert + faceted normals) ———
...
float sgFiber(vec2 p, float freq, float ang) {
  ...
  if (uHeroWingLook > 0.5) {
    // v51 — stained emerald glass (Noomo structure, dark-green palette)
    ...
    // --- Optical thickness: keep mass dense (dark forest, not pale glass) ---
    float thick = mix(1.85, 0.55, heightLite);
    thick *= mix(1.18, 0.62, tip);
    thick *= mix(1.12, 0.88, along);
    thick *= mix(1.0, 0.88, smoothstep(0.45, 1.05, away));
    ...
    thick = clamp(thick, 0.22, 2.6);

    // Beer-Lambert — dark forest glass (site greens #162D24 → #8EAC85)
    vec3 absDeep = vec3(2.45, 0.72, 1.85);
    vec3 absMid  = vec3(1.85, 0.48, 1.35);
    vec3 absThin = vec3(1.15, 0.32, 0.88);
    vec3 sigma = mix(absDeep, absMid, smoothstep(0.18, 0.62, heightLite));
    sigma = mix(sigma, absThin, smoothstep(0.62, 1.0, heightLite) * 0.65);
    sigma *= mix(0.95, 1.55, inclAmt * (0.65 + heightDark * 0.4));
    vec3 trans = exp(-sigma * thick);

    vec3 gInk  = vec3(0.012, 0.038, 0.022);
    vec3 gGlow = vec3(0.38, 0.62, 0.44);
    vec3 feather = mix(gInk, trans, 0.78);
    feather = mix(feather, vec3(feather.r * 0.45, feather.g, feather.b * 0.55), 0.42);

    vec3 islandDark = vec3(0.02, 0.09, 0.045);
    vec3 islandMid  = vec3(0.06, 0.24, 0.12);
    vec3 islandLite = vec3(0.16, 0.42, 0.24);
    vec3 island = mix(islandDark, islandMid, inclAmt);
    island = mix(island, islandLite, inclAmt2 * heightLite * 0.45);
    feather = mix(feather, island, 0.38 * mix(0.4, 0.75, inclAmt));

    vec3 faceCol = mix(islandDark, mix(islandMid, islandLite, heightLite * 0.5), cr1.y);
    feather = mix(feather, faceCol, 0.28 * cellAA);
    vec3 faceCol2 = mix(islandDark, islandMid, cr2.y);
    feather = mix(feather, faceCol2, 0.14 * cellAA);
    ...
    feather *= mix(0.62, 1.08, faceLit);
    feather = mix(feather, gInk, faceShade * 0.42 * (0.55 + heightDark));
    float seam = pow(1.0 - cr1.x, 1.35) * cellAA;
    float cut = pow(smoothstep(0.25, 0.98, cr1.x), 1.5) * cellAA;
    feather = mix(feather, gInk, seam * 0.48 * (0.55 + heightDark));
    feather = mix(feather, mix(trans, gGlow, 0.25), cut * 0.32 * (0.2 + heightLite * 0.35));
    float cut2 = pow(smoothstep(0.3, 0.98, cr2.x), 1.6) * cellAA;
    feather = mix(feather, gGlow, cut2 * 0.1 * heightLite);

    float spark = pow(smoothstep(0.55, 0.95, cr3.y) * cr3.x, 1.2) * cellAA;
    feather = mix(feather, gGlow, spark * 0.14 * (0.2 + fresnel * 0.5));

    feather = mix(feather, mix(trans, gGlow, 0.2), fib1 * fibZone * 0.12 * heightLite);
    feather = mix(feather, gInk, fib2 * fibZone * 0.32 * (0.5 + heightDark));
    feather = mix(feather, gInk, (1.0 - fib1) * fibZone * 0.16 * heightDark);
    feather = mix(feather, islandMid, fib3 * fibZone * 0.1);

    float film = 0.5 + 0.5 * cos(thick * 16.0 + viewN * 7.5 + inc * 9.0 + leafId * 4.0);
    vec3 filmA = vec3(0.08, 0.28, 0.14);
    vec3 filmB = vec3(0.16, 0.42, 0.22);
    vec3 filmC = vec3(0.28, 0.55, 0.34);
    vec3 filmCol = mix(filmA, filmB, clamp(film, 0.0, 1.0));
    filmCol = mix(filmCol, filmC, pow(fresnel, 1.2));
    feather = mix(feather, filmCol, fresnel * 0.18 * (0.25 + heightLite * 0.25));

    float leafEdge = smoothstep(0.42, 1.05, away);
    float rim = pow(max(leafEdge, fresnel), 1.25);
    feather = mix(feather, mix(trans, gGlow, 0.35), rim * 0.22 * (0.25 + heightLite * 0.3));
    feather = mix(feather, vec3(0.42, 0.62, 0.46), rim * rim * 0.12 * heightLite);

    float ao = pow(1.0 - ndl0, 1.35) * 0.45 + (1.0 - along) * 0.12 * heightDark;
    feather = mix(feather, gInk, ao * 0.5 * (0.6 + heightDark));

    feather = mix(feather, mix(trans, gGlow, 0.25), tip * uTipBlendStrength * 0.14);
    feather = mix(feather, gGlow, heightLite * tip * 0.05);

    float luma = dot(feather, vec3(0.299, 0.587, 0.114));
    feather = mix(vec3(luma), feather, 1.38);
    feather *= mix(0.52, 0.92, 0.22 + heightLite * 0.35 + tip * 0.12);
    feather = clamp(feather, 0.0, 0.72);
    ...
    float alpha = mix(0.98, 0.62, ghost) * mix(1.0, od, 0.35);
    alpha *= mix(1.0, 0.9, fresnel * 0.25);
    gl_FragColor.a *= clamp(alpha, 0.5, 1.0);
  } else {
...
export function applyBirdMaterial(bird, textures, lightweightOrOptions = false) {
  ...
  bird.traverse((child) => {
    ...
    const material = new THREE.MeshPhysicalMaterial({
      ...
      emissiveIntensity: useHeroWingLook ? (isBottom ? 0.12 : 0.06) : isWing ? 0.0 : 0.004,
      ...
      sheen: useHeroWingLook ? (isBottom ? 0.45 : 0.28) : 0.0,
      ...
      transmission: useHeroWingLook ? (isBottom ? 0.28 : 0.1) : 0.0,
      thickness: useHeroWingLook ? (isBottom ? 0.45 : 0.75) : 0,
      ior: useHeroWingLook ? (isBottom ? 1.36 : 1.42) : 1.5,
      transparent: useHeroWingLook,
      opacity: useHeroWingLook ? (isBottom ? 0.78 : 0.96) : 1,
      flatShading: false,
  ...
  materials.forEach((mat, index) => {
    ...
    if ("clearcoat" in mat) {
      ...
      mat.envMapIntensity = isHeroWingLook
        ? THREE.MathUtils.lerp(isBottomFeather ? 0.7 : 0.45, isBottomFeather ? 0.85 : 0.55, t)
    ...
    if (isHeroWingLook && "transmission" in mat) {
      mat.transmission = isBottomFeather
        ? THREE.MathUtils.lerp(0.22, 0.28, t)
        : THREE.MathUtils.lerp(0.08, 0.1, t);
      mat.opacity = isBottomFeather
        ? THREE.MathUtils.lerp(0.74, 0.78, t)
        : THREE.MathUtils.lerp(0.94, 0.96, t);
      mat.thickness = isBottomFeather
        ? THREE.MathUtils.lerp(0.38, 0.45, t)
        : THREE.MathUtils.lerp(0.65, 0.75, t);
      mat.ior = isBottomFeather ? 1.36 : 1.42;
...
export default function EagleScrollScene({
  ...
  useLayoutEffect(() => {
    ...
    renderer.toneMappingExposure = backgroundOnly ? 0.82 : 0.96;

    scene.add(new THREE.AmbientLight(0x6a8a74, backgroundOnly ? 0.45 : 1.7));
    ...
    if (!backgroundOnly) {
      ...
      fillLight = new THREE.HemisphereLight(0x6a9a78, 0x0c1c14, 0.55);
      scene.add(fillLight);

      jadeLight = new THREE.PointLight(0x2a6840, 1.05, 52);
      jadeLight.position.set(4.2, 2.2, 4.2);
      scene.add(jadeLight);

      rimLight = new THREE.PointLight(0xb8d4bc, 0.85, 56);
      rimLight.position.set(5.4, 2.8, 3.6);
      scene.add(rimLight);

      const topSoft = new THREE.DirectionalLight(0x3a5a44, 0.28);
    ...
    if (backgroundOnly) {
      bottomLight = new THREE.DirectionalLight(0x4a7a58, 0.42);
      bottomLight.position.set(0.4, -7, 2.5);
      scene.add(bottomLight);

      topShadeLight = new THREE.DirectionalLight(0x08140e, 0.4);
      topShadeLight.position.set(-0.5, 8, 1.5);
      scene.add(topShadeLight);

      limeAccent = new THREE.PointLight(0x3a7a50, 0.45, 44);
    ...
    function createScrollAnimation(triggerEl) {
      ...
      // Initial camera for embedded hero (dark7-v51).
      ...
      ScrollTrigger.getById(DARK7_V51_HERO_PIN_ID)?.kill();
      ...
      gsapCtx = gsap.context(() => {
        ...
        scrollTween = gsap.to(state, {
          progress: 1,
          ease: "none",
          scrollTrigger: Dark7V51ScrollTrigger({
            id: embeddedScroll ? DARK7_V51_HERO_PIN_ID : "eagle-scroll-scene",
            trigger: triggerEl,
            start: "top top",
      ...
      requestAnimationFrame(() => {
        refreshDark7V51ScrollTriggers();
        const progress =
          getDark7V51ScrollTop() < 8 ? 0 : (scrollTween?.scrollTrigger?.progress ?? 0);
        applyScrollProgress(progress);
        onScrollProgressRef.current?.(progress <= 0.001 ? 0 : progress);
      });

      window.setTimeout(() => {
        refreshDark7V51ScrollTriggers(true);
        const progress =
          getDark7V51ScrollTop() < 8 ? 0 : (scrollTween?.scrollTrigger?.progress ?? 0);
        applyScrollProgress(progress);
        onScrollProgressRef.current?.(progress <= 0.001 ? 0 : progress);
