...
import {
  ...
  getDark7V52ScrollTop,
  refreshDark7V52ScrollTriggers,
  notifyHeroPinReady,
...
/**
 * v52 — v51c stained glass restored; Noomo-like greens only where they belong
...
function applyFeatherSurfaceShader(
  ...
  material.customProgramCacheKey = () =>
    `feather-stained-glass-volume-v52b-${isWing ? 1 : 0}-${heroWingLook ? 1 : 0}-${axis?.axisIdx ?? "b"}-${axis?.acrossIdx ?? "a"}`;
...
// ——— v52: stained-glass volume (Beer-Lambert + faceted normals) ———
...
float sgFiber(vec2 p, float freq, float ang) {
  ...
  if (uHeroWingLook > 0.5) {
    // v52 — stained emerald glass (Noomo: dark only in deep/overlap/patches)
...
export default function EagleScrollScene({
  ...
  useLayoutEffect(() => {
    ...
    function createScrollAnimation(triggerEl) {
      ...
      // Initial camera for embedded hero (dark7-v52).
      ...
      ScrollTrigger.getById(DARK7_V52_HERO_PIN_ID)?.kill();
      ...
      gsapCtx = gsap.context(() => {
        ...
        scrollTween = gsap.to(state, {
          ...
          scrollTrigger: Dark7V52ScrollTrigger({
            id: embeddedScroll ? DARK7_V52_HERO_PIN_ID : "eagle-scroll-scene",
            trigger: triggerEl,
            start: "top top",
      ...
      requestAnimationFrame(() => {
        refreshDark7V52ScrollTriggers();
        const progress =
          getDark7V52ScrollTop() < 8 ? 0 : (scrollTween?.scrollTrigger?.progress ?? 0);
