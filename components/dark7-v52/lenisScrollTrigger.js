...
/** Lenis + ScrollTrigger scroller for all dark7-v52 sections. */
export const DARK7_V52_SCROLLER =
...
export function getDark7V52ScrollTop() {
  if (lenisInstance) return lenisInstance.scroll;
  if (DARK7_V52_SCROLLER) {
    return ScrollTrigger.getScrollFunc(DARK7_V52_SCROLLER)?.() ?? window.scrollY;
  }
  return window.scrollY;
}

/** True when any pinned ScrollTrigger is currently active. */
export function hasActiveDark7V52Pin() {
  return ScrollTrigger.getAll().some(
    (st) => st.isActive && Boolean(st.vars?.pin || st.pin),
  );
}

/** Merge dark7-v52 scroll defaults into a ScrollTrigger config object. */
export function Dark7V52ScrollTrigger(vars = {}) {
  return {
    scroller: DARK7_V52_SCROLLER,
 ...
 */
function applyDark7V52ScrollerProxy(lenis) {
  if (!DARK7_V52_SCROLLER) return;

  // Lenis always transforms documentElement — "fixed" pins shift downstream triggers.
  const pinType = lenis ? "transform" : DARK7_V52_SCROLLER.style.transform ? "transform" : "fixed";

  ScrollTrigger.scrollerProxy(DARK7_V52_SCROLLER, {
    scrollTop(value) {
      if (arguments.length) {
...
export function initDark7V52LenisScroll(lenis) {
  if (typeof window === "undefined" || !DARK7_V52_SCROLLER) return;

  lenisInstance = lenis;
  if (typeof window !== "undefined") window.__techeyrieLenis = lenis;
  layoutSettled = false;

  applyDark7V52ScrollerProxy(lenis);

  const onRefreshInit = () => applyDark7V52ScrollerProxy(lenis);
  ...
  lenis.on("scroll", () => {
    ...
    if (typeof window !== "undefined") {
      window.dispatchEvent(
        new CustomEvent("dark7-v52-scroll", { detail: lenis.scroll }),
      );
    }
  });
  ...
  requestAnimationFrame(() => {
    applyDark7V52ScrollerProxy(lenis);
    refreshDark7V52ScrollTriggers(true);
  });
}

export function destroyDark7V52LenisScroll(lenis) {
  if (typeof window !== "undefined" && window.__techeyrieLenis === lenis) {
 ...
 */
export function refreshDark7V52ScrollTriggers(hard = true) {
  if (typeof window === "undefined") return;

  if (hard && layoutSettled && hasActiveDark7V52Pin()) {
    ScrollTrigger.update();
    return;
  }
...
export function markDark7V52ScrollLayoutSettled() {
  layoutSettled = true;
}

export const DARK7_V52_SCROLL_LAYOUT_READY_EVENT = "dark7-v52-scroll-layout-ready";
...
export function notifyScrollLayoutReady() {
  ...
  refreshDark7V52ScrollTriggers(true);
  window.dispatchEvent(new Event(DARK7_V52_SCROLL_LAYOUT_READY_EVENT));

  requestAnimationFrame(() => refreshDark7V52ScrollTriggers(true));
  window.setTimeout(() => refreshDark7V52ScrollTriggers(true), 120);
}
...
export function subscribeAfterScrollLayout(callback) {
  ...
  const run = () => {
    ...
    callback();
    requestAnimationFrame(() => {
      if (!cancelled) refreshDark7V52ScrollTriggers(true);
    });
  };
  ...
  const onReady = () => {
    window.removeEventListener(DARK7_V52_SCROLL_LAYOUT_READY_EVENT, onReady);
    window.clearTimeout(fallbackTimer);
    run();
  };

  window.addEventListener(DARK7_V52_SCROLL_LAYOUT_READY_EVENT, onReady);
...
export const DARK7_V52_HERO_PIN_ID = "dark7-v52-hero-pin";
