...
/** Lenis + ScrollTrigger scroller for all dark7-v53 sections. */
export const DARK7_V53_SCROLLER =
...
export function getDark7V53ScrollTop() {
  if (lenisInstance) return lenisInstance.scroll;
  if (DARK7_V53_SCROLLER) {
    return ScrollTrigger.getScrollFunc(DARK7_V53_SCROLLER)?.() ?? window.scrollY;
  }
  return window.scrollY;
}

/** True when any pinned ScrollTrigger is currently active. */
export function hasActiveDark7V53Pin() {
  return ScrollTrigger.getAll().some(
    (st) => st.isActive && Boolean(st.vars?.pin || st.pin),
  );
}

/** Merge dark7-v53 scroll defaults into a ScrollTrigger config object. */
export function Dark7V53ScrollTrigger(vars = {}) {
  return {
    scroller: DARK7_V53_SCROLLER,
 ...
 */
function applyDark7V53ScrollerProxy(lenis) {
  if (!DARK7_V53_SCROLLER) return;

  // Lenis always transforms documentElement — "fixed" pins shift downstream triggers.
  const pinType = lenis ? "transform" : DARK7_V53_SCROLLER.style.transform ? "transform" : "fixed";

  ScrollTrigger.scrollerProxy(DARK7_V53_SCROLLER, {
    scrollTop(value) {
      if (arguments.length) {
...
export function initDark7V53LenisScroll(lenis) {
  if (typeof window === "undefined" || !DARK7_V53_SCROLLER) return;

  lenisInstance = lenis;
  layoutSettled = false;

  applyDark7V53ScrollerProxy(lenis);

  const onRefreshInit = () => applyDark7V53ScrollerProxy(lenis);
  ...
  lenis.on("scroll", () => {
    ...
    if (typeof window !== "undefined") {
      window.dispatchEvent(
        new CustomEvent("dark7-v53-scroll", { detail: lenis.scroll }),
      );
    }
  });
...
export function destroyDark7V53LenisScroll(lenis) {
  lenisInstance = null;
  layoutSettled = false;
  removeRefreshInitListener?.();
 ...
 */
export function refreshDark7V53ScrollTriggers(hard = true) {
  if (typeof window === "undefined") return;

  if (hard && layoutSettled && hasActiveDark7V53Pin()) {
    ScrollTrigger.update();
    return;
  }
...
export function markDark7V53ScrollLayoutSettled() {
  layoutSettled = true;
}

export const DARK7_V53_SCROLL_LAYOUT_READY_EVENT = "dark7-v53-scroll-layout-ready";
...
export function notifyScrollLayoutReady() {
  ...
  refreshDark7V53ScrollTriggers(true);
  window.dispatchEvent(new Event(DARK7_V53_SCROLL_LAYOUT_READY_EVENT));

  requestAnimationFrame(() => refreshDark7V53ScrollTriggers(true));
  window.setTimeout(() => refreshDark7V53ScrollTriggers(true), 120);
}
...
export function subscribeAfterScrollLayout(callback) {
  ...
  const run = () => {
    ...
    callback();
    requestAnimationFrame(() => {
      if (!cancelled) refreshDark7V53ScrollTriggers(true);
    });
  };
  ...
  const onReady = () => {
    window.removeEventListener(DARK7_V53_SCROLL_LAYOUT_READY_EVENT, onReady);
    window.clearTimeout(fallbackTimer);
    run();
  };

  window.addEventListener(DARK7_V53_SCROLL_LAYOUT_READY_EVENT, onReady);
...
export const DARK7_V53_HERO_PIN_ID = "dark7-v53-hero-pin";
