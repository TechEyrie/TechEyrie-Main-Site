...
import {
  Dark7V54ScrollTrigger,
  getDark7V54ScrollTop,
  refreshDark7V54ScrollTriggers,
  notifyHeroPinReady,
  DARK7_V54_HERO_PIN_ID,
...
/**
 * v54 — stained-glass volume (Beer-Lambert + faceted normals)
...
function applyFeatherSurfaceShader(
  ...
  material.customProgramCacheKey = () =>
    `feather-stained-glass-volume-v54-${isWing ? 1 : 0}-${heroWingLook ? 1 : 0}-${axis?.axisIdx ?? "b"}-${axis?.acrossIdx ?? "a"}`;
...
float sgFiber(vec2 p, float freq, float ang) {
  ...
  if (uHeroWingLook > 0.5) {
    // v54 — stained emerald glass (Noomo structure, dark-green palette)
...
export default function EagleScrollScene({
  ...
  useLayoutEffect(() => {
    ...
    function createScrollAnimation(triggerEl) {
      ...
      // Initial camera for embedded hero (dark7-v54).
      ...
      ScrollTrigger.getById(DARK7_V54_HERO_PIN_ID)?.kill();
      ...
      gsapCtx = gsap.context(() => {
        ...
        scrollTween = gsap.to(state, {
          progress: 1,
          ease: "none",
          scrollTrigger: Dark7V54ScrollTrigger({
            id: embeddedScroll ? DARK7_V54_HERO_PIN_ID : "eagle-scroll-scene",
            trigger: triggerEl,
            start: "top top",
      ...
      requestAnimationFrame(() => {
        refreshDark7V54ScrollTriggers();
        const progress =
          getDark7V54ScrollTop() < 8 ? 0 : (scrollTween?.scrollTrigger?.progress ?? 0);
        applyScrollProgress(progress);
        onScrollProgressRef.current?.(progress <= 0.001 ? 0 : progress);
      });

      window.setTimeout(() => {
        refreshDark7V54ScrollTriggers(true);
        const progress =
          getDark7V54ScrollTop() < 8 ? 0 : (scrollTween?.scrollTrigger?.progress ?? 0);
        applyScrollProgress(progress);
        onScrollProgressRef.current?.(progress <= 0.001 ? 0 : progress);
