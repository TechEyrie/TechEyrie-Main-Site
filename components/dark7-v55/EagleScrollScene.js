...
import {
  Dark7V55ScrollTrigger,
  getDark7V55ScrollTop,
  refreshDark7V55ScrollTriggers,
  notifyHeroPinReady,
  DARK7_V55_HERO_PIN_ID,
...
/**
 * v55 — Noomo stained-glass (Beer-Lambert + faceted normals)
...
function applyFeatherSurfaceShader(
  ...
  material.customProgramCacheKey = () =>
    `feather-stained-glass-volume-v55a-${isWing ? 1 : 0}-${heroWingLook ? 1 : 0}-${axis?.axisIdx ?? "b"}-${axis?.acrossIdx ?? "a"}`;
...
float sgFiber(vec2 p, float freq, float ang) {
  ...
  if (uHeroWingLook > 0.5) {
    // v55 — Noomo luminous sage-mint glass (thin volume, dark only in overlaps)
    ...
    float heightDark = pow(1.0 - relY, 0.9);
    float heightLite = pow(relY, 0.78);

    vec3 V = normalize(cameraPosition - vFeatherWorldPos);

    float thick = mix(1.05, 0.16, heightLite);
    thick *= mix(1.08, 0.38, tip);
    thick *= mix(1.04, 0.72, along);
    thick *= mix(1.0, 0.8, smoothstep(0.4, 1.05, away));

    vec3 wp = vFeatherWorldPos * vec3(2.15, 1.45, 2.0) + vec3(uMeshSeed * 0.31);
    float inc = sgFbm3(wp);
    float inc2 = sgFbm3(wp * 2.2 + vec3(8.1, leafId * 4.0, 2.2));
    float inclAmt = smoothstep(0.32, 0.8, inc);
    float inclAmt2 = smoothstep(0.42, 0.88, inc2);
    thick *= mix(0.72, 1.28, inclAmt * heightDark);
    thick *= mix(0.9, 1.1, inclAmt2);

    vec2 cUV = vec2(
      along * mix(3.4, 5.2, leafId) + leafId2 * 1.2,
      acrossN * mix(2.4, 3.8, leafId3) + side * 0.4
    );
    cUV = mat2(1.08, 0.22, -0.18, 1.02) * cUV;
    vec4 cr1 = sgCrystal(cUV);
    vec4 cr2 = sgCrystal(cUV * 2.2 + vec2(leafId3 * 2.1, 1.4));
    vec4 cr3 = sgCrystal(cUV * 4.2 + vec2(2.2, along * 1.8));
    float cellAA = 1.0 - smoothstep(0.1, 0.38, fwidth(cUV.x) + fwidth(cUV.y));

    vec3 n = n0;
    n += vec3(cr1.z, cr1.w, 0.0) * 1.05 * cellAA;
    n += vec3(cr2.z, cr2.w, 0.0) * 0.62 * cellAA;
    n += vec3(cr3.z, cr3.w, 0.0) * 0.28 * cellAA;
    n = normalize(n);

    float ndl = clamp(dot(n, lightDir) * 0.5 + 0.5, 0.0, 1.0);
    float viewN = clamp(dot(n, V), 0.0, 1.0);
    float fresnel = pow(1.0 - viewN, 1.35);

    thick *= mix(0.84, 1.22, cr1.y * cellAA);
    thick = clamp(thick, 0.08, 1.85);

    vec2 fUV = vec2(along, acrossN);
    float fib1 = sgFiber(fUV, mix(20.0, 32.0, leafId), mix(0.95, 1.25, leafId2));
    float fib2 = sgFiber(fUV * 1.2 + 0.17, mix(34.0, 48.0, leafId3), mix(1.15, 1.45, leafId));
    float fib3 = sgFiber(fUV * 0.7, mix(11.0, 16.0, leafId2), mix(0.7, 0.95, leafId3));
    float fibZone = smoothstep(0.04, 0.88, away);

    vec3 absDeep = vec3(1.55, 0.28, 0.92);
    vec3 absMid  = vec3(0.82, 0.16, 0.52);
    vec3 absThin = vec3(0.32, 0.07, 0.22);
    vec3 sigma = mix(absDeep, absMid, smoothstep(0.1, 0.52, heightLite));
    sigma = mix(sigma, absThin, smoothstep(0.42, 1.0, heightLite + tip * 0.45));
    sigma *= mix(0.82, 1.28, inclAmt * heightDark);
    vec3 trans = exp(-sigma * thick);

    vec3 gInk  = vec3(0.05, 0.14, 0.09);
    vec3 gGlow = vec3(0.78, 0.96, 0.84);
    vec3 feather = mix(gInk, trans, 0.92);
    feather = mix(feather, vec3(feather.r * 0.62, feather.g, feather.b * 0.74), 0.22);

    vec3 islandDark = vec3(0.06, 0.22, 0.12);
    vec3 islandMid  = vec3(0.22, 0.58, 0.36);
    vec3 islandLite = vec3(0.52, 0.86, 0.62);
    vec3 island = mix(islandDark, islandMid, inclAmt);
    island = mix(island, islandLite, inclAmt2 * (0.5 + heightLite * 0.5));
    feather = mix(feather, island, 0.38 * mix(0.22, 0.78, inclAmt));
    feather = mix(feather, islandDark, (1.0 - inclAmt) * heightDark * 0.16);

    vec3 faceCol = mix(islandMid, islandLite, cr1.y * (0.4 + heightLite));
    feather = mix(feather, faceCol, 0.28 * cellAA);
    feather *= mix(0.78, 1.22, pow(ndl, 1.25));
    feather = mix(feather, gInk, pow(1.0 - ndl, 1.2) * 0.18 * heightDark);

    float seam = pow(1.0 - cr1.x, 1.35) * cellAA;
    float cut = pow(smoothstep(0.22, 0.98, cr1.x), 1.45) * cellAA;
    feather = mix(feather, gInk, seam * 0.22 * heightDark);
    feather = mix(feather, mix(trans, gGlow, 0.55), cut * 0.52 * (0.35 + heightLite * 0.5 + fresnel * 0.3));
    feather = mix(feather, gGlow, pow(smoothstep(0.3, 0.98, cr2.x), 1.5) * cellAA * 0.22 * (0.3 + tip));
    feather = mix(feather, gGlow, pow(smoothstep(0.55, 0.95, cr3.y) * cr3.x, 1.15) * cellAA * 0.28 * (0.3 + fresnel));

    feather = mix(feather, mix(trans, gGlow, 0.4), fib1 * fibZone * 0.16 * (0.4 + heightLite));
    feather = mix(feather, gInk, fib2 * fibZone * 0.12 * heightDark);

    float film = 0.5 + 0.5 * cos(thick * 14.0 + viewN * 7.0 + inc * 8.0 + leafId * 4.0);
    vec3 filmCol = mix(vec3(0.18, 0.48, 0.3), vec3(0.62, 0.92, 0.72), clamp(film, 0.0, 1.0));
    feather = mix(feather, filmCol, fresnel * 0.32 * (0.35 + heightLite * 0.4 + tip * 0.2));

    float rim = pow(max(smoothstep(0.42, 1.05, away), fresnel), 1.2);
    feather = mix(feather, mix(trans, gGlow, 0.7), rim * 0.42 * (0.4 + heightLite * 0.45 + tip * 0.35));
    feather = mix(feather, vec3(0.86, 0.98, 0.9), rim * rim * 0.2 * (0.25 + tip + heightLite));

    feather = mix(feather, gInk, pow(1.0 - ndl0, 1.35) * 0.22 * (0.4 + heightDark));
    feather = mix(feather, mix(trans, gGlow, 0.6), tip * uTipBlendStrength * 0.28);
    feather = mix(feather, gGlow, heightLite * tip * 0.12);

    float luma = dot(feather, vec3(0.299, 0.587, 0.114));
    feather = mix(vec3(luma), feather, 1.22);
    feather *= mix(0.88, 1.16, 0.32 + heightLite * 0.5 + tip * 0.2);
    feather = clamp(feather, 0.0, 1.18);

    gl_FragColor.rgb = mix(gl_FragColor.rgb, feather, uPaintStrength);

    float od = 1.0 - exp(-thick * 1.35);
    float ghost = smoothstep(0.22, 0.72, relY);
    ghost = max(ghost, tip * 0.7 * smoothstep(0.12, 0.62, relY));
    float alpha = mix(0.9, 0.28, ghost) * mix(1.0, od, 0.38);
    alpha *= mix(1.0, 0.78, fresnel * 0.35);
    gl_FragColor.a *= clamp(alpha, 0.22, 1.0);
  } else {
...
export function applyBirdMaterial(bird, textures, lightweightOrOptions = false) {
  ...
  bird.traverse((child) => {
    ...
    const material = new THREE.MeshPhysicalMaterial({
      ...
      emissiveIntensity: useHeroWingLook ? (isBottom ? 0.18 : 0.08) : isWing ? 0.0 : 0.004,
      ...
      roughness: useHeroWingLook ? (isBottom ? 0.07 : 0.14) : isWing ? 0.97 : 0.92,
      metalness: useHeroWingLook ? 0.0 : 0.0,
      clearcoat: useHeroWingLook ? 1.0 : 0.0,
      clearcoatRoughness: useHeroWingLook ? (isBottom ? 0.04 : 0.1) : 1.0,
      reflectivity: useHeroWingLook ? 0.78 : 0.008,
      sheen: useHeroWingLook ? (isBottom ? 0.55 : 0.32) : 0.0,
      sheenRoughness: useHeroWingLook ? 0.18 : 1.0,
      sheenColor: new THREE.Color(swatch.cool ?? swatch.tip),
      specularIntensity: useHeroWingLook ? 1.2 : 0.12,
      transmission: useHeroWingLook ? (isBottom ? 0.52 : 0.24) : 0.0,
      thickness: useHeroWingLook ? (isBottom ? 0.32 : 0.55) : 0,
      ior: useHeroWingLook ? (isBottom ? 1.33 : 1.4) : 1.5,
      transparent: useHeroWingLook,
      opacity: useHeroWingLook ? (isBottom ? 0.62 : 0.88) : 1,
      flatShading: false,
  ...
  materials.forEach((mat, index) => {
    ...
    mat.roughness = isHeroWingLook
      ? (isBottomFeather
        ...
        : THREE.MathUtils.lerp(0.16, 0.12, t))
    ...
    if (isHeroWingLook && "transmission" in mat) {
      ...
      mat.opacity = isBottomFeather
        ? THREE.MathUtils.lerp(0.58, 0.64, t)
        : THREE.MathUtils.lerp(0.84, 0.9, t);
      mat.thickness = isBottomFeather
        ? THREE.MathUtils.lerp(0.28, 0.34, t)
        : THREE.MathUtils.lerp(0.48, 0.58, t);
      mat.ior = isBottomFeather ? 1.33 : 1.4;
...
export default function EagleScrollScene({
  ...
  useLayoutEffect(() => {
    ...
    function createScrollAnimation(triggerEl) {
      ...
      // Initial camera for embedded hero (dark7-v55).
      ...
      ScrollTrigger.getById(DARK7_V55_HERO_PIN_ID)?.kill();
      ...
      gsapCtx = gsap.context(() => {
        ...
        scrollTween = gsap.to(state, {
          progress: 1,
          ease: "none",
          scrollTrigger: Dark7V55ScrollTrigger({
            id: embeddedScroll ? DARK7_V55_HERO_PIN_ID : "eagle-scroll-scene",
            trigger: triggerEl,
            start: "top top",
      ...
      requestAnimationFrame(() => {
        refreshDark7V55ScrollTriggers();
        const progress =
          getDark7V55ScrollTop() < 8 ? 0 : (scrollTween?.scrollTrigger?.progress ?? 0);
        applyScrollProgress(progress);
        onScrollProgressRef.current?.(progress <= 0.001 ? 0 : progress);
      });

      window.setTimeout(() => {
        refreshDark7V55ScrollTriggers(true);
        const progress =
          getDark7V55ScrollTop() < 8 ? 0 : (scrollTween?.scrollTrigger?.progress ?? 0);
        applyScrollProgress(progress);
        onScrollProgressRef.current?.(progress <= 0.001 ? 0 : progress);
