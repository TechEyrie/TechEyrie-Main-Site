...
/** Lenis + ScrollTrigger scroller for all dark7-v55 sections. */
export const DARK7_V55_SCROLLER =
...
export function getDark7V55ScrollTop() {
  if (lenisInstance) return lenisInstance.scroll;
  if (DARK7_V55_SCROLLER) {
    return ScrollTrigger.getScrollFunc(DARK7_V55_SCROLLER)?.() ?? window.scrollY;
  }
  return window.scrollY;
}

/** True when any pinned ScrollTrigger is currently active. */
export function hasActiveDark7V55Pin() {
  return ScrollTrigger.getAll().some(
    (st) => st.isActive && Boolean(st.vars?.pin || st.pin),
  );
}

/** Merge dark7-v55 scroll defaults into a ScrollTrigger config object. */
export function Dark7V55ScrollTrigger(vars = {}) {
  return {
    scroller: DARK7_V55_SCROLLER,
 ...
 */
function applyDark7V55ScrollerProxy(lenis) {
  if (!DARK7_V55_SCROLLER) return;

  // Lenis always transforms documentElement — "fixed" pins shift downstream triggers.
  const pinType = lenis ? "transform" : DARK7_V55_SCROLLER.style.transform ? "transform" : "fixed";

  ScrollTrigger.scrollerProxy(DARK7_V55_SCROLLER, {
    scrollTop(value) {
      if (arguments.length) {
...
export function initDark7V55LenisScroll(lenis) {
  ...
  const onRefreshInit = () => applyDark7V55ScrollerProxy(lenis);
  ...
  lenis.on("scroll", () => {
    ...
    if (typeof window !== "undefined") {
      window.dispatchEvent(
        new CustomEvent("dark7-v55-scroll", { detail: lenis.scroll }),
      );
    }
  });
 ...
 */
export function refreshDark7V55ScrollTriggers(hard = true) {
  if (typeof window === "undefined") return;

  if (hard && layoutSettled && hasActiveDark7V55Pin()) {
    ScrollTrigger.update();
    return;
  }
...
export function markDark7V55ScrollLayoutSettled() {
  layoutSettled = true;
}

export const DARK7_V55_SCROLL_LAYOUT_READY_EVENT = "dark7-v55-scroll-layout-ready";
...
export function notifyScrollLayoutReady() {
  ...
  refreshDark7V55ScrollTriggers(true);
  window.dispatchEvent(new Event(DARK7_V55_SCROLL_LAYOUT_READY_EVENT));

  requestAnimationFrame(() => refreshDark7V55ScrollTriggers(true));
  window.setTimeout(() => refreshDark7V55ScrollTriggers(true), 120);
}
...
export function subscribeAfterScrollLayout(callback) {
  ...
  const run = () => {
    ...
    callback();
    requestAnimationFrame(() => {
      if (!cancelled) refreshDark7V55ScrollTriggers(true);
    });
  };
  ...
  const onReady = () => {
    window.removeEventListener(DARK7_V55_SCROLL_LAYOUT_READY_EVENT, onReady);
    window.clearTimeout(fallbackTimer);
    run();
  };

  window.addEventListener(DARK7_V55_SCROLL_LAYOUT_READY_EVENT, onReady);
...
export const DARK7_V55_HERO_PIN_ID = "dark7-v55-hero-pin";
