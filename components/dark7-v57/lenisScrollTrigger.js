...
/** Lenis + ScrollTrigger scroller for all dark7-v57 sections. */
export const DARK7_V57_SCROLLER =
...
export function getDark7V57ScrollTop() {
  if (lenisInstance) return lenisInstance.scroll;
  if (DARK7_V57_SCROLLER) {
    return ScrollTrigger.getScrollFunc(DARK7_V57_SCROLLER)?.() ?? window.scrollY;
  }
  return window.scrollY;
}

/** True when any pinned ScrollTrigger is currently active. */
export function hasActiveDark7V57Pin() {
  return ScrollTrigger.getAll().some(
    (st) => st.isActive && Boolean(st.vars?.pin || st.pin),
  );
}

/** Merge dark7-v57 scroll defaults into a ScrollTrigger config object. */
export function Dark7V57ScrollTrigger(vars = {}) {
  return {
    scroller: DARK7_V57_SCROLLER,
 ...
 */
function applyDark7V57ScrollerProxy(lenis) {
  if (!DARK7_V57_SCROLLER) return;

  // Lenis always transforms documentElement — "fixed" pins shift downstream triggers.
  const pinType = lenis ? "transform" : DARK7_V57_SCROLLER.style.transform ? "transform" : "fixed";

  ScrollTrigger.scrollerProxy(DARK7_V57_SCROLLER, {
    scrollTop(value) {
      if (arguments.length) {
...
export function initDark7V57LenisScroll(lenis) {
  if (typeof window === "undefined" || !DARK7_V57_SCROLLER) return;

  lenisInstance = lenis;
  layoutSettled = false;

  applyDark7V57ScrollerProxy(lenis);

  const onRefreshInit = () => applyDark7V57ScrollerProxy(lenis);
  ...
  lenis.on("scroll", () => {
    ...
    if (typeof window !== "undefined") {
      window.dispatchEvent(
        new CustomEvent("dark7-v57-scroll", { detail: lenis.scroll }),
      );
    }
  });
...
export function destroyDark7V57LenisScroll(lenis) {
  lenisInstance = null;
  layoutSettled = false;
  removeRefreshInitListener?.();
 ...
 */
export function refreshDark7V57ScrollTriggers(hard = true) {
  if (typeof window === "undefined") return;

  if (hard && layoutSettled && hasActiveDark7V57Pin()) {
    ScrollTrigger.update();
    return;
  }
...
export function markDark7V57ScrollLayoutSettled() {
  layoutSettled = true;
}

export const DARK7_V57_SCROLL_LAYOUT_READY_EVENT = "dark7-v57-scroll-layout-ready";
...
export function notifyScrollLayoutReady() {
  ...
  refreshDark7V57ScrollTriggers(true);
  window.dispatchEvent(new Event(DARK7_V57_SCROLL_LAYOUT_READY_EVENT));

  requestAnimationFrame(() => refreshDark7V57ScrollTriggers(true));
  window.setTimeout(() => refreshDark7V57ScrollTriggers(true), 120);
}
...
export function subscribeAfterScrollLayout(callback) {
  ...
  const run = () => {
    ...
    callback();
    requestAnimationFrame(() => {
      if (!cancelled) refreshDark7V57ScrollTriggers(true);
    });
  };
  ...
  const onReady = () => {
    window.removeEventListener(DARK7_V57_SCROLL_LAYOUT_READY_EVENT, onReady);
    window.clearTimeout(fallbackTimer);
    run();
  };

  window.addEventListener(DARK7_V57_SCROLL_LAYOUT_READY_EVENT, onReady);
...
export const DARK7_V57_HERO_PIN_ID = "dark7-v57-hero-pin";
