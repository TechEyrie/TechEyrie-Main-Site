...
function TestimonialArrowButton({ name }) {
  ...
  return (
    <button
      type="button"
      className="testimonials-arrow-btn hero-cta-btn inline-flex flex-shrink-0 cursor-pointer items-center justify-center p-2.5 transition-colors duration-300 sm:p-3"
      style={{
        backgroundColor: hovered ? "#162D24" : "#F7F3F033",
        borderRadius: "12px",
      }}
    ...
    >
      <ArrowRight className="h-4 w-4 text-[#F7F3F0] sm:h-[18px] sm:w-[18px] md:h-5 md:w-5" strokeWidth={1.8} />
    </button>
  );
}
    ...
    const unsubLayout = subscribeAfterScrollLayout(() => {
      ...
      headingTrigger = ScrollTrigger.create(
        Dark7V6ScrollTrigger({
          ...
          onUpdate: (self) => {
            const opacity = Math.abs(self.progress - 0.5) * 2;
            gsap.set(heading, { opacity });
          },
        }),
      );
