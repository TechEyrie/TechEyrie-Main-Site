...
/** Lenis + ScrollTrigger scroller for all dark7-v60 sections. */
export const DARK7_V60_SCROLLER =
...
export function getDark7V60ScrollTop() {
  if (lenisInstance) return lenisInstance.scroll;
  if (DARK7_V60_SCROLLER) {
    return ScrollTrigger.getScrollFunc(DARK7_V60_SCROLLER)?.() ?? window.scrollY;
  }
  return window.scrollY;
}

/** True when any pinned ScrollTrigger is currently active. */
export function hasActiveDark7V60Pin() {
  return ScrollTrigger.getAll().some(
    (st) => st.isActive && Boolean(st.vars?.pin || st.pin),
  );
}

/** Merge dark7-v60 scroll defaults into a ScrollTrigger config object. */
export function Dark7V60ScrollTrigger(vars = {}) {
  return {
    scroller: DARK7_V60_SCROLLER,
 ...
 */
function applyDark7V60ScrollerProxy(lenis) {
  if (!DARK7_V60_SCROLLER) return;

  // Lenis always transforms documentElement — "fixed" pins shift downstream triggers.
  const pinType = lenis ? "transform" : DARK7_V60_SCROLLER.style.transform ? "transform" : "fixed";

  ScrollTrigger.scrollerProxy(DARK7_V60_SCROLLER, {
    scrollTop(value) {
      if (arguments.length) {
...
export function initDark7V60LenisScroll(lenis) {
  if (typeof window === "undefined" || !DARK7_V60_SCROLLER) return;

  lenisInstance = lenis;
  layoutSettled = false;

  applyDark7V60ScrollerProxy(lenis);

  const onRefreshInit = () => applyDark7V60ScrollerProxy(lenis);
  ...
  lenis.on("scroll", () => {
    ...
    if (typeof window !== "undefined") {
      window.dispatchEvent(
        new CustomEvent("dark7-v60-scroll", { detail: lenis.scroll }),
      );
    }
  });
...
export function destroyDark7V60LenisScroll(lenis) {
  lenisInstance = null;
  layoutSettled = false;
  removeRefreshInitListener?.();
 ...
 */
export function refreshDark7V60ScrollTriggers(hard = true) {
  if (typeof window === "undefined") return;

  if (hard && layoutSettled && hasActiveDark7V60Pin()) {
    ScrollTrigger.update();
    return;
  }
...
export function markDark7V60ScrollLayoutSettled() {
  layoutSettled = true;
}

export const DARK7_V60_SCROLL_LAYOUT_READY_EVENT = "dark7-v60-scroll-layout-ready";
...
export function notifyScrollLayoutReady() {
  ...
  refreshDark7V60ScrollTriggers(true);
  window.dispatchEvent(new Event(DARK7_V60_SCROLL_LAYOUT_READY_EVENT));

  requestAnimationFrame(() => refreshDark7V60ScrollTriggers(true));
  window.setTimeout(() => refreshDark7V60ScrollTriggers(true), 120);
}
...
export function subscribeAfterScrollLayout(callback) {
  ...
  const run = () => {
    ...
    callback();
    requestAnimationFrame(() => {
      if (!cancelled) refreshDark7V60ScrollTriggers(true);
    });
  };
  ...
  const onReady = () => {
    window.removeEventListener(DARK7_V60_SCROLL_LAYOUT_READY_EVENT, onReady);
    window.clearTimeout(fallbackTimer);
    run();
  };

  window.addEventListener(DARK7_V60_SCROLL_LAYOUT_READY_EVENT, onReady);
...
export const DARK7_V60_HERO_PIN_ID = "dark7-v60-hero-pin";
