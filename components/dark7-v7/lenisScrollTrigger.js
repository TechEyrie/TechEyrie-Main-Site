...
/** Lenis + ScrollTrigger scroller for all dark7-v7 sections. */
export const DARK7_V7_SCROLLER =
...
export function Dark7V7ScrollTrigger(vars = {}) {
  return {
 ...
 */
function applyDark7V7ScrollerProxy(lenis) {
  if (!DARK7_V7_SCROLLER) return;

  // Lenis always transforms documentElement — "fixed" pins shift downstream triggers.
  const pinType = lenis ? "transform" : DARK7_V7_SCROLLER.style.transform ? "transform" : "fixed";
...
export function initDark7V7LenisScroll(lenis) {
  ...
  const onRefreshInit = () => applyDark7V7ScrollerProxy(lenis);
  ...
  lenis.on("scroll", () => {
    ...
    if (typeof window !== "undefined") {
      window.dispatchEvent(
        new CustomEvent("dark7-v7-scroll", { detail: lenis.scroll }),
      );
    }
  });
...
export function notifyScrollLayoutReady() {
  ...
  refreshDark7V7ScrollTriggers(true);
  window.dispatchEvent(new Event(DARK7_V7_SCROLL_LAYOUT_READY_EVENT));

  requestAnimationFrame(() => refreshDark7V7ScrollTriggers(true));
  window.setTimeout(() => refreshDark7V7ScrollTriggers(true), 120);
}
...
export function subscribeAfterScrollLayout(callback) {
  ...
  const onReady = () => {
    window.removeEventListener(DARK7_V7_SCROLL_LAYOUT_READY_EVENT, onReady);
    window.clearTimeout(fallbackTimer);
    run();
  };

  window.addEventListener(DARK7_V7_SCROLL_LAYOUT_READY_EVENT, onReady);
...
export const DARK7_V7_HERO_PIN_ID = "dark7-v7-hero-pin";
