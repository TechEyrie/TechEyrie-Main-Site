...
export default function AirvoirSection({ theme = "light" }) {
  ...
  const bgStyle =
    theme === "dark"
      ? {
          background:
            "radial-gradient(ellipse at 15% 20%, #3a6b58 0%, #2a5544 42%, #1e3a2f 100%)",
        }
    ...
    >
      {theme === "dark" && (
        <>
          ...
          />
          {/* Very soft teal at the top so its top color matches TE2 bottom but stays light */}
          <div
            className="absolute inset-x-0 top-0 h-28 sm:h-32 md:h-40 pointer-events-none z-[2]"
            style={{
              background:
                "linear-gradient(to bottom, rgba(45,82,68,0.45) 0%, rgba(45,82,68,0) 100%)",
            }}
          />
          {/* Keep only a bottom blend so the rest of the section eases out */}
          <div
            className="absolute inset-x-0 bottom-0 h-28 sm:h-36 md:h-44 pointer-events-none z-[2]"
            style={{
              background:
                "linear-gradient(to top, rgba(26,50,40,0.7) 0%, rgba(26,50,40,0) 100%)",
            }}
          />
        </>
      )}
