...
export default function Footer({ theme = 'light' }) {
  ...
  const footerBg = theme === 'dark' ? '#1a3228' : lightColors.background;
  const cardBg = theme === 'dark' ? '#183228' : lightColors.tertiary;
  const textColor = theme === 'dark' ? '#FAF8F4' : lightColors.text;
  const secondaryTextColor = theme === 'dark' ? '#c8c2ad' : '#444444';
  const borderColor = theme === 'dark' ? 'rgba(201, 187, 168, 0.24)' : 'rgba(0, 0, 0, 0.1)';

  const bgPatternStyle = theme === 'dark'
    ? {
        backgroundImage: `
      linear-gradient(135deg, rgba(17, 20, 19, 0.95) 0%, rgba(20, 24, 22, 0.95) 100%),
      radial-gradient(circle at 15% 15%, rgba(201, 187, 168, 0.08), transparent 35%),
      radial-gradient(circle at 85% 85%, rgba(107, 155, 122, 0.06), transparent 40%)
    `,
  ...
  return (
    <>
      <style jsx global>{`
        .dark5-footer {
          background-color: #1a3228 !important;
        }

        .dark5-footer-card {
          background-color: #183228 !important;
          border-color: rgba(201, 187, 168, 0.24) !important;
        }
        ...
        .social-icon-animate:hover {
          ...
          box-shadow: 0 8px 25px rgba(201, 187, 168, 0.35);
        }
...
function SocialIcon({ href, label, type, theme = 'light' }) {
  const socialBg = theme === 'dark' ? '#b8a88a' : '#181818';
  const socialColor = theme === 'dark' ? '#1a3228' : '#FFFFFF';
