...
export default function Footer({ theme = 'light' }) {
  ...
  const footerBg = theme === 'dark' ? '#1e3a2f' : lightColors.background;
  const cardBg = theme === 'dark' ? '#1a382c' : lightColors.tertiary;
  const textColor = theme === 'dark' ? '#FAF8F4' : lightColors.text;
  const secondaryTextColor = theme === 'dark' ? '#c8c2ad' : '#444444';
  const borderColor = theme === 'dark' ? 'rgba(201, 187, 168, 0.2)' : 'rgba(0, 0, 0, 0.1)';

  const bgPatternStyle = theme === 'dark'
    ? {
        backgroundImage: `
      linear-gradient(135deg, rgba(30, 58, 46, 0.96) 0%, rgba(42, 85, 68, 0.92) 100%),
      radial-gradient(circle at 15% 15%, rgba(212, 184, 118, 0.12), transparent 38%),
      radial-gradient(circle at 85% 85%, rgba(155, 196, 166, 0.14), transparent 42%)
    `,
    ...
    <>
      <style jsx global>{`
        ...
        .dark5-footer-card {
          ...
          border-color: rgba(212, 184, 118, 0.3) !important;
        }
