  ...
  const bgStyle = useMemo(
    () =>
      theme === "dark"
        ? {
            background:
              "linear-gradient(135deg, #1a3228 0%, #254636 18%, #2d5244 55%, #1a3228 100%)",
          }
      ...
      {theme === "dark" && !sharedBackground && (
        <>
          <div
            ...
            style={{ background: "linear-gradient(to bottom, #1a3228 0%, rgba(26,50,40,0) 100%)" }}
          />
          <div
            className="absolute inset-x-0 bottom-0 h-32 sm:h-40 md:h-48 pointer-events-none z-[1]"
            style={{ background: "linear-gradient(to top, rgba(45,82,68,0.9) 0%, rgba(45,82,68,0) 100%)" }}
          />
        </>
      )}
        ...
        {theme === "dark" && !prefersReducedMotion && (
          ...
          >
            ...
            <div className="absolute inset-0" style={{
              background: "radial-gradient(ellipse 120% 90% at 60% 85%, rgba(142, 162, 88, 0.46) 0%, rgba(52, 102, 82, 0.34) 42%, rgba(17, 20, 19, 0) 72%)",
