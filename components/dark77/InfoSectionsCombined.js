...
import { dark77MainSurfaceStyle, dark77OrbGradient } from "./dark77PageSurface";
...
export default function InfoSectionsCombined({ theme = "light" }) {
  ...
  const bgStyle = useMemo(
    () => (theme === "dark" ? dark77MainSurfaceStyle : { backgroundColor: "#F9F7F0" }),
    [theme],
  );
      ...
      {theme === "dark" && (
        <>
            ...
            style={{
              ...
              backgroundImage: `
                conic-gradient(
                  ...
                  rgba(242,235,221,0.22) 18%,
                  rgba(107,155,122,0.2) 42%,
                  rgba(242,235,221,0) 72%,
                  rgba(26,50,40,0) 100%
                ),
                radial-gradient(
                  ellipse at 50% 50%,
                  rgba(242,235,221,0.16) 0%,
                  rgba(107,155,122,0.12) 40%,
                  rgba(26,50,40,0) 70%
                )
              `,
              mixBlendMode: "screen",
          ...
          {orbDefs.map((def, i) => (
            <div
              ...
              style={{
                ...
                height: `${def.size}px`,
                transform: "translate(-50%, -50%)",
                backgroundImage: dark77OrbGradient,
                opacity: 0.02,
                filter: "blur(18px)",
                boxShadow: "0 0 140px rgba(250,244,234,0.18)",
              }}
            />
          ))}
          ...
          <div
            ...
            style={{
              background: "linear-gradient(to bottom, rgba(30,58,46,0.85) 0%, rgba(30,58,46,0) 100%)",
            }}
          />
          <div
            className="absolute inset-x-0 bottom-0 h-24 pointer-events-none z-[4]"
            style={{
              background: "linear-gradient(to top, rgba(30,58,46,0.85) 0%, rgba(30,58,46,0) 100%)",
            }}
          />
        </>
      )}
