...
import { dark77MainSurfaceStyle } from './dark77PageSurface';

gsap.registerPlugin(ScrollTrigger);

export default function TalkToExpertSection({ theme = 'light', dark7 = false }) {
  const sectionRef = useRef(null);
  const leftTextRef = useRef(null);
  const rightTextRef = useRef(null);

  // dark7: luxury green surface. light theme = black band; dark theme = light gray band
  const bgStyle = dark7
    ? dark77MainSurfaceStyle
    : theme === 'light'
  ...
  return (
      <section
        ref={sectionRef}
        className={`relative overflow-hidden py-12 sm:py-16 md:py-20 lg:py-24 xl:py-28 2xl:py-32${dark7 ? " talk-expert-dark7" : ""}`}
        style={bgStyle}
      >
        ...
        {/* Subtle background pattern for light / dark7 */}
        {(theme === 'light' || dark7) && (
          <div className="pointer-events-none absolute inset-0 opacity-30">
            ...
            >
              <defs>
                ...
                >
                  <circle cx="20" cy="20" r="1" fill="#E8DCC8" opacity="0.1" />
                </pattern>
              </defs>
        ...
        <div className="pointer-events-none absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 opacity-[0.03]">
          <svg width="600" height="600" viewBox="0 0 600 600" fill="none" className="w-full max-w-[600px]">
            <path
              ...
              stroke={dark7 || theme === 'light' ? '#E8DCC8' : '#111111'}
              strokeWidth="30"
              strokeLinecap="round"
        ...
        <div className="pointer-events-none absolute bottom-16 right-12 hidden lg:block">
          <svg width="110" height="110" viewBox="0 0 110 110" fill="none">
            <rect x="0" y="0" width="48" height="48" rx="8" fill={theme === 'light' || dark7 ? '#4A4A4A' : '#E8E8E8'} />
            <rect x="62" y="0" width="48" height="48" rx="8" fill="#E8DCC8" />
          </svg>
        </div>
        ...
        <div className="pointer-events-none absolute bottom-24 left-12 hidden lg:block">
          <svg width="70" height="70" viewBox="0 0 70 70" fill="none">
            <rect x="0" y="0" width="70" height="70" rx="8" fill="#E8DCC8" />
          </svg>
        </div>
        ...
        <div className="pointer-events-none absolute top-20 left-20 hidden xl:block">
          <svg width="50" height="50" viewBox="0 0 50 50" fill="none">
            <rect x="0" y="0" width="50" height="50" rx="6" fill={theme === 'light' || dark7 ? '#4A4A4A' : '#E8E8E8'} />
          </svg>
        </div>
        ...
        <div className="relative z-10 mx-auto max-w-[1800px] px-4 sm:px-6 md:px-8">
          ...
          <div className="flex flex-col items-center justify-center gap-6 sm:gap-8 md:gap-10 lg:flex-row lg:gap-12 xl:gap-20">
            ...
            >
              <h2 className={`font-italiana font-light text-[24px] sm:text-[32px] md:text-[40px] lg:text-[56px] xl:text-[64px] 2xl:text-[72px] leading-[1.05] tracking-[-0.02em] ${dark7 ? 'text-[#f3f3f3]' : theme === 'light' ? 'text-white' : 'text-[#111111]'}`}>
                Talk to
              </h2>
            </div>
            ...
            <div className="relative flex-shrink-0 my-6 sm:my-8 lg:my-0">
              {/* Back phone frame */}
              <div 
                className={`absolute -right-3 -top-2 sm:-right-4 sm:-top-3 md:-right-5 md:-top-4 lg:-right-6 lg:-top-4 z-0 h-[280px] w-[160px] sm:h-[340px] sm:w-[200px] md:h-[400px] md:w-[240px] lg:h-[460px] lg:w-[260px] xl:h-[500px] xl:w-[280px] 2xl:h-[520px] 2xl:w-[300px] rounded-[24px] sm:rounded-[28px] md:rounded-[32px] lg:rounded-[36px] border-[2px] sm:border-[2.5px] md:border-[3px] p-2 sm:p-2.5 md:p-3 ${theme === 'light' || dark7 ? 'border-white/20 bg-[#1A1A1A] shadow-[0_20px_50px_rgba(0,0,0,0.5)]' : 'border-black/10 bg-white shadow-[0_20px_50px_rgba(0,0,0,0.15)]'}`}
                style={{ opacity: 0.4 }}
              >
                <div className={`relative h-full w-full overflow-hidden rounded-[18px] sm:rounded-[22px] md:rounded-[24px] lg:rounded-[28px] ${theme === 'light' || dark7 ? 'bg-gradient-to-b from-[#3A3A3A] to-[#2A2A2A]' : 'bg-gradient-to-b from-gray-100 to-gray-200'}`}>
                ...
                {/* Top notch */}
                <div className={`absolute left-1/2 top-2 sm:top-3 md:top-4 z-20 h-3 w-12 sm:h-4 sm:w-16 md:h-5 md:w-20 -translate-x-1/2 rounded-full ${theme === 'light' || dark7 ? 'bg-[#1A1A1A]' : 'bg-white'}`} />
              </div>

              {/* Front phone frame */}
              <div 
                className={`relative z-10 h-[280px] w-[160px] sm:h-[340px] sm:w-[200px] md:h-[400px] md:w-[240px] lg:h-[460px] lg:w-[260px] xl:h-[500px] xl:w-[280px] 2xl:h-[520px] 2xl:w-[300px] rounded-[24px] sm:rounded-[28px] md:rounded-[32px] lg:rounded-[36px] border-[2px] sm:border-[2.5px] md:border-[3px] p-2 sm:p-2.5 md:p-3 ${theme === 'light' || dark7 ? 'border-white/30 bg-[#1A1A1A] shadow-[0_35px_90px_rgba(0,0,0,0.6)]' : 'border-black/10 bg-white shadow-[0_25px_60px_rgba(0,0,0,0.2)]'}`}
              >
                <div className={`relative h-full w-full overflow-hidden rounded-[18px] sm:rounded-[22px] md:rounded-[24px] lg:rounded-[28px] ${theme === 'light' || dark7 ? 'bg-gradient-to-b from-[#3A3A3A] to-[#2A2A2A]' : 'bg-gradient-to-b from-gray-100 to-gray-200'}`}>
                  {/* Expert image */}
                  <div className="relative h-full w-full">
                  ...
                  <div className="absolute bottom-3 sm:bottom-4 md:bottom-6 left-1/2 z-10 -translate-x-1/2">
                    <Link
                      ...
                      className="group flex h-10 w-10 sm:h-12 sm:w-12 md:h-14 md:w-14 lg:h-16 lg:w-16 items-center justify-center rounded-lg sm:rounded-xl md:rounded-2xl bg-[#E8DCC8] shadow-[0_8px_25px_rgba(242,235,221,0.4)] transition-all duration-300 hover:scale-110 hover:shadow-[0_12px_35px_rgba(242,235,221,0.6)] active:scale-95"
                    >
                      <svg
                        width="16"
                        height="16"
              ...
              <div className={`absolute inset-0 -z-10 scale-110 rounded-[24px] sm:rounded-[28px] md:rounded-[32px] lg:rounded-[36px] ${theme === 'light' || dark7 ? 'bg-[#E8DCC8]/8' : 'bg-[#E8DCC8]/5'} blur-2xl`} />
            </div>
            ...
            >
              <h2 className={`font-playfair italic font-light text-[24px] sm:text-[32px] md:text-[40px] lg:text-[56px] xl:text-[64px] 2xl:text-[72px] leading-[1.05] tracking-[-0.02em] ${dark7 ? 'text-[#e8e4dc]' : theme === 'light' ? 'text-white' : 'text-[#111111]'}`}>
                an expert
              </h2>
            </div>
          </div>
