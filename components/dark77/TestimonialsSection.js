...
import { dark77CreamBandGradient } from './dark77PageSurface';
...
export default function TestimonialsSection({ theme, sharedBackground = false }) {
  ...
  const bgColorStyle = isDark
    ? sharedBackground
      ...
      : { background: dark77CreamBandGradient }
  ...
  const cardBgStyle = isDark
    ? { backgroundColor: '#FFFCF7', color: '#152820' }
    : { backgroundColor: lightColors.tertiary };
  const cardText = isDark ? 'text-[#1a2820]' : 'text-slate-800';
  const cardShadow = isDark ? 'testimonial-lux-card shadow-xl' : 'shadow-[0_8px_30px_rgb(0,0,0,0.04)]';



  return (
    <div
      className={`relative min-h-[150vh] ${textColor} selection:bg-indigo-500/30 transition-colors duration-500 ${bgColor} ${isDark ? 'testimonials-lux-section' : ''}`}
      style={bgColorStyle}
    >
        {isDark && !sharedBackground && (
          <>
            <div
              className="absolute inset-x-0 top-0 h-20 sm:h-24 pointer-events-none z-[2]"
              style={{
                background:
            ...
            />
            <div
              className="absolute inset-x-0 bottom-0 h-20 sm:h-24 pointer-events-none z-[2]"
              style={{
                background:
                  "linear-gradient(to top, #F8F3E8 0%, rgba(248,243,232,0) 100%)",
              }}
            />
          </>
        )}
