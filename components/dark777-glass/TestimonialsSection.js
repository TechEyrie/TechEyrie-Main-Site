...
export default function TestimonialsSection({ theme, sharedBackground = false }) {
  ...
  const bgColorStyle = isDark
    ? sharedBackground
      ...
      : {
          // Start and end with the shared deep green so the
          // testimonial band blends cleanly into neighboring sections.
          background:
  ...
  return (
    <div
      className={`testimonials-section-dark relative min-h-[150vh] ${textColor} selection:bg-indigo-500/30 transition-colors duration-500 ${bgColor}`}
      style={bgColorStyle}
    >
        {isDark && !sharedBackground && (
          <>
            {/* Soften top/bottom joins against neighboring dark sections */}
            <div
