...
function ExpertiseDetailTemplate({ slug, theme = "dark" }) {
  const data = useMemo(() => getExpertiseDetailData(slug), [slug]);
    ...
    const measureHeights = () => {
      setFaqHeights((prev) => {
        const next = Array.from(
          { length: faqCount },
          (_, idx) => faqContentRefs.current[idx]?.scrollHeight || 0
        );
        if (
          prev.length === next.length &&
          next.every((height, idx) => height === prev[idx])
        ) {
          return prev;
        }
        return next;
    ...
    >
      ...
      <section className="relative z-10 px-6 md:px-10 lg:px-16 pt-24 md:pt-32 pb-14 md:pb-16">
        <div className="max-w-[1700px] mx-auto">
          <nav className="d7-reveal exp-breadcrumb exp-mint font-merriweather text-[12px] md:text-[13px] uppercase tracking-[0.14em] mb-8">
            <Link href="/expertise" className="hover:opacity-90 transition-colors cursor-pointer">
              Expertise
            </Link>
            <span className="mx-2 opacity-50">/</span>
            <span className="opacity-85">{data.shortTitle}</span>
          </nav>

          <div className="grid lg:grid-cols-[62%_38%] gap-10 items-center">
            <div className="d7-reveal">
              <p className="exp-light font-merriweather uppercase tracking-[0.16em] text-[13px] md:text-[15px] font-semibold mb-7">
                Expertise
              </p>
              ...
              <Link href="/contact" className="inline-flex items-center gap-3 group mt-8 cursor-pointer">
                <span className="exp-light font-[Helvetica_Now_Text,Helvetica,Arial,sans-serif] text-[16px] md:text-[20px] font-bold tracking-tight">
                  Talk to an Expert
                </span>
      ...
      <section className="relative z-10 px-6 md:px-10 lg:px-16 py-16 md:py-20 lg:py-24">
        <div className="max-w-[1700px] mx-auto">
          <div className="d7-reveal mb-10 md:mb-14 flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
            <h3 className={`font-italiana text-[44px] sm:text-[52px] md:text-[72px] lg:text-[84px] leading-[0.98] exp-light`}>Key Offerings</h3>
            <span className={`font-merriweather text-[13px] md:text-[15px] lg:text-[16px] uppercase tracking-[0.16em] shrink-0 ${isDark ? "exp-mint" : "text-[#1f614d]"}`}>
              Built for impact
            </span>
          </div>
      ...
      <section className="relative z-10 px-6 md:px-10 lg:px-16 py-20 md:py-24 lg:py-28 overflow-hidden">
        ...
        <div className="max-w-[1700px] mx-auto relative">
          <div className="d7-reveal flex flex-col lg:flex-row lg:items-end lg:justify-between gap-6 lg:gap-10 mb-10 md:mb-14">
            <div>
              <p className="font-merriweather text-[12px] md:text-[13px] uppercase tracking-[0.18em] exp-mint mb-4">
                Stack &amp; toolchain
              </p>
              <h3 className={`font-italiana text-[42px] md:text-[72px] lg:text-[86px] leading-[0.95] exp-light`}>
                Technologies &amp; Tools
              </h3>
            </div>
          ...
          >
            ...
            <div className="relative">
              {(() => {
                ...
                const coreLabel = data.techCoreLabel || "Core stack";
                const platformLabel = data.techPlatformLabel || "Cloud & ops";
                return (
                  <>
                    <div className="mb-10 md:mb-12">
                      <p className="font-merriweather text-[11px] md:text-[12px] uppercase tracking-[0.2em] exp-mint mb-4 md:mb-5">
                        {coreLabel}
                      </p>
          ...
          <div className="grid lg:grid-cols-[58%_42%] gap-6 md:gap-8">
            <article className={`d7-reveal relative rounded-3xl p-8 md:p-10 lg:p-12 overflow-hidden flex flex-col min-h-0 ${cardTheme}`}>
              ...
              <div className="relative z-[1] flex-1 flex flex-col gap-6 md:gap-8">
                ...
                <div>
                  <p className="font-merriweather text-[11px] md:text-[12px] uppercase tracking-[0.2em] exp-mint mb-3">
                ...
                <div className={`rounded-2xl p-4 md:p-5 mt-auto border ${isDark ? "border-[#74F5A1]/20 bg-[#0d241d]/55" : "border-[#d8d1c4] bg-[#faf7f1]"}`}>
                  ...
                  <p className="font-merriweather text-[11px] md:text-[12px] uppercase tracking-[0.14em] exp-mint mt-3">
      ...
      <section className="relative z-10 px-6 md:px-10 lg:px-16 py-14">
        <div className="max-w-[1700px] mx-auto">
          <h3 className={`d7-reveal font-italiana text-[36px] md:text-[58px] mb-8 exp-light`}>FAQ</h3>
          <div className="space-y-3">
            {data.faqs.map((faq, idx) => {
              const isOpen = idx === openFaq;
              return (
                <div key={faq.q} className={`d7-reveal rounded-xl overflow-hidden ${cardTheme}`}>
                  <button type="button" className="w-full text-left p-5 flex items-center justify-between transition-colors duration-300 hover:bg-white/[0.03] cursor-pointer" onClick={() => setOpenFaq(isOpen ? -1 : idx)}>
                    <span className="font-merriweather text-[14px] md:text-[15px]">{faq.q}</span>
                    <span className={`exp-mint text-xl transition-transform duration-300 ${isOpen ? "rotate-45" : "rotate-0"}`}>+</span>
                  </button>
                  <div
      ...
      <section className="relative z-10 px-6 md:px-10 lg:px-16 py-14">
        <div className="max-w-[1700px] mx-auto">
          <h3 className={`d7-reveal font-italiana text-[36px] md:text-[58px] mb-8 exp-light`}>Related Expertise</h3>
          <div className="grid md:grid-cols-2 gap-5 mb-14">
            {data.relatedExpertise.map((item) => (
              <Link
                key={item.slug}
              ...
              >
                <div className="relative h-[220px] md:h-[260px]">
                  <Image src={item.image} alt={item.title} fill className="object-cover group-hover:scale-[1.03] transition-transform duration-500" unoptimized />
                  <div className="absolute inset-0 bg-gradient-to-t from-black/70 to-transparent" />
                  <span className="absolute bottom-4 left-4 font-merriweather text-[11px] uppercase tracking-[0.14em] exp-mint">
                    {item.title}
                  </span>
                </div>
          ...
          <h3 className={`d7-reveal font-italiana text-[36px] md:text-[58px] mb-8 exp-light`}>Related Services</h3>
          <div className="grid md:grid-cols-3 gap-5">
            {data.related.map((service) => (
              <Link key={service.slug} href={`/services1/${service.slug}`} className={`d7-reveal rounded-xl overflow-hidden block transition-transform duration-300 hover:-translate-y-1 cursor-pointer ${cardTheme}`}>
...
export default memo(ExpertiseDetailTemplate);
