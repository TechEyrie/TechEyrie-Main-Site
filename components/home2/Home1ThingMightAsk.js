...
function FAQItem({ item, isLast }) {
  ...
  return (
    <div>
      ...
      >
        {/* Question — increased ~50% from text-[12px] → text-[17px] */}
        <span className="font-ppneue text-[15px] sm:text-[16px] md:text-[17px] font-normal leading-[1.45] tracking-[0.02em] text-[#1a1a1a] uppercase">
          {item.q}
        </span>
        <span
          ref={arrowRef}
      ...
      >
        {/* Answer — increased ~50% from text-[11px] → text-[15px] */}
        <p className="font-ppneue pb-5 text-[13px] sm:text-[14px] md:text-[15px] font-light leading-[1.85] tracking-[0.03em] text-[#1a1a1a]/55 uppercase">
          {item.a}
        </p>
      </div>

      {isLast && <div className="h-px w-full bg-[#1a1a1a]/15" />}
    </div>
  );
}
...
function FAQGroup({ group, groupRef }) {
  return (
    <div ref={groupRef}>
      {/* Category — regular font weight, increased size */}
      <h3 className="font-ppneue mb-8 text-[28px] sm:text-[32px] md:text-[36px] font-normal leading-tight tracking-[-0.01em] text-[#1a1a1a] uppercase">
        {group.category}
      </h3>
...
export default function Home10FAQ() {
  ...
  useGSAP(
    () => {
      // ── Heading — trigger on headingWrapRef (stable DOM node) ──
    ...
    >
      <div className="mx-auto max-w-[1280px]">
        ...
        <div ref={headingWrapRef} className="mb-20 sm:mb-24">
          <h2 className="font-ppneue text-left text-[52px] sm:text-[72px] md:text-[88px] lg:text-[104px] xl:text-[116px] font-semibold leading-[0.9] tracking-[-0.03em] text-[#1a1a1a] uppercase">
