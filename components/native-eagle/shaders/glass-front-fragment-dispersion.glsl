...
void main() {
  ...
  vec3 texel;
  vec3 pal;
  // Runtime loop — Three.js #pragma unroll_loop does not expand in ShaderMaterial
  for (int i = 0; i < 5; i++) {
  ...
  #ifdef EAGLE2_TEAL_LIFT
    color = max(color, vGlassColor * min(dot(color, vec3(0.2126, 0.7152, 0.0722)) * 3.2, 1.0));
    color.g = min(color.g * 1.18, maxColorValue);
    color.b = min(color.b * 1.06, maxColorValue);
  #endif

  color = clamp(color, 0., maxColorValue);

  #ifdef GLASS_DEBUG_SOLID
    gl_FragColor = vec4(vGlassColor, 1.0);
    return;
  #endif
