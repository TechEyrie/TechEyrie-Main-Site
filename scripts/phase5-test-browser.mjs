/**
 * Phase 5 — browser validation for GlassFront + dual-pass (requires dev server).
 * Run: node scripts/phase5-test-browser.mjs
 */
import fs from "fs";
  ...
  try {
    ...
    await page.waitForFunction(
      () => {
        ...
        return d?.loaded === true && d?.glassFrontShaderCount >= 17 && d?.glassBackShaderCount >= 17;
      },
      null,
    ...
    result.debug = await page.evaluate(() => {
      ...
      return {
        ...
        glassFrontPlaceholderCount: d.glassFrontPlaceholderCount,
    ...
    result.gates = {
      ...
      glassFrontShaders: d.glassFrontShaderCount >= 17,
      glassBackNoPlaceholder: d.glassBackPlaceholderCount === 0,
      glassFrontNoPlaceholder: d.glassFrontPlaceholderCount === 0,
...
const tmpScript = path.join(outDir, "_phase5-browser.cjs");
