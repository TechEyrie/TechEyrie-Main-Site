/**
 * eagle-project-3 — copy of eagle-project-2 setup with pure white background.
 * Re-run: node scripts/setup-eagle-project-3.mjs
 */
import fs from "fs";
...
const dest = path.join(root, "public/eagle-project-3");
...
console.log("Creating eagle-project-3 from eagle-project-1 …");
rimraf(dest);
...
html = html.replace(/\/eagle-project-1\//g, "/eagle-project-3/");
html = html.replace(
  'baseURL: "/eagle-project-1/"',
  'baseURL: "/eagle-project-3/"',
);

const isolatedCss = `    <style id="eagle-project-3-base-style">
      /* eagle-project-3: eagle on pure white — no lavender/pink bed */
      html, body {
      ...
      html, body, canvas, .fixed.inset-0 {
        background: #ffffff !important;
      }
...
if (!html.includes("eagle-project-3: eagle on pure white")) {
...
html = html.replace(/\[\s*20\s*\]/, "[20]");

html = html.replace(/"1",\s*\n\s*10,/, '"1",\n        1,');

html = html.replace(
  "<title>Eagle — isolated</title>",
  "<title>Eagle — white background</title>",
);
...
const patch3 = spawnSync(
  process.execPath,
  [path.join(__dirname, "patch-eagle-3-isolated.mjs")],
...
if (patch3.status !== 0 || patchTeal.status !== 0) {
  console.warn("Some patches reported issues");
}

console.log("Done:", dest);
console.log("Open: /eagle-project-3/");
